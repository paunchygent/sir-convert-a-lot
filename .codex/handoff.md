...
1. For STT Task 352, do not start Story 53 yet. FasterWhisper is now proven on the Hemma ROCm sidecar lane with codec probing restored. Continue through Task 354: provision/accept the required Hugging Face gated access for the pyannote diarization pipeline first, then rerun live observation/profile proof; govern an alternative real diarization backend only if pyannote access cannot be provisioned and the replacement satisfies exact and min/max speaker hints.
