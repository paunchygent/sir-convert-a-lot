...
- [x] One local gradient-membership proof exists for the semantic-only lane.
- [x] The proof fails if scaffold ids can still appear in
  `text_embedding.weight.grad`.
- [x] The proof is linked as the required gate before any new Hemma long proof.

## Acceptance Criteria

- [x] The synthetic proof asserts that only semantic ids can appear in
  `text_embedding.weight.grad` row membership.
- [x] A deliberately distinct scaffold-id set is shown not to enter the
  trainable text-embedding gradient row set.
- [x] This proof becomes the first required validation after Candidate 1 code
