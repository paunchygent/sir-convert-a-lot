"""Focused tests for the row-local outlier Qwen stability lab row-local outlier assessment.

Purpose:
    Lock the post-sub-talker disagreement row-local classification contract so the strongest
    output-scale member can only open one truthful row-local micro-family branch.

Relationships:
    - Exercises `qwen_row_local_outlier_assessment.py`.
...
def test_build_row_local_outlier_assessment_marks_row_local_difference() -> None:
    """Pair plus row 13 agreement with line 4 upstream should stay row-local."""
    assessment = build_row_local_outlier_assessment(
        settings=_settings(),
    ...
    assert assessment == QwenRowLocalOutlierAssessment(
        ...
        next_micro_family_rule=(
            "ROW_LOCAL_MICRO_FAMILY may test one upstream row-local micro-family only against "
            "`talker_core.layer_16.input_layernorm.output`."
        ),
    )


def test_build_row_local_outlier_assessment_marks_pair_masking() -> None:
...
def test_build_row_local_outlier_assessment_marks_non_repeatable_outlier() -> None:
