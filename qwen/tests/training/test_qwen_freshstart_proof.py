"""Tests for the Qwen backward-lineage and fresh-start proof lane fresh-start proof surface.
...
Relationships:
    - Exercises `qwen_freshstart_proof.py`.
...
def test_prepare_writes_freshstart_package(
    ...
    """Preparing the Qwen fresh-start proof should write a deterministic proof package."""
    result = main(
        ["prepare", "--output-root", tmp_path.as_posix(), "--proof-id", "freshstart-proof"]
    )
    capsys.readouterr()

    assert result == 0
    proof_root = tmp_path / "freshstart-proof"
    ...
    assert config_payload["command_name"] == "qwen-freshstart-proof"
    ...
    assert "qwen-freshstart-proof launch --proof-id freshstart-proof" in plan_markdown
    assert "swedish_pilot_train lines 1..16" in plan_markdown
    assert "Mini-bundle materialized" in checklist_markdown


def test_launch_uses_remote_qwen_lineage_surface(
    tmp_path: Path,
    ...
    def fake_run(
        ...
        return subprocess.CompletedProcess(
            command,
            0,
            json.dumps({"proof_id": "freshstart-proof", "remote_launch_root": "/srv/scratch/x"}),
            "",
        )

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.qwen_freshstart_runtime.subprocess.run",
        fake_run,
    )

    result = main(
        ["launch", "--output-root", tmp_path.as_posix(), "--proof-id", "freshstart-proof"]
    )
    capsys.readouterr()
    ...
    assert (tmp_path / "freshstart-proof" / "launch.json").exists() is True
...
def test_remote_launch_materializes_bundle_and_calls_qwen_train(
    ...
    def fake_launch(command: list[str], *, label: str) -> dict[str, object]:
        del label
        calls.append(list(command))
        return {"launch_id": "freshstart-proof-freshstart", "container_name": "qwen-train"}

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.qwen_freshstart_proof.run_local_qwen_train_json",
        fake_launch,
    )
    ...
    remote_bundle_root = tmp_path / "remote-proof" / "freshstart-proof" / "mini-bundle"
    assert (
