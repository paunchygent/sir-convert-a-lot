"""
Focused tests for the row-local micro-family Qwen stability lab row-local micro-family assessment.

Purpose:
    Lock the post-row-local outlier family-classification contract so the fp32-output-cap
    family opens only the next truthful mechanism branch.

Relationships:
    - Exercises `qwen_micro_family_assessment.py`.
...
def test_validate_row_local_micro_family_contract_rejects_wrong_family() -> None:
    """The row-local micro-family probe should accept only the fixed fp32-output-cap family."""
    settings = _settings(stabilization_variants=(ROW_LOCAL_MICRO_FAMILY_BASELINE_VARIANT, "off"))

    with pytest.raises(SystemExit, match="supports only the fixed baseline plus fp32 output-cap"):
        validate_row_local_micro_family_contract(settings)


def test_build_row_local_micro_family_assessment_marks_upstream_persistent() -> None:
    """Both cap variants keeping line 4 upstream should close the family as persistent."""
    assessment = build_row_local_micro_family_assessment(
        settings=_settings(),
...
def test_build_row_local_micro_family_assessment_marks_converged_downstream() -> None:
    ...
    assert assessment.winning_candidate_variant == ROW_LOCAL_MICRO_FAMILY_CANDIDATE_VARIANTS[0]
    assert assessment.dominant_surface == "talker_core.layer_15.output"


def test_build_row_local_micro_family_assessment_marks_non_local_regression() -> None:
...
def _settings(
    *,
    stabilization_variants: tuple[str, ...] = ROW_LOCAL_MICRO_FAMILY_REQUIRED_VARIANTS,
) -> QwenStabilityLabSettings:
    return QwenStabilityLabSettings(
        ...
        hook_profile=TALKER_CORE_POST_SUB_TALKER_DISAGREEMENT_ROW_LOCAL_OUTLIER_HOOK_PROFILE,
...
def _matrix_rows(
    *,
    ...
    return tuple(
        ...
        for variant, variant_rows in zip(
            ROW_LOCAL_MICRO_FAMILY_REQUIRED_VARIANTS, family_rows, strict=True
        )
