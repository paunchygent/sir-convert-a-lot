"""Focused tests for Qwen backward-lineage and fresh-start proof lane backward-lineage hook session
plumbing.

Purpose:
    Lock the reversible input-layernorm internal input-layernorm wrapper lifecycle and the exact
    internal gradient-observation chain without depending on the broader
    backward-lineage probe runtime.

Relationships:
    - Exercises `qwen_backward_lineage_hooks.py` directly.
    - Reuses `sft_12hz_talker_core_trace.py` for the canonical input-layernorm internal trace names.
"""
...
def test_input_layernorm_internal_hook_session_wraps_input_layernorm_and_restores_forward() -> None:
    """The input-layernorm internal session should capture the full internal chain and restore the
    module.
    """
...
def test_layer15_residual_output_hook_session_wraps_layer15_mlp_and_captures_residual_path() -> (
    None
):
    """The residual/output session captures residual input, sum, output, and handoff."""
...
def test_layer15_output_return_hook_session_captures_pre_scale_and_emitted_output() -> None:
    """The output-return session should isolate the winner-specific layer-15 return split."""
    model = _FakeResidualRootModel()
    layer_15 = model.talker.model.layers[15]
    layer_16 = model.talker.model.layers[16]
    session = build_gradient_hook_session(
...
def test_fp32_scaled_layer15_output_hook_session_captures_fp32_scaled_and_emitted_output() -> None:
    """The fp32-scaled output session should isolate the fp32-scaled versus emitted output seam."""
    model = _FakeResidualRootModel()
    layer_15 = model.talker.model.layers[15]
    layer_16 = model.talker.model.layers[16]
    session = build_gradient_hook_session(
