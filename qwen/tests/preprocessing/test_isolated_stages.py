"""Tests for isolated Qwen preprocessing stage orchestration."""
...
import pytest
from scripts.sir_convert_a_lot.cli.ml.qwen_isolated_stages import (
    _build_parser,
...
def test_isolated_stages_parser_launch_defaults() -> None:
    """The runner should default to stage auto and detached launch."""
    parser = _build_parser()
    args = parser.parse_args(["launch"])

    assert args.stage == "auto"
    assert args.promote_on_success is False
    assert args.resume_row_processing is False


def test_isolated_stages_parser_accepts_resume_row_processing_flag() -> None:
    """The runner should accept row-processing resume launches."""
    parser = _build_parser()
...
def test_isolated_stages_parser_accepts_stop_command() -> None:
    """The runner should expose a committed stop command."""
    parser = _build_parser()
...
def test_isolated_stages_stage_selector_auto_uses_existing_run_state(tmp_path: Path) -> None:
...
def test_inspect_detached_stage_reads_container_status_and_outputs(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The detached status view should combine Docker and preprocessing state."""
    run_root = tmp_path / "run"
    run_root.mkdir(parents=True, exist_ok=True)
    (run_root / "status.json").write_text(
    ...
    launch = DetachedStageLaunch(
        generated_at="2026-03-09T12:00:00Z",
        launch_id="qwen-finalization-20260309t120000z",
        stage="finalization",
        container_name="qwen-finalization-20260309t120000z-container",
        container_id="container-id",
        repo_root="/home/paunchygent/apps/sir-convert-a-lot",
        run_root=run_root.as_posix(),
        promoted_root="/srv/scratch/sir-convert-a-lot/build/reference/qwen3-tts-swedish-corpus",
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.preprocessing.isolated_stages.docker_checked",
    ...
    assert status.status_found is True
    assert status.report_found is True
    assert status.report is not None
    assert status.report["prepared_rows"] == 52
...
def test_load_optional_json_uses_sudo_fallback_on_permission_error(
    ...
    """Unreadable artifacts should still load through sudo fallback."""
    ...
    def _fake_subprocess_checked(command: list[str], *, label: str) -> str:
        ...
        assert label == "sudo cat qwen detached artifact"
        return json.dumps({"status": "running"})

    monkeypatch.setattr(Path, "read_text", _fake_read_text)
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.preprocessing.isolated_stages.subprocess_checked",
...
def test_stop_command_writes_stop_metadata(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The stop command should persist deterministic stop metadata."""
    output_root = tmp_path / "verification"
    launch_root = output_root / "qwen-row-processing-20260309t120000z"
    launch_root.mkdir(parents=True, exist_ok=True)
    launch = DetachedStageLaunch(
        generated_at="2026-03-09T12:00:00Z",
        launch_id="qwen-row-processing-20260309t120000z",
        stage="row-processing",
        container_name="qwen-row-processing-20260309t120000z-container",
        container_id="container-id",
        repo_root="/home/paunchygent/apps/sir-convert-a-lot",
        run_root="/srv/scratch/sir-convert-a-lot/build/runs/qwen3-tts-swedish-preprocessing/run-id",
        promoted_root="/srv/scratch/sir-convert-a-lot/build/reference/qwen3-tts-swedish-corpus",
        command=["sudo", "-n", "docker", "run", "-d"],
    )
    (launch_root / "launch.json").write_text(json.dumps(asdict(launch)) + "\n", encoding="utf-8")

    def _fake_stop_detached_stage(payload: DetachedStageLaunch) -> DetachedStageStop:
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.cli.ml.qwen_isolated_stages.stop_detached_stage",
