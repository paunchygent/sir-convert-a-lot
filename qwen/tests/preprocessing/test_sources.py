"""Source-adapter and staged-corpus tests.
...
Relationships:
    - Tests the canonical source-adapter modules and staged public-corpus loader.
    - Reuses deterministic fixture helpers from `test_support`.
"""
...
import pytest
from scripts.sir_convert_a_lot.ml.qwen.preprocessing.family_assignment import (
...
from scripts.sir_convert_a_lot.ml.qwen.preprocessing.sources.fleurs import fleurs_sv_source_records
...
def test_rixvox_source_records_from_parquet_stops_after_max_rows_during_iteration(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.preprocessing.sources.rixvox.pq.ParquetFile",
        _FakeParquetFile,
    )
...
def test_build_rixvox_audio_locator_index_stops_after_required_paths_are_resolved(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.preprocessing.sharding.tarfile.open",
