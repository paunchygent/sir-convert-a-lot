...
- 2026-03-15:
  ...
  - Opened `T185`: the preserved legacy Task 101 lane can relaunch without manual JSON edits and can override a stale bundle root, but the first probe exposed stale reused-run artifacts and an impossible saved `next_step_in_epoch` that did not fail closed against the replacement bundle `dataloader_length`. Follow-up landed stale-artifact filtering, a trainer-side resume-cursor guard, and a standalone eval baseline for the original `1236` checkpoint (`eval_loss=6.440637648105621` over `8` eval batches). That probe also wrote a newer compatible durable checkpoint at `1238`, so the canonical next relaunch is now strict resume from `state-step-00001238`, not another rollback to `1236`. Live recovery progress is now tracked in `docs/reference/ref-task101-training-eval-pilot-progress-2026-03-15.md`, while the March 13 pipeline analysis is historical evidence only.
  - Completed `T185`: `qwen-train resume` can now explicitly override checkpoint cadence, eval cadence, and retention when a preserved legacy lane must be promoted onto the current scheduled-control contract. Hemma is now running the canonical recovered lane under launch `20260315T110545Z`, resumed from `state-step-00001238` with truthful `500/100/3` status fields, and abandoned failed launch roots plus stale detached monitor workers from the failed relaunch attempts were cleaned up so only the active Qwen training container remains.
  - The strict `1238` relaunch then completed one real held-out eval at optimizer step `1300` (`best_eval_loss=6.574727833271027`) before failing at optimizer step `1358` with `NonFiniteLossError` after `3` consecutive non-finite optimizer steps. The latest durable checkpoint still remained `state-step-00001238`, while live `phase_history` misleadingly reused the same `checkpoint-save` label for durable trainer-state saves and export-only epoch/final saves. `T180` delivered the first-pass truth layer for bounded non-finite forensics, checkpoint-phase truth, sampler truth, and explicit epoch semantics.
  - The next detached replay from the same `1238` checkpoint failed later at optimizer step `1408` and proved the remaining root cause is at the optimizer boundary: step `1405` still had finite losses but a non-finite `grad_norm`, and step `1406` entered with `input_text_embedding` already poisoned. New task `T186` now owns deterministic optimizer-boundary replay, targeted text-embedding / text-projection parameter probes, targeted optimizer-state probes, and the fail-closed guard that must land before `T179` can run again.
  - Opened Story 28 with `T187-T191` as the permanent architecture-hardening lane for the Qwen training control plane. `RULE-095` now codifies the `400` LoC cap, no-shim/no-compat-wrapper policy, DRY/SOLID/SRP/DDD expectations, and the requirement to split `qwen_train.py`, `orchestrator.py`, `reporting.py`, and `sft_12hz_loop.py` into bounded modules before more feature logic lands there.
...
- Task 101 live follow-on: monitor the active recovered lane `20260315T110545Z`, capture the next durable checkpoint and held-out eval outputs, and continue appending operator truth to `docs/reference/ref-task101-training-eval-pilot-progress-2026-03-15.md`.
- Story 28 follow-on: finish docs-first implementation of `T188-T191`, migrate imports in one pass without compatibility wrappers, and add permanent architecture guard tests so the Qwen control-plane god files cannot regrow.
