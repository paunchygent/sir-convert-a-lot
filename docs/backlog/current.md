...
- Current ownership-governance follow-on is to promote the `88` quarantined canonical processed-root conflicts into an explicit exclusion/ledger surface so future shard allocation can treat them as unavailable automatically instead of relying on operator memory.
