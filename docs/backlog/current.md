...
- 2026-03-15:
  ...
  - The strict `1238` relaunch then completed one real held-out eval at optimizer step `1300` (`best_eval_loss=6.574727833271027`) before failing at optimizer step `1358` with `NonFiniteLossError` after `3` consecutive non-finite optimizer steps. The latest durable checkpoint still remained `state-step-00001238`, while live `phase_history` misleadingly reused the same `checkpoint-save` label for durable trainer-state saves and export-only epoch/final saves. `T180` is now the active remediation slice for bounded non-finite forensics, checkpoint-phase truth, and explicit epoch-semantics reporting before the next `T179` bounded retry.
  - `T180` follow-up now also owns default-on per-microbatch row provenance plus ordered tensor-finiteness probes for the failing optimizer-step window, and the bucketed sampler no longer relies on hidden process-global `random.shuffle(...)`; train shuffling is now explicit and eval batching is deterministic by contract.
