...
- 2026-03-12:
  - Completed `T148-T149`: Task 101 pilot-bundle materialization now stages copy, bounded `finalize-batch`, and final `assemble`, persists `task101_pilot_bundle_plan.json`, `task101_pilot_bundle_events.jsonl`, and `task101_pilot_bundle_status.json`, and finalization now runs inside the governed Task 100/101 Qwen image with the fixed in-container HF cache contract (`/cache/huggingface`) instead of the host PDM environment. Same-day follow-up also added bundle/per-batch runtime provenance so legacy host-generated shards fail closed, kept the selected bundle root host-visible inside the container so status/report paths do not drift, and preserved direct `finalize-batch` plus `build` under the same governed runtime contract.
  - Opened and implemented `T150` as the immediate throughput correction after the live Hemma Task 101 bundle run showed the governed runtime was still CPU-bound during `audio_codes` generation. The active host-runtime bundle build at `/srv/scratch/sir-convert-a-lot/build/reference/qwen3-tts-swedish-task101-pilot-bundle-20260312h` was intentionally stopped after `swedish_pilot_train:batch-00012` completed and `batch-00013` had started so the resumable bundle root can be retried under the new GPU-backed tokenizer posture. The governed Task 101 batch runtime now loads `Qwen3TTSTokenizer` onto `cuda:0` with `bfloat16` plus `flash_attention_2`, writes `reports/task101_pilot_bundle_audio_codes_runtime.json`, and fails closed instead of silently continuing on CPU.

## Next Actions

- Current Task 101 follow-on is to pull the new governed runtime on Hemma, rebuild the Task 100/101 Qwen image if needed, and rerun the stopped pilot-bundle root so incomplete `swedish_pilot_train:batch-00013` and later batches regenerate under the GPU-backed tokenizer posture with `reports/task101_pilot_bundle_audio_codes_runtime.json` as evidence.
