...
- 2026-03-11:
  ...
  - Completed `T140` to freeze the canonical pilot dataset and make conflict exclusions enforceable: canonical processed roots now emit owned-row, conflict-row, and freeze-summary artifacts, Task 121 exclusion flows now accept explicit `--exclude-row-keys-path` manifests, and future shard allocation can exclude quarantined conflict rows by contract instead of relying on memory.
...
- Current ownership-governance follow-on is to rebuild the first post-pilot shard registry from the remaining universe using the frozen pilot canonical root plus its conflict-row manifest as mandatory exclusions.
