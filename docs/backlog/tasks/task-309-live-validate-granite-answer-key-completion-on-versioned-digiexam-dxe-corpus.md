...
- [ ] Detached resource-monitor launch/status/summary artifacts for the live
  validation window.
- [ ] Provider microprobe report covering vLLM `choice`, JSON Schema choice
  object, and JSON Schema gap-fill object.
- [ ] In-process full-corpus advisory live-validation JSON report and Markdown
...
- [ ] Hemma preflight proves remote revision, ROCm/GPU state, scratch-backed
  cache path, port `8017`, vLLM `/v1/models`, disabled request logging,
  localhost-only exposure, and no CPU fallback.
- [ ] Long-run execution uses a committed detached runner/status surface and
  retains structured reports outside git.
- [ ] The persistent Granite/vLLM container is left running after validation,
...
- [ ] Detached resource monitoring runs alongside the live validation and is
...
- [ ] Unknown IDs and duplicate IDs are counted explicitly and must be zero for
  promotion.
- [ ] Wrong-but-valid answers are the primary safety metric; promotion should
  require zero wrong-but-valid answers unless a later governed decision accepts
  a narrower risk posture.
- [ ] Manual follow-up is acceptable and safer than plausible wrong answer-key
  completion.
- [ ] Persistent failure paths are documented across the run without
...
- [ ] Report-evaluation tests cover wrong-but-valid, manual follow-up,
  ...
  backend failure-code aggregation.
...
- The live Hemma container was not launched in this local implementation
  slice. Provider microprobe, in-process advisory, deployed-service smoke,
  resource-monitor, and reviewed-apply reports remain live-execution outputs,
  not local dry-run evidence.
