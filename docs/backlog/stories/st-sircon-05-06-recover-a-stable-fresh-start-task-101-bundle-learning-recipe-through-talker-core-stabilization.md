...
type: story
id: ST-SIRCON-05-06
title: Recover a stable fresh-start Task 101 bundle-learning recipe through talker-core
  stabilization
...
links:
  decisions: []
acceptance_criteria:
- '`T214` is recorded as closed discovery evidence rather than left as the active
  explanation lane.'
- One lightweight Story 31 exploration surface exists for rapid matrix iteration on
  the late-middle talker-core seam without a proof package per hypothesis.
- One bounded talker-core stabilization surface exists that targets the late-middle
  seam exposed by `T214` without reopening replay framing or altering the clean text-token
  semantics.
- One smallest-signal local promotion gate exists that tests the exact fresh-start
  failure family before any governed Hemma run.
- One exact-control runtime surface exists for the original restored Task 101 recipe
  with only the `T206` token-span correction, so the repo can answer the practical
  full-bundle control question without approximation.
- Only the first promoted candidate gets a short governed fresh-start Hemma proof.
- The restart target remains blocked until the stabilization lane records a truthful
  success surface that justifies a larger clean-start proof.
retired_ids:
- story-31-recover-a-stable-fresh-start-task-101-bundle-learning-recipe-through-talker-core-stabilization
---
...
- reuse [qwen_freshstart_bundle.py](./scripts/sir_convert_a_lot/ml/qwen/training/qwen_freshstart_bundle.py)
  for tiny truthful train-slice materialization
- reuse [qwen_backward_lineage_bundle.py](./scripts/sir_convert_a_lot/ml/qwen/training/qwen_backward_lineage_bundle.py)
  when exact selected failing rows are the sharper input than a prefix slice
- reuse [backward_lineage_probe.py](./scripts/sir_convert_a_lot/ml/qwen/training/backward_lineage_probe.py)
  as the fastest existing forward/backward experiment kernel on the real lane
- reuse [qwen_backward_lineage_hooks.py](./scripts/sir_convert_a_lot/ml/qwen/training/qwen_backward_lineage_hooks.py)
  and [sft_12hz_talker_core_trace.py](./scripts/devops/qwen_finetuning_patches/sft_12hz_talker_core_trace.py)
  for hook profiles and layer-target resolution
- reuse [qwen_freshstart_runtime.py](./scripts/sir_convert_a_lot/ml/qwen/training/qwen_freshstart_runtime.py)
  plus `qwen-train launch/status` only when a candidate is promoted
- reuse [qwen_backward_lineage_detached.py](./scripts/sir_convert_a_lot/ml/qwen/training/qwen_backward_lineage_detached.py)
