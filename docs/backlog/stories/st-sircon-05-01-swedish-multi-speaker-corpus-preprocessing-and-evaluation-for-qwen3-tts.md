...
links:
  decisions: []
acceptance_criteria:
- Task 105 publishes the research map and repomix package anchoring the external evidence
  pass before locking the pilot subset.
- Task 102 defines the multi-speaker Swedish corpus policy and first bounded training,
  development, and evaluation split.
- Task 103 defines and implements the preprocessing and manifest pipeline for Swedish
  Qwen full-finetune runs.
- Task 106 extends the first slice to revision-pinned rixvox, fleurs, and labeled
  waxholm inputs without deprecated dataset-script loading.
- Task 107 proves the staged public-corpus preprocessing bundle on Hemma and records
  the remaining rixvox blocker before T101.
- Task 108 resolves the rixvox audio-backed train-manifest blocker before the first
  bounded Hemma pilot.
- Task 109 remediates runtime-model drift so public-corpus preprocessing uses the
  selected Qwen container runtime rather than the Hemma host venv.
- Task 110 hardens the public-corpus lane into a disk-backed row-processing and finalization
  pipeline before larger rixvox runs scale out.
- Task 119 removes the rixvox train preflight bottleneck so bounded Hemma launches
  test actual worker settings.
- Task 116 expands the staged rixvox train pool and proves one sustained detached
  row-processing window toward the bounded Hemma pilot corpus.
- Task 111 defines a provenance-safe ASR relabeling lane without silently replacing
  source transcripts.
- Task 104 publishes the Colab H100 scaling lane and compares it against the Hemma
  pilot.
- Tasks 134-139 define and synchronize canonical ownership, deduplication, immutable
  shard registry, and shard-id-only work allocation.
- Task 140 freezes the canonical pilot root into an explicit ownership and conflict
  artifact family with quarantined rows excluded from future allocation.
- The story documents the difference between the official Qwen single-speaker training
  surface and this repository's planned multi-speaker Swedish language-expansion objective.
retired_ids:
- story-24-swedish-multi-speaker-corpus-preprocessing-and-evaluation-for-qwen3-tts
---
