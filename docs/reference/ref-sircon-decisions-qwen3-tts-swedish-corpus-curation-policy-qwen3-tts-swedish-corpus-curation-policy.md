---
type: reference
id: REF-SIRCON-DECISIONS-qwen3-tts-swedish-corpus-curation-policy
...
summary: Qwen3-TTS Swedish Corpus Curation Policy
retired_ids:
- REF-qwen3-tts-swedish-corpus-curation-policy
---

## Outcome And Purpose

State the outcome whose material choices this ledger must close.

## Decision Boundary

State which choices this ledger owns, which authority it inherits, and which
decisions belong instead in an ADR, plan, backlog item, review, or runbook.
...
Include every material requirement, non-goal, dependency, assumption, proof
obligation, authority boundary, and completion condition. Keep normative rows
open until the user approves them.

## Closure State

List every open row and the downstream work it blocks. When all rows close,
state the accepted contract and the planning or backlog action it permits.
Ledger closure does not itself authorize implementation.
