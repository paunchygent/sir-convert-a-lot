---
type: reference
id: REF-SIRCON-RESEARCH-sir-convert-gateway-cutover-caller-inventory
...
summary: Sir Convert Gateway Cutover Caller Inventory
retired_ids:
- REF-sir-convert-gateway-cutover-caller-inventory
---
...
| Skriptoteket Conversion Hub | Historical pre-cutover inventory found `apps_conversion_hub.py`, `conversion_hub_jobs.py`, and `sir_convert_client_v2.py` submitting user-owned conversion jobs to `/v2/convert/jobs`, polling status, and downloading artifacts; at that time production env defaulted to `https://convert.hule.education` with `SIR_CONVERT_A_LOT_V2_API_KEY`. | superseded direct public host path | Gateway/user-originated context with `InternalIdentityContextV1` audience `sir-convert-a-lot`; same-host internal transport can be retained only with verified context-derived ownership | Current production proof is via Skriptoteket through HuleEdu browser-session/Gateway. The public Sir Convert host remains reserved/fail-closed; see Story 57 / Task 373 / PR-0406 proof artifact `20260629T152928Z`. |
...
| HuleEdu Gateway | product/browser public entry | `/sir-convert/v2/convert/...` Gateway route family | HuleEdu browser session + CSRF | Gateway routes sign `InternalIdentityContextV1` with audience `sir-convert-a-lot` | Retained production proof for the DigiExam answer-key review-state handshake is tracked by Story 57 / Task 373 / PR-0406 artifact `20260629T152928Z`. |
