---
type: reference
id: REF-pdf-telemetry-overhead-evidence
...
Capture deterministic synthetic load evidence for conversion telemetry overhead to quantify telemetry overhead and confirm
bounded-cardinality metric behavior under sustained queued-job execution.

## Benchmark Command

```bash
pdm run benchmark:pdf-telemetry-overhead \
  --total-jobs 40 \
  --max-workers 8 \
  --stub-work-seconds 0.2 \
  --output-json build/benchmarks/pdf-throughput/pdf-telemetry-overhead-local.json \
  --data-root build/benchmarks/pdf-throughput/pdf-telemetry-overhead-runtime
```
