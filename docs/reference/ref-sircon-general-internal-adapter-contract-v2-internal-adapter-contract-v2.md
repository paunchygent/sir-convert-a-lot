---
type: reference
id: REF-SIRCON-GENERAL-internal-adapter-contract-v2
...
- Skriptoteket and HuleEdu adapters may submit governed audio transcription
  jobs through the route-specific contract once their downstream lanes wire the
  existing Service API v2 transport.
- The adapter must remain transport-only: it builds headers and job specs,
...
- After the Gateway/internal identity cutover, user-originated internal calls
  ...
  `docs/reference/ref-sircon-general-sir-convert-internalidentitycontextv1-authorization-profile-sir-convert-internalidentitycontextv1-authorization-profile.md`.
