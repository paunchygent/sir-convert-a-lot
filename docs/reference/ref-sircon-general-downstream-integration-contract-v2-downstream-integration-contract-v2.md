---
type: reference
id: REF-SIRCON-GENERAL-downstream-integration-contract-v2
...
### Skriptoteket STT progress summary

- Use the existing create/poll lifecycle and `job.progress` audio fields for UI progress; do not expose transcript text, speaker names, credentials, hashes, or artifact bytes in telemetry.
- Preserve safe stage markers (`queued`, `starting`, `probing_media`, `normalizing_audio`, `diarizing`, `transcribing`, `aligning_segments`, `packaging`, `succeeded`, `failed`, `canceled`) and monotonic audio/pipeline progress semantics.
- Failed or canceled jobs remain terminal and `/result`/`/artifact` return `409 job_not_succeeded`; partial transcript artifacts are not exposed.
...
- Route-specific requests use the v2 multipart `file`, `job_spec`, and optional constrained companions described above.
- Callers use the canonical v2 lifecycle; route examples remain in the source contract authorities.
