"""
Runtime helpers for the Qwen backward-lineage and fresh-start proof lane fresh-start proof surface.
...
Relationships:
    - Used by `qwen_freshstart_proof.py` for prepare/launch/status flows.
...
def ensure_remote_scratch_headroom(config: QwenFreshstartProofConfig) -> dict[str, object]:
    ...
    if result.returncode != 0 and result.stdout.strip() == "":
        raise SystemExit(
            "Qwen fresh-start proof scratch audit failed.\n"
            f"stdout:\n{result.stdout.strip()}\n"
            f"stderr:\n{result.stderr.strip()}"
        )
    ...
    if payload.get("meets_required_headroom") is True:
        return payload
    raise SystemExit(
        "Qwen fresh-start proof launch blocked because Hemma scratch headroom "
...
def remote_launch_proof_args(config: QwenFreshstartProofConfig) -> list[str]:
...
def remote_status_proof_args(config: QwenFreshstartProofConfig) -> list[str]:
...
def qwen_train_launch_args(config: QwenFreshstartProofConfig) -> list[str]:
...
def qwen_train_status_args(config: QwenFreshstartProofConfig) -> list[str]:
