"""Execution helpers for the Qwen stability lab deterministic parity probe.
...
def build_runtime_args(
    ...
    *,
    ...
    return argparse.Namespace(
        ...
        tracker_project_name="qwen-swedish-finetune",
...
def require_training_dataset(prepared: PreparedTrainingRun) -> TTSDataset:
    ...
    if not isinstance(dataset, TTSDataset):
        raise TypeError(
            "Qwen stability lab parity probe requires the patched TTSDataset training surface."
        )
    return dataset
...
def select_rows(
    ...
    *,
    ...
    if missing_lines:
        raise SystemExit(
            "Qwen stability lab parity probe could not find all required rows in the selected "
