"""Gradient-hook profiles for Qwen backward-lineage and fresh-start proof lane lineage probes.
...
from scripts.sir_convert_a_lot.ml.qwen.training import (
    qwen_backward_lineage_layernorm_hooking as layernorm_hooking,
)
from scripts.sir_convert_a_lot.ml.qwen.training import (
    qwen_layer15_residual_output_hooking as layer15_residual_output_residual_output_hooking,
)
...
class GradientHookSession:
    ...
    def install_pre_forward_hooks(self, *, model: object) -> None:
        ...
        if (
            ...
            self._install_row_local_outlier_trace(model=model)
            return
        if (
            self._hook_profile
            == TALKER_CORE_POST_ROW_LOCAL_MICRO_FAMILY_DOWNSTREAM_CONVERGENCE_HOOK_PROFILE
        ):
            self._install_downstream_convergence_trace(model=model)
            return
        if (
            self._hook_profile
            == TALKER_CORE_POST_DOWNSTREAM_CONVERGENCE_LAYER15_OUTPUT_SPLIT_HOOK_PROFILE
        ):
            self._install_layer15_output_split_trace(model=model)
            return
        if (
            self._hook_profile
            == TALKER_CORE_POST_LAYER15_OUTPUT_SPLIT_LAYER15_RESIDUAL_OUTPUT_HOOK_PROFILE
        ):
            self._install_layer15_residual_output_trace(model=model)
            return
        if (
            self._hook_profile
            == TALKER_CORE_POST_LAYER15_RESIDUAL_OUTPUT_LAYER15_OUTPUT_RETURN_HOOK_PROFILE
        ):
            self._install_layer15_output_return_trace(model=model)
            return
        if (
            self._hook_profile
            == TALKER_CORE_POST_LAYER15_OUTPUT_MULTIPLY_FP32_SCALED_OUTPUT_HOOK_PROFILE
        ):
            self._install_post_layer15_output_multiply_fp32_scaled_output_trace(model=model)
            return
        if self._hook_profile == TALKER_CORE_HOOK_PROFILE:
            ...
            self._hook_profile == TALKER_CORE_POST_INPUT_LAYERNORM_OUTPUT_DISAGREEMENT_HOOK_PROFILE
        ):
            trace_targets = iter_talker_core_sub_talker_disagreement_trace_targets(model)
        else:
    ...
    def _install_input_layernorm_internal_trace(self, *, model: object) -> None:
        """Patch the layer-16 input-layernorm forward for internal tracing."""
    ...
    def _install_row_local_outlier_trace(self, *, model: object) -> None:
        """Patch the row-local line-4 corridor while keeping the state vector fixed."""
        self._install_layer16_input_layernorm_output_trace(
            model=model,
            trace_targets=iter_talker_core_row_local_outlier_trace_targets(model),
            output_name=talker_core_row_local_outlier_trace_names()[-1],
            profile_label="ROW_LOCAL_OUTLIER",
        )

    def _install_downstream_convergence_trace(self, *, model: object) -> None:
        """Patch the downstream convergence downstream corridor while keeping the winner fixed."""
        self._install_layer16_input_layernorm_output_trace(
            model=model,
            trace_targets=iter_talker_core_downstream_convergence_trace_targets(model),
            output_name=talker_core_downstream_convergence_trace_names()[-1],
            profile_label="DOWNSTREAM_CONVERGENCE",
        )

    def _install_layer15_output_split_trace(self, *, model: object) -> None:
        """Install the layer-15 split layer-15 residual/output split without changing the winner."""
        for target in iter_talker_core_layer15_output_split_trace_targets(model):
            handle = (
                target.module.register_forward_hook(
                    self._build_forward_hook(target.name, target.tensor_selector)
                )
    ...
    def _install_layer15_residual_output_trace(self, *, model: object) -> None:
        """Install the residual/output residual-input / residual-sum / output-return split."""
        layer15_residual_output_residual_output_hooking.install_layer15_residual_output_trace(
            model=model,
            attach_tensor=self._attach_tensor,
    ...
    def _install_layer15_output_return_trace(self, *, model: object) -> None:
        """Install the output-return pre-scale / post-scale layer-15 return split."""
        layer15_residual_output_residual_output_hooking.install_layer15_output_return_trace(
            model=model,
            attach_tensor=self._attach_tensor,
            build_forward_hook=self._build_forward_hook,
            build_forward_pre_hook=self._build_forward_pre_hook,
    ...
    def _install_post_layer15_output_multiply_fp32_scaled_output_trace(
        self, *, model: object
    ) -> None:
        """Install the fp32-scaled output fp32-scaled-output split beneath the fixed multiply-site
        confirmation seam.
        """
        layer15_residual_output_residual_output_hooking.install_post_layer15_output_multiply_fp32_scaled_output_trace(
            model=model,
            attach_tensor=self._attach_tensor,
            build_forward_hook=self._build_forward_hook,
            build_forward_pre_hook=self._build_forward_pre_hook,
