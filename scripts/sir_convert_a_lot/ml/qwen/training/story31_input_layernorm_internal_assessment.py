"""Focused T233 internal layernorm assessment for the Story 31 stability lab.

Purpose:
    Derive the normalization-internal pair-versus-single-row conclusion from
    the compact Story 31 matrix rows so the runner can keep orchestration
    separate from the T233 mechanism decision.

Relationships:
    - Imported by `story31_stability_lab_runner.py` for run-time validation and
      report assembly.
    - Reuses `sft_12hz_talker_core_trace.py` for the canonical T233 internal
      trace order and `story31_stability_lab_contracts.py` for the typed payload.
"""
...
from scripts.sir_convert_a_lot.ml.qwen.training.story31_stability_lab_contracts import (
    ...
    Story31InputLayernormInternalAssessment,
    Story31StabilityLabSettings,
)

T233_TARGET_LOSS_KIND = "sub_talker_loss"
T233_REQUIRED_VARIANT = LAYER16_GATED_FP32_RESCALE_1E3_LAYER16_OUT_0P5_LAYER15_OUT_0P5
T233_TARGET_INTERNAL_SURFACES = talker_core_input_layernorm_internal_trace_names()


def validate_input_layernorm_internal_contract(settings: Story31StabilityLabSettings) -> None:
    """Reject unsupported T233 variant families before the Hemma probe starts."""
    if settings.hook_profile != TALKER_CORE_INPUT_LAYERNORM_INTERNAL_HOOK_PROFILE:
        return
    if settings.stabilization_variants == (T233_REQUIRED_VARIANT,):
        return
    raise SystemExit(
        "Story 31 input-layernorm-internal probing supports only the ranked T229/T230 "
        f"baseline variant `{T233_REQUIRED_VARIANT}`."
    )
...
def build_input_layernorm_internal_assessment(
    *,
...
) -> Story31InputLayernormInternalAssessment | None:
    """Build the focused T233 assessment when the internal profile is active."""
    if settings.hook_profile != TALKER_CORE_INPUT_LAYERNORM_INTERNAL_HOOK_PROFILE:
        return None
    if settings.stabilization_variants != (T233_REQUIRED_VARIANT,):
        return None
    comparison_rows = _comparison_rows_for_input_layernorm_internal_assessment(
        ...
        stabilization_variant=T233_REQUIRED_VARIANT,
    ...
    if len(comparison_rows) != 3:
        ambiguity_reason = "Story 31 T233 could not resolve the full pair-versus-single case set."
        ...
        ambiguity_reason = "One or more required T233 cases stayed finite under the internal probe."
    elif any(row.matched_internal_surface is None for row in comparison_rows):
        ambiguity_reason = (
            "One or more required T233 cases failed outside the committed "
    ...
    return Story31InputLayernormInternalAssessment(
        stabilization_variant=T233_REQUIRED_VARIANT,
        target_loss_kind=T233_TARGET_LOSS_KIND,
        target_internal_surfaces=T233_TARGET_INTERNAL_SURFACES,
...
def _build_input_layernorm_internal_comparison_row(
    *,
    ...
    matched_internal_surface = (
        ...
        if row.first_non_finite_talker_core_hook_tensor in T233_TARGET_INTERNAL_SURFACES
        else None
    )
...
def _required_matrix_row(
    *,
    ...
    raise SystemExit(
        ...
        f"`{stabilization_variant}:{case_id}` for the T233 assessment."
    )
...
def _next_micro_family_rule(earliest_internal_surface: str | None) -> str:
    ...
    return "The next task remains blocked until the T233 ambiguity is resolved."
