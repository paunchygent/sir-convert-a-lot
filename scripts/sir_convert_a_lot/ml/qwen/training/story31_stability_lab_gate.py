"""Promotion-gate evaluation for the Story 31 talker-core stability lab.

Purpose:
    Turn one compact Story 31 matrix result into a strict promotion decision
...
Relationships:
    - Used by `story31_stability_lab.py` for the public `gate` command.
    - Consumes `results.json` emitted by `story31_stability_lab_runner.py`.
"""
...
def evaluate_promotion_gate(
    *,
    ...
    """Evaluate whether one Story 31 candidate has earned governed-proof promotion."""
    ...
    return Story31PromotionGateReport(
...
def persist_promotion_gate(
    output_root: Path, report: Story31PromotionGateReport
) -> tuple[Path, Path]:
    """Persist the compact Story 31 promotion-gate artifacts under one output root."""
...
def load_results_payload(results_path: Path) -> dict[str, object]:
    """Load one previously emitted Story 31 matrix payload from disk."""
    payload = json.loads(results_path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise SystemExit("Story 31 stability lab results payload was not a JSON object.")
    return payload


def build_promotion_gate_markdown(report: Story31PromotionGateReport) -> str:
    """Render one concise markdown summary for the Story 31 promotion gate."""
    lines = [
        "# Story 31 Promotion Gate",
        "",
...
def _rows_by_variant_case(
    ...
    if not isinstance(rows_value, list):
        raise SystemExit("Story 31 stability lab results payload was missing `matrix_rows`.")
    rows_by_variant_case: dict[tuple[str, str], dict[str, object]] = {}
    for row in rows_value:
        if not isinstance(row, dict):
            raise SystemExit("Story 31 stability lab matrix row payload was malformed.")
...
def _required_matrix_row(
    ...
    try:
        ...
        raise SystemExit(
            f"Story 31 promotion gate could not resolve required matrix row `{variant}:{case_id}`."
        ) from exc
...
def _required_str(payload: dict[str, object], key: str) -> str:
    ...
    if not isinstance(value, str):
        raise SystemExit(f"Story 31 promotion gate payload missing string `{key}`.")
    return value
...
def _required_bool(payload: dict[str, object], key: str) -> bool:
    ...
    if not isinstance(value, bool):
        raise SystemExit(f"Story 31 promotion gate payload missing boolean `{key}`.")
    return value


def _mask_policy_matches(report: Story31PromotionGateReport) -> bool:
