"""Detached worker entrypoint for the Qwen fallback proof lane fallback standalone eval.

Purpose:
    Execute the canonical Qwen fallback proof lane fallback standalone eval in a background
    host process and persist one worker-status artifact for detached
    inspection.

Relationships:
    - Invoked by `qwen_fallback_eval_detached.py`.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Run the detached Qwen fallback proof lane fallback eval worker."
    )
...
def main(argv: list[str] | None = None) -> int:
    """Run the Qwen fallback standalone eval and persist detached worker status."""
    ...
    try:
        ...
        failure_path(output_root).write_text(
            f"Detached Qwen fallback proof lane fallback eval worker failed: {exc}\n",
            encoding="utf-8",
        )
        exit_code = 1
    write_json(
