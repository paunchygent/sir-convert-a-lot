"""Contract-diff helpers for the T221 historical Task 101 control lane.

Purpose:
    Keep the written evidence comparing the documented historical Task 101
    contract, the invalid T220 approximation, and the recreated T221 launch in
    one focused module so the public T221 surface stays small and readable.

Relationships:
    - Used by `t221_historical_control.py`.
    - Reused by tests that verify T221 writes the expected historical-control
...
INVALID_T220_APPROXIMATION: dict[str, object] = {
    "launch_id": "task220-20260317t183856z-control-a1",
    "pilot_bundle_root": (
        "/srv/scratch/sir-convert-a-lot/build/verification/"
        "task-152-task101-finalization-benchmark-20260312j/"
...
def build_contract_diff_payload(
    *,
    ...
    """Return the written contract diff between historical, T220, and T221."""
    return {
        "task": "T221",
        "launch_id": launch_id,
        "historical_contract": {
        ...
        "invalid_t220": INVALID_T220_APPROXIMATION,
        "t221_recreation": {
        ...
        "remaining_known_diffs": [
            ...
            (
                "T221 uses the current trainer module with full_channel_masked + text_span_only, "
                "not the older Task 101 probe implementation."
            ),
            (
                "T221 bounds max_steps to the requested probe window and sets eval_interval_steps "
                "to suppress in-training eval, which is the closest current replacement for the "
                "historical no-in-training-eval lane."
            ),
        ],
    }
...
def render_contract_diff_markdown(payload: dict[str, object]) -> str:
    """Render the written T221 contract diff as concise markdown evidence."""
    historical = _required_object(payload, "historical_contract")
    invalid_t220 = _required_object(payload, "invalid_t220")
    t221_recreation = _required_object(payload, "t221_recreation")
    ...
    return "\n".join(
        [
            ...
            "",
            "## Invalid T220 Approximation",
            f"- launch_id: `{invalid_t220['launch_id']}`",
            f"- pilot_bundle_root: `{invalid_t220['pilot_bundle_root']}`",
            f"- batch_size: `{invalid_t220['batch_size']}`",
            f"- max_steps: `{invalid_t220['max_steps']}`",
            f"- launch_surface: `{invalid_t220['launch_surface']}`",
            "",
            "## T221 Recreation",
            f"- pilot_bundle_root: `{t221_recreation['pilot_bundle_root']}`",
            f"- effective_bundle_mount_root: `{t221_recreation['effective_bundle_mount_root']}`",
            f"- batch_size: `{t221_recreation['batch_size']}`",
            f"- max_steps: `{t221_recreation['max_steps']}`",
            f"- checkpoint_interval_steps: `{t221_recreation['checkpoint_interval_steps']}`",
            f"- eval_interval_steps: `{t221_recreation['eval_interval_steps']}`",
            f"- gradient_accumulation_steps: `{t221_recreation['gradient_accumulation_steps']}`",
            f"- text_embedding_assembly_mode: `{t221_recreation['text_embedding_assembly_mode']}`",
            f"- text_embedding_mask_policy: `{t221_recreation['text_embedding_mask_policy']}`",
            f"- train_rows: `{t221_recreation['train_rows']}`",
            f"- eval_rows: `{t221_recreation['eval_rows']}`",
            f"- image: `{t221_recreation['image']}`",
            f"- image_id: `{t221_recreation['image_id']}`",
            "",
...
def _required_object(payload: dict[str, object], key: str) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise SystemExit(f"T221 contract payload lacked a valid `{key}` object.")
    return value
...
def _required_str_list(payload: dict[str, object], key: str) -> list[str]:
    ...
    if not isinstance(value, list) or any(not isinstance(item, str) for item in value):
        raise SystemExit(f"T221 contract payload lacked a valid `{key}` string list.")
    return list(value)
