"""Markdown rendering helpers for the Qwen fallback proof surfaces.

Purpose:
    Render deterministic operator-facing plan and checklist markdown for the
    bounded Qwen fallback proof packages without mixing those concerns into
    artifact path/config loading.

Relationships:
    - Used by `qwen_fallback_proof.py` during proof-package preparation.
    - Consumes `QwenFallbackProofConfig` plus launch-root helpers from
      `qwen_fallback_proof_artifacts.py`.
"""
...
def render_plan_markdown(
    ...
    *,
    ...
    """Render one concise markdown plan for the prepared Qwen fallback proof."""
    return "\n".join(
        [
            f"# {config.lane_label} Proof Plan",
            "",
...
def render_checklist_markdown(config: QwenFallbackProofConfig) -> str:
    """Render the operator checklist for one prepared Qwen fallback proof."""
    ...
    return "\n".join(
        [
            f"# {config.lane_label} Proof Checklist",
            "",
            "## Preflight",
            "",
            ...
            "## Close-Out",
            "",
            "- [ ] Record the outcome in the training reference ledger and proof docs.",
            (
            ...
            (
                ...
                "eval result before changing the next restart decision."
            ),
        ]
    )
