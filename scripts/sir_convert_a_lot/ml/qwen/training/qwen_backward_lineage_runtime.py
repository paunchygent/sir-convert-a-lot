"""Runtime helpers for the Qwen backward-lineage and fresh-start proof lane backward-lineage proof
surface.

Purpose:
    Build and execute the canonical local and remote commands for the shared
    Qwen backward-lineage and fresh-start proof lane backward-lineage Hemma proof lane across
    `backward-lineage`, `per-layer talker-core trace`, and
    `layer-16/layer-15 boundary trace`.

Relationships:
    - Used by `qwen_backward_lineage_proof.py`.
...
def ensure_remote_scratch_headroom(config: QwenBackwardLineageProofConfig) -> dict[str, object]:
...
def remote_launch_proof_args(config: QwenBackwardLineageProofConfig) -> list[str]:
...
def remote_status_proof_args(config: QwenBackwardLineageProofConfig) -> list[str]:
