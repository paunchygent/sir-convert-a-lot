"""
CLI implementation for the Qwen backward-lineage and fresh-start proof lane backward-lineage proof.

Purpose:
    Expose one deterministic `prepare`, `launch`, and `status` surface for the
    backward-lineage backward-lineage Hemma proof lane while delegating the actual detached
    proof execution to bounded helpers.

Relationships:
    - Uses `qwen_backward_lineage_artifacts.py` for proof-package state.
    - Uses `qwen_backward_lineage_detached.py` for the remote background worker.
    - Uses `qwen_backward_lineage_runner.py` for the host-side proof logic.
"""
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_backward_lineage_artifacts import (
    ...
    QwenBackwardLineageProofConfig,
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_backward_lineage_runtime import (
...
def build_parser() -> argparse.ArgumentParser:
    """Build the committed CLI parser for the backward-lineage proof surface."""
    parser = argparse.ArgumentParser(description="Operate the Qwen backward-lineage proof.")
...
def main(argv: list[str] | None = None) -> int:
    """
    Prepare or operate the Qwen backward-lineage and fresh-start proof lane backward-lineage proof
    surface.
    """
    parser = build_parser()
...
def _config_from_remote_launch_args(args: argparse.Namespace) -> QwenBackwardLineageProofConfig:
    hook_profile = str(args.hook_profile)
    return QwenBackwardLineageProofConfig(
        task_label=run_label_for_hook_profile(hook_profile),
...
def _config_for_remote_status(args: argparse.Namespace) -> QwenBackwardLineageProofConfig:
    return QwenBackwardLineageProofConfig(
        task_label=run_label_for_hook_profile(DEFAULT_HOOK_PROFILE),
        command_name=DEFAULT_COMMAND_NAME,
...
def _runner_args(config: QwenBackwardLineageProofConfig) -> list[str]:
    command = [
        "--source-bundle-root",
...
def _render_plan_markdown(config: QwenBackwardLineageProofConfig) -> str:
    source_lines = ", ".join(str(value) for value in config.source_lines)
    return "\n".join(
        [
            "# Qwen backward-lineage and fresh-start proof lane Backward-Lineage Proof Plan",
            "",
...
def _render_checklist_markdown(config: QwenBackwardLineageProofConfig) -> str:
    source_lines = ", ".join(str(value) for value in config.source_lines)
    return "\n".join(
        [
            "# Qwen backward-lineage and fresh-start proof lane Backward-Lineage Checklist",
            "",
...
def _render_status_markdown(payload: dict[str, object]) -> str:
    return "\n".join(
        [
            "# Qwen backward-lineage and fresh-start proof lane Backward-Lineage Status",
            "",
