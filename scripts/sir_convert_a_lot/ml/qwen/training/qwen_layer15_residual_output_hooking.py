"""
Hook helpers for the residual/output/output-return/fp32-scaled output layer-15 output-path splits.

Purpose:
    Keep the residual/output residual-path capture, the output-return pre-output-scale return
    capture, and the fp32-scaled output fp32-scaled-output capture out of the central
    hook-session module so the Qwen stability lab backward-lineage plumbing stays under
...
def build_post_layer15_output_split_layer15_forward(
    *,
...
def install_layer15_residual_output_trace(
    *,
    model: object,
    attach_tensor: Callable[[str, torch.Tensor], None],
    ...
    """Install the residual/output residual-path hooks and raw residual-sum wrapper."""
    _residual_input_name, residual_sum_name, _output_name, _ = (
        talker_core_layer15_residual_output_trace_names()
    )
    for target in iter_talker_core_layer15_residual_output_trace_targets(model):
        handle = (
    ...
    patch_module_forward(
        layer_15,
        build_post_layer15_output_split_layer15_forward(
...
def install_layer15_output_return_trace(
    *,
    model: object,
    attach_tensor: Callable[[str, torch.Tensor], None],
    build_forward_hook: Callable[
    ...
    """Install the output-return return-path hooks and raw pre-scale output capture."""
    pre_output_scale_return_name, _output_name, _ = talker_core_layer15_output_return_trace_names()
    for target in iter_talker_core_layer15_output_return_trace_targets(model):
        handle = (
            target.module.register_forward_hook(
                build_forward_hook(target.name, target.tensor_selector)
            )
...
def install_post_layer15_output_multiply_fp32_scaled_output_trace(
    *,
    model: object,
    attach_tensor: Callable[[str, torch.Tensor], None],
    build_forward_hook: Callable[
    ...
    """Install the fp32-scaled output fp32-scaled-output hooks beneath the fixed multiply-site
    confirmation seam.
    """
    fp32_scaled_output_name, _output_name, _ = (
        talker_core_post_layer15_output_multiply_fp32_scaled_output_trace_names()
    )
    for target in iter_talker_core_post_layer15_output_multiply_fp32_scaled_output_trace_targets(
        model
    ):
        handle = (
            target.module.register_forward_hook(
