"""Remote-command helpers for the Qwen fallback replay proof surfaces.

Purpose:
    Build and execute the canonical remote `qwen-train` commands for Qwen fallback proof
    while keeping remote execution logic separate from local proof-artifact
    management.

Relationships:
    - Used by the fallback proof entrypoints.
    - Consumes proof configuration and path helpers from `qwen_fallback_proof_artifacts.py`.
"""
...
RUN_HEMMA_QWEN_FALLBACK_EVAL_DETACHED_PREFIX = [
    "pdm",
    "run",
    "run-hemma",
    "--",
...
def full_remote_eval_detached_command(eval_args: list[str]) -> list[str]:
    ...
    return [*RUN_HEMMA_QWEN_FALLBACK_EVAL_DETACHED_PREFIX, *eval_args]


def run_remote_eval_detached_json(eval_args: list[str], *, label: str) -> dict[str, object]:
    """Run one remote detached Qwen fallback proof eval command and parse its JSON payload."""
...
def ensure_remote_scratch_headroom(config: QwenFallbackProofConfig) -> dict[str, object]:
    ...
    payload = _run_remote_scratch_policy_json(
        ...
        label=f"{config.lane_label.lower()} scratch audit",
    )
    ...
    if meets_headroom is True:
        return payload
    raise SystemExit(
        f"{config.lane_label} launch blocked because Hemma scratch headroom is below the "
        "required threshold. "
...
def window_qwen_train_args(config: QwenFallbackProofConfig) -> list[str]:
...
def window_remote_command(config: QwenFallbackProofConfig) -> list[str]:
    """Return the full remote bounded replay command."""
    return full_remote_command(window_qwen_train_args(config))


def status_window_qwen_train_args(config: QwenFallbackProofConfig) -> list[str]:
...
def status_window_remote_command(config: QwenFallbackProofConfig) -> list[str]:
...
def fallback1470_qwen_train_args(config: QwenFallbackProofConfig) -> list[str]:
...
def fallback1470_remote_command(config: QwenFallbackProofConfig) -> list[str]:
...
def status_fallback1470_qwen_train_args(config: QwenFallbackProofConfig) -> list[str]:
...
def status_fallback1470_remote_command(config: QwenFallbackProofConfig) -> list[str]:
...
def gate_qwen_train_args(config: QwenFallbackProofConfig) -> list[str]:
...
def gate_remote_command(config: QwenFallbackProofConfig) -> list[str]:
    """Return the full remote continuation command for the `1500` gate."""
    return full_remote_command(gate_qwen_train_args(config))


def status_gate_qwen_train_args(config: QwenFallbackProofConfig) -> list[str]:
...
def status_gate_remote_command(config: QwenFallbackProofConfig) -> list[str]:
...
def fallback_eval_detached_args(
    config: QwenFallbackProofConfig,
    *,
...
def fallback_eval_remote_command(config: QwenFallbackProofConfig) -> list[str]:
...
def status_fallback_eval_detached_args(config: QwenFallbackProofConfig) -> list[str]:
...
def status_fallback_eval_remote_command(config: QwenFallbackProofConfig) -> list[str]:
...
def status_summary_markdown(phase_name: str, status_payload: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            f"# Qwen fallback proof {phase_name} Status",
            "",
...
def ensure_window_passed(config: QwenFallbackProofConfig) -> dict[str, object]:
    ...
    status_payload = run_remote_training_json(
        ...
        label=f"{config.lane_label.lower()} bounded replay status",
    )
    pilot_status = status_payload.get("pilot_status")
    current_optimizer_step = (
        None if not isinstance(pilot_status, dict) else pilot_status.get("current_optimizer_step")
    ...
    if status_payload.get("running") is True:
        raise SystemExit(
            f"{config.lane_label} bounded replay is still running; "
            "do not launch the `1500` gate yet."
        )
    if status_payload.get("exit_code") != 0:
        raise SystemExit(
            f"{config.lane_label} bounded replay did not exit cleanly; "
    ...
    if (
        ...
        raise SystemExit(
            f"{config.lane_label} bounded replay did not reach the required "
            "optimizer-step gate before `1500` continuation."
        )
    return status_payload


def ensure_fallback_passed(config: QwenFallbackProofConfig) -> dict[str, object]:
    """Require a successful fallback replay before launching standalone eval."""
    status_payload = run_remote_training_json(
        status_fallback1470_qwen_train_args(config),
        label=f"{config.lane_label.lower()} fallback `1470` status",
    )
    pilot_status = status_payload.get("pilot_status")
    current_optimizer_step = (
        None if not isinstance(pilot_status, dict) else pilot_status.get("current_optimizer_step")
    ...
    if status_payload.get("running") is True:
        raise SystemExit(
            f"{config.lane_label} fallback `1470` replay is still running; "
            "do not launch standalone eval yet."
        )
    if status_payload.get("exit_code") != 0:
        raise SystemExit(
            f"{config.lane_label} fallback `1470` replay did not exit cleanly; "
    ...
    if (
        ...
        raise SystemExit(
            f"{config.lane_label} fallback replay did not reach optimizer step "
            f"`{config.fallback_max_steps}` before standalone eval."
        )
    checkpoint_path = latest_checkpoint_path(status_payload)
    if checkpoint_path is None:
        raise SystemExit(
            f"{config.lane_label} fallback replay did not expose a durable checkpoint path for "
            "standalone eval."
        )
    return status_payload
