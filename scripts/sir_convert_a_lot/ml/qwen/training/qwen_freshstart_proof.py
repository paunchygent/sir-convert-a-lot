"""
CLI implementation for the Qwen backward-lineage and fresh-start proof lane fresh-start discriminant
proof.
...
Relationships:
    - Uses `qwen_freshstart_artifacts.py` for proof-package state.
    - Uses `qwen_freshstart_bundle.py` for remote mini-bundle creation.
    - Uses `qwen_freshstart_runtime.py` for local/remote command execution.
"""
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_freshstart_artifacts import (
    DEFAULT_BATCH_SIZE,
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_freshstart_bundle import (
    materialize_mini_bundle,
)
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_freshstart_runtime import (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Prepare and operate the Qwen fresh-start proof.")
...
def main(argv: list[str] | None = None) -> int:
    """Prepare or operate the Qwen backward-lineage and fresh-start proof lane fresh-start proof
    surface.
    """
    parser = build_parser()
...
def _config_from_remote_launch_args(args: argparse.Namespace) -> QwenFreshstartProofConfig:
    return QwenFreshstartProofConfig(
        task_label="Qwen fresh-start proof",
...
def _config_for_remote_status(args: argparse.Namespace) -> QwenFreshstartProofConfig:
    return QwenFreshstartProofConfig(
        task_label="Qwen fresh-start proof",
        command_name=DEFAULT_COMMAND_NAME,
...
def _render_plan_markdown(config: QwenFreshstartProofConfig) -> str:
    train_slice = (
...
def _render_checklist_markdown(config: QwenFreshstartProofConfig) -> str:
    train_slice = (
        f"{config.train_manifest_family} lines {config.train_line_start}..{config.train_line_end}"
    )
    eval_slice = (
...
def _render_status_markdown(payload: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            "# Qwen fresh-start proof Fresh-Start Status",
            "",
