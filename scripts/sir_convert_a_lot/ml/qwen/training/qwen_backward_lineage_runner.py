"""Host-side Hemma proof runner for the Qwen backward-lineage and fresh-start proof lane
...
Purpose:
    ...
    persist deterministic report artifacts for backward-lineage.
...
Relationships:
    ...
    - Used by the detached backward-lineage worker and the public proof surface.
"""
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/qwen-backward-lineage-proof")
DEFAULT_MANIFEST_FAMILY = "swedish_pilot_train"
DEFAULT_SOURCE_LINES = (13, 4)
DEFAULT_OUTPUT_ROOT_HOME_MOUNT_BASE = Path(
    "/home/paunchygent/.data/sir-convert-a-lot/qwen-backward-lineage-output-roots"
)
...
class BackwardLineageProofReport:
    """Deterministic report contract for one backward-lineage backward-lineage proof."""
...
def parse_args(argv: list[str] | None) -> BackwardLineageProofSettings:
    ...
    parser = argparse.ArgumentParser(description="Run the Qwen backward-lineage proof.")
...
def build_report_markdown(report: BackwardLineageProofReport) -> str:
    """Render one concise markdown summary for the backward-lineage backward-lineage proof."""
    lines = [
        "# Qwen backward-lineage and fresh-start proof lane Backward-Lineage Proof",
        "",
...
def main(argv: list[str] | None = None) -> int:
    """Run the host-side backward-lineage proof and persist deterministic report artifacts."""
    ...
    try:
        ...
        failure_path.write_text(
            f"Qwen backward-lineage proof failed: {exc}\n",
            encoding="utf-8",
        )
        raise
