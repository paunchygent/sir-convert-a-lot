"""Mini-bundle materialization for the Qwen backward-lineage and fresh-start proof lane fresh-start
proof.

Purpose:
    Build one tiny truthful training bundle from the canonical Qwen pilot training pilot
    bundle so fresh-start Candidate 1 probes can run without touching the full
    bundle contract.

Relationships:
    - Used by `qwen_freshstart_proof.py` during remote launch handling.
...
def _resolve_source_bundle_root(
    *,
    ...
    """Resolve one manifest source root, including dated Qwen pilot training bundle roots."""
