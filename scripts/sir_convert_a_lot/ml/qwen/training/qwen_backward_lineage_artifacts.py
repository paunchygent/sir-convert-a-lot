"""Artifact helpers for the Qwen backward-lineage and fresh-start proof lane backward-lineage proof
surface.

Purpose:
    Own deterministic proof configuration, artifact paths, and config loading
    for the shared Qwen backward-lineage and fresh-start proof lane backward-lineage Hemma proof
    lane across `backward-lineage`,
    `per-layer talker-core trace`, and `layer-16/layer-15 boundary trace`.

Relationships:
    - Used by `qwen_backward_lineage_proof.py` for local proof packages.
    - Used by `qwen_backward_lineage_runtime.py` for remote command building.
"""
...
DEFAULT_LOCAL_PROOF_ROOT = Path("build/verification/qwen-backward-lineage")
DEFAULT_REMOTE_PROOF_OUTPUT_ROOT = Path(
    "/srv/scratch/sir-convert-a-lot/build/verification/qwen-backward-lineage-proof"
)
...
DEFAULT_COMMAND_NAME = "qwen-backward-lineage"
DEFAULT_SOURCE_LINES = (13, 4)
_HOOK_PROFILE_TASK_METADATA = {
    "baseline": ("Qwen backward-lineage baseline", "qwen-lineage-baseline"),
    "talker_core": ("Qwen talker-core lineage", "qwen-talker-core-lineage"),
    "talker_core_boundary": (
        "Qwen talker-core boundary lineage",
        "qwen-talker-core-boundary-lineage",
    ),
}
...
class QwenBackwardLineageProofConfig:
...
def run_label_for_hook_profile(hook_profile: str) -> str:
...
def remote_proof_root(config: QwenBackwardLineageProofConfig) -> Path:
...
def build_prepare_config(args: argparse.Namespace) -> QwenBackwardLineageProofConfig:
...
def load_config(local_proof_root: Path) -> QwenBackwardLineageProofConfig:
