"""Host-side runner for the Story 31 deterministic parity probe.

Purpose:
    Execute the exact `T225` failure-family comparison under one local output
...
Relationships:
    - Used by `story31_parity_probe.py` for the public CLI surface.
    - Delegates per-path execution to `story31_parity_probe_runtime.py`.
"""
...
from scripts.sir_convert_a_lot.ml.qwen.training.story31_parity_probe_contracts import (
    ...
    Story31ParityCheckpointComparison,
    Story31ParityPathReport,
    Story31ParityProbeReport,
    Story31ParityProbeSettings,
)
from scripts.sir_convert_a_lot.ml.qwen.training.story31_parity_probe_runtime import (
    run_parity_path,
)
...
def run_story31_parity_probe(settings: Story31ParityProbeSettings) -> Story31ParityProbeReport:
    """Run the deterministic Story 31 parity probe and classify its result."""
    ...
    try:
        ...
        write_markdown(failure_path, f"# Story 31 Parity Probe Failure\n\n`{error!r}`")
        raise
...
def persist_report(
    output_root: Path,
    report: Story31ParityProbeReport,
...
def build_report_markdown(report: Story31ParityProbeReport) -> str:
    """Render one concise markdown summary for the Story 31 parity report."""
    lines = [
        "# Story 31 Parity Probe",
        "",
...
) -> list[Story31ParityCheckpointComparison]:
    comparisons: list[Story31ParityCheckpointComparison] = []
    ...
    *,
    ...
    if classification == "divergence_before_non_finite_boundary":
        return (
            ...
            "sufficient to escalate into T227 trainer/runtime remediation."
        )
    if classification == "divergence_at_non_finite_boundary":
        return (
