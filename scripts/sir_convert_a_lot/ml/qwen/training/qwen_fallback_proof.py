"""Shared command surface for the Qwen fallback proof lanes.

Purpose:
    Expose one reusable prepare/launch/status CLI for bounded Qwen fallback proof
    lanes while delegating proof configuration and remote execution to bounded
    helper modules.

Relationships:
    - Used by the fallback replay and accumulation proof entrypoints.
    - Uses `qwen_fallback_proof_artifacts.py` for deterministic local proof artifacts.
    - Uses `qwen_fallback_proof_runtime.py` for detached Hemma launch/status commands.
"""
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_fallback_proof_artifacts import (
    ...
    FALLBACK_REPLAY_PROOF_PROFILE,
    QwenFallbackProofProfile,
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_fallback_proof_runtime import (
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_proof_rendering import (
...
def build_parser(profile: QwenFallbackProofProfile) -> argparse.ArgumentParser:
    """Build the CLI parser for one Qwen fallback replay proof surface."""
    parser = argparse.ArgumentParser(
        description=(f"Prepare and operate the detached Hemma {profile.lane_label} proof surface.")
    )
...
def run_main(profile: QwenFallbackProofProfile, argv: list[str] | None = None) -> int:
    """Prepare or operate one fallback proof detached proof surface."""
    ...
    if args.command == "launch-window":
        ...
        launch_payload = run_remote_training_json(
            ...
            label=f"{config.lane_label.lower()} bounded replay launch",
        )
    ...
    if args.command == "launch-fallback1470":
        ...
        launch_payload = run_remote_training_json(
            ...
            label=f"{config.lane_label.lower()} fallback `1470` replay launch",
        )
    ...
    if args.command == "status-fallback1470":
        status_payload = run_remote_training_json(
            ...
            label=f"{config.lane_label.lower()} fallback `1470` replay status",
        )
    ...
    if args.command == "launch-fallback-eval":
        ...
        if checkpoint_path is None:
            raise SystemExit(
                f"{config.lane_label} fallback replay did not expose a durable checkpoint path."
            )
        launch_payload = run_remote_eval_detached_json(
            fallback_eval_detached_args(config, checkpoint_path=checkpoint_path),
            label=f"{config.lane_label.lower()} fallback standalone eval launch",
        )
    ...
    if args.command == "status-fallback-eval":
        status_payload = run_remote_eval_detached_json(
            ...
            label=f"{config.lane_label.lower()} fallback standalone eval status",
        )
    ...
    if args.command == "launch-gate1500":
        ...
        launch_payload = run_remote_training_json(
            ...
            label=f"{config.lane_label.lower()} `1500` gate launch",
        )
    ...
    if args.command == "status-gate1500":
        status_payload = run_remote_training_json(
            ...
            label=f"{config.lane_label.lower()} `1500` gate status",
        )
    ...
    raise SystemExit(f"Unsupported {profile.lane_label} proof command: {args.command}")


def main(argv: list[str] | None = None) -> int:
    """Prepare or operate the detached Hemma Qwen fallback replay proof surface."""
    return run_main(FALLBACK_REPLAY_PROOF_PROFILE, argv)
