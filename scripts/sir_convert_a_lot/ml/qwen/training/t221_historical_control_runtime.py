"""Runtime helpers for the T221 historical Task 101 control lane.

Purpose:
    Recreate the documented historical Task 101 launch contract as faithfully
...
Relationships:
    - Used by `t221_historical_control.py` for the committed T221 launch and
      status surface.
...
CONTAINER_HISTORICAL_BUNDLE_ROOT = Path("/app/historical-task101-pilot-bundle")
...
class HistoricalControlImageSettings:
    """Image-build settings for the dedicated T221 historical control lane."""
...
def prepare_runtime_dependencies(
    *,
    ...
    """Prepare image, cache, scratch, and historical-bundle mounts for T221."""
...
def build_detached_historical_control_command(
    ...
    *,
    ...
    """Build the detached Docker argv for one T221 historical control run."""
    ...
    command = [
        ...
        "--label",
        "sir_convert_a_lot.task=t221-historical-control",
        "--device",
        "/dev/kfd",
        "--device",
        "/dev/dri",
...
def launch_detached_historical_control(
    ...
    *,
    ...
    """Launch one detached T221 historical control run and return metadata."""
    ...
    container_id = docker_checked(
        command,
        label="docker run t221 detached historical control",
    ).strip()
    ...
    return DetachedLaunch(
        ...
        diagnostic={
            "kind": "t221_historical_task101_control",
