"""Contracts for the Qwen stability lab deterministic parity probe.
...
Relationships:
    - Imported by `qwen_parity_probe.py` for the public command surface.
    - Imported by `qwen_parity_probe_runner.py` and
      `qwen_parity_probe_runtime.py` for typed artifact exchange.
"""
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_historical_pilot_control import (
    DEFAULT_BATCH_SIZE,
...
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_historical_pilot_control import (
    DEFAULT_EVAL_MANIFEST_FAMILY as DEFAULT_QWEN_PILOT_EVAL_MANIFEST_FAMILY,
)
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/qwen-parity-probe")
...
class QwenParityProbeSettings:
    """Configuration for one deterministic Qwen stability lab parity-probe run."""
    ...
    eval_manifest_family: str = DEFAULT_QWEN_PILOT_EVAL_MANIFEST_FAMILY
...
@dataclass(frozen=True)
class QwenParityCheckpointComparison:
...
@dataclass(frozen=True)
class QwenParityProbeReport:
    """Machine-readable report for one deterministic Qwen stability lab parity run."""
    ...
    checkpoint_comparisons: tuple[QwenParityCheckpointComparison, ...]
...
DEFAULT_QWEN_PARITY_PROBE_SETTINGS = QwenParityProbeSettings(output_root=DEFAULT_OUTPUT_ROOT)
