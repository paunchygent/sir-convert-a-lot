"""Detached host-process helpers for the Qwen backward-lineage and fresh-start proof lane
...
Relationships:
    - Launches `qwen_backward_lineage_detached_worker.py`.
    - Used by `qwen_backward_lineage_proof.py` for remote launch/status.
"""
...
_WORKER_MODULE = "scripts.sir_convert_a_lot.ml.qwen.training.qwen_backward_lineage_detached_worker"
...
class DetachedBackwardLineageLaunch:
    """Deterministic launch metadata for one detached backward-lineage proof run."""
...
class DetachedBackwardLineageStatus:
    """Deterministic status view for one detached backward-lineage proof run."""
...
def build_detached_worker_command(output_root: Path, proof_args: Sequence[str]) -> list[str]:
    """Build the background worker command for one detached backward-lineage proof run."""
...
def launch_detached_backward_lineage_proof(
    *,
    ...
    """Launch one detached backward-lineage proof worker and return launch metadata."""
...
def inspect_detached_backward_lineage_proof(
    ...
    """Inspect one detached backward-lineage proof worker plus its canonical artifacts."""
