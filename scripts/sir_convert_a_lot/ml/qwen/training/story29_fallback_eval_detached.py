"""Detached standalone-eval helpers for the Story 29 fallback proof lane.

Purpose:
    Launch and inspect one detached host process that runs `qwen-train eval`
    for the Story 29 fallback checkpoint so Hemma eval evidence survives the
    local client session.

Relationships:
    - Launches `story29_fallback_eval_detached_worker.py` as the bounded
      background worker for the canonical `qwen-train eval` surface.
    - Used by `cli.ml.qwen_story29_eval_detached`.
"""
...
_WORKER_MODULE = "scripts.sir_convert_a_lot.ml.qwen.training.story29_fallback_eval_detached_worker"
...
class DetachedStory29EvalLaunch:
    """Deterministic launch metadata for one detached Story 29 eval run."""
...
@dataclass(frozen=True)
class DetachedStory29EvalStatus:
    """Deterministic status view for one detached Story 29 eval run."""
...
def default_launch_id() -> str:
    """Return one deterministic launch id for a new Story 29 eval run."""
...
def normalize_eval_args(output_root: Path, raw_eval_args: Sequence[str]) -> tuple[str, ...]:
    ...
    if not _has_flag(eval_args, "--output-root"):
        raise SystemExit("Detached Story 29 eval requires explicit `--output-root`.")
    if not _has_flag(eval_args, "--launch-root"):
        raise SystemExit("Detached Story 29 eval requires explicit `--launch-root`.")
...
def build_detached_worker_command(output_root: Path, eval_args: Sequence[str]) -> list[str]:
    """Build the background worker command for one detached Story 29 eval run."""
...
def launch_detached_story29_eval(
    *,
    output_root: Path,
    repo_root: Path,
    eval_args: Sequence[str],
    launch_id: str | None = None,
) -> DetachedStory29EvalLaunch:
    """Launch one detached Story 29 eval worker and return launch metadata."""
    ...
    return DetachedStory29EvalLaunch(
...
def inspect_detached_story29_eval(
    launch: DetachedStory29EvalLaunch,
) -> DetachedStory29EvalStatus:
    """Inspect one detached Story 29 eval worker plus its canonical artifacts."""
    ...
    return DetachedStory29EvalStatus(
...
def _load_optional_json(path: Path) -> dict[str, object] | None:
    ...
    if not isinstance(payload, dict):
        raise SystemExit(f"Detached Story 29 eval artifact was malformed at `{path}`.")
    return payload
...
def _optional_int(payload: dict[str, object], key: str) -> int | None:
    ...
    if not isinstance(value, int):
        raise SystemExit(f"Detached Story 29 eval worker status returned malformed `{key}`.")
    return value
