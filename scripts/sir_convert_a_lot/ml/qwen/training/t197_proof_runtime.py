"""Remote-command helpers for the Task 197 proof surface.

Purpose:
    Build and execute the canonical remote `qwen-train` commands for Task 197
    while keeping remote execution logic separate from local proof-artifact
    management.

Relationships:
    - Used by `t197_proof.py` to launch and inspect the proof phases.
...
def window_qwen_train_args(config: T197ProofConfig) -> list[str]:
...
def window_remote_command(config: T197ProofConfig) -> list[str]:
    """Return the full remote bounded replay command."""
    return full_remote_command(window_qwen_train_args(config))


def status_window_qwen_train_args(config: T197ProofConfig) -> list[str]:
...
def status_window_remote_command(config: T197ProofConfig) -> list[str]:
...
def gate_qwen_train_args(config: T197ProofConfig) -> list[str]:
...
def gate_remote_command(config: T197ProofConfig) -> list[str]:
    """Return the full remote continuation command for the `1500` gate."""
    return full_remote_command(gate_qwen_train_args(config))


def status_gate_qwen_train_args(config: T197ProofConfig) -> list[str]:
...
def status_gate_remote_command(config: T197ProofConfig) -> list[str]:
...
def ensure_window_passed(config: T197ProofConfig) -> dict[str, object]:
    ...
    if status_payload.get("running") is True:
        raise SystemExit(
            "Task 197 bounded replay is still running; do not launch the `1500` gate yet."
        )
    if status_payload.get("exit_code") != 0:
        raise SystemExit(
            "Task 197 bounded replay did not exit cleanly; do not launch the `1500` gate."
        )
    if (
        ...
        raise SystemExit(
            "Task 197 bounded replay did not reach the required optimizer-step "
            "gate before `1500` continuation."
        )
