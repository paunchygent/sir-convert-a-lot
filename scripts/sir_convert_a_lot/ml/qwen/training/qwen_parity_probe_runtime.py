"""Runtime orchestration for the Qwen stability lab deterministic parity probe.

Purpose:
    Prepare one bounded Qwen runtime per comparison path, resolve the exact
    `the diagnostic lane` microbatch family, and project the resulting execution artifacts
    into the stable `QwenParityPathReport` contract used by the runner.

Relationships:
    - Imported by `qwen_parity_probe_runner.py`.
    - Delegates execution to `qwen_parity_probe_execution.py`.
    - Delegates artifact shaping to `qwen_parity_probe_artifacts.py`.
"""

from __future__ import annotations

from scripts.devops.qwen_finetuning_patches.sft_12hz_setup import prepare_training_run
from scripts.sir_convert_a_lot.ml.qwen.training.qwen_parity_probe_artifacts import (
