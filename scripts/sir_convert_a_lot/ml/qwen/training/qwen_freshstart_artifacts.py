"""
Artifact helpers for the Qwen backward-lineage and fresh-start proof lane fresh-start discriminant
proof.
...
Relationships:
    - Used by `qwen_freshstart_proof.py` for local proof-package handling.
    - Used by `qwen_freshstart_runtime.py` for remote command construction.
"""
...
DEFAULT_LOCAL_PROOF_ROOT = Path("build/verification/qwen-freshstart-proof")
DEFAULT_REMOTE_PROOF_OUTPUT_ROOT = Path(
    "/srv/scratch/sir-convert-a-lot/build/verification/qwen-freshstart-proof"
)
...
DEFAULT_PROOF_LABEL = "Qwen fresh-start proof"
DEFAULT_COMMAND_NAME = "qwen-freshstart-proof"
DEFAULT_PROOF_ID_PREFIX = "qwen-freshstart"


@dataclass(frozen=True)
class QwenFreshstartProofConfig:
...
def build_prepare_config(args: argparse.Namespace) -> QwenFreshstartProofConfig:
    ...
    return QwenFreshstartProofConfig(
        task_label=DEFAULT_PROOF_LABEL,
...
def load_config(local_proof_root: Path) -> QwenFreshstartProofConfig:
