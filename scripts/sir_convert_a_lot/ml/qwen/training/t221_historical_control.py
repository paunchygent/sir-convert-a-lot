"""Dedicated historical-control surface for the T221 Task 101 rerun.

Purpose:
    Recreate the documented historical Task 101 launch contract closely enough
    to answer the original-recipe plus `T206` token-fix question without
    silently drifting back to the later RCA or benchmark launch posture.

Relationships:
    - Uses `t221_historical_control_runtime.py` for the dedicated detached
...
from scripts.sir_convert_a_lot.ml.qwen.training.t221_historical_control_contract_diff import (
...
DEFAULT_COMMAND_NAME = "qwen-t221-historical-control"
...
def build_parser() -> argparse.ArgumentParser:
    """Build the committed CLI parser for the T221 historical control lane."""
    parser = argparse.ArgumentParser(
        description="Launch or inspect the T221 historical Task 101 control lane."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    launch = subparsers.add_parser("launch", help="Launch the detached T221 historical control.")
    ...
    for command_name, help_text in (
        ("status", "Inspect one detached T221 launch."),
        ("stop", "Stop one detached T221 launch intentionally."),
    ):
...
def main(argv: list[str] | None = None) -> int:
    """Launch, inspect, or stop the committed T221 historical control lane."""
    parser = build_parser()
    ...
    if args.command == "launch":
        ...
        run_checked(
            ...
            label="rocm-smi t221 historical control preflight",
        )
...
def _build_settings_from_args(args: argparse.Namespace) -> TrainingSettings:
    """Build the bounded T221 training settings from parsed CLI arguments."""
...
def _validate_launch_environment(
    ...
    """Fail fast when T221 paths drift off the intended Hemma storage tiers."""
...
def _validate_historical_bundle(
    ...
    *,
    ...
    if not train_manifest.is_file() or not eval_manifest.is_file():
        raise SystemExit(
            "T221 historical control could not find the required historical manifests under "
    ...
    if (
        ...
        raise SystemExit(
            "T221 historical bundle row counts drifted from the documented Task 101 contract: "
...
def _resolve_launch_root(*, output_root: Path, requested_launch_root: Path | None) -> Path:
    ...
    if not pointer_path.is_file():
        raise SystemExit("T221 status/stop requires --launch-root or a latest-launch pointer.")
    payload = json.loads(pointer_path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise SystemExit("T221 latest-launch metadata was malformed.")
    launch_root_value = payload.get("launch_root")
    if not isinstance(launch_root_value, str):
        raise SystemExit("T221 latest-launch metadata lacked `launch_root`.")
    return Path(launch_root_value)


def _load_launch(current_launch_root: Path) -> DetachedLaunch:
    """Load one previously recorded detached launch payload for T221."""
    return load_launch(
        current_launch_root,
...
def _default_launch_id() -> str:
    """Return the canonical launch id for one T221 historical-control run."""
    from datetime import UTC, datetime

    return f"task221-{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ').lower()}-a1"
