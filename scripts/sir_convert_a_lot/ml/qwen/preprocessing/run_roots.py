...
Purpose:
    ...
    and promotion helpers so staged preprocessing no longer executes directly
    inside one shared mutable corpus directory.

Relationships:
    - Used by `cli/ml/qwen_preprocess.py` to allocate live run roots and
      record run-scoped metadata.
...
class RunContext:
    """Resolved run-root context for one preprocessing execution."""
...
def prepare_run_root(context: RunContext) -> None:
...
def promote_run_root(context: RunContext) -> Path:
