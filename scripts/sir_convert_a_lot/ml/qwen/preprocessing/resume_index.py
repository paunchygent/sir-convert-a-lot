"""Helper surface for Qwen preprocessing completed-row resume indexes.

Purpose:
    Provide one committed CLI for rebuilding or validating the
    completed-row resume index so older run roots can gain the faster resume
    path without notebook-only or ad hoc shell logic.

Relationships:
    - Wraps `ml.qwen.preprocessing.storage` resume-index helpers.
    - Operates against existing preprocessing run roots on Hemma or Drive-backed
      Colab storage.
"""
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Rebuild or validate the Qwen preprocessing completed-row resume index."
    )
...
def main(argv: list[str] | None = None) -> int:
    """Run the Qwen preprocessing completed-row resume-index helper surface."""
