...
Relationships:
    - Used by `cli/ml/qwen_preprocess.py` to materialize or reload the bounded
      source-selection result before row-processing starts.
    - Consumed by stage-isolated preprocessing orchestration to reason about
      whether source selection has already completed for a run root.
    - Serializes `SourceRecord` contracts from `ml.qwen.common.models`.
"""
