"""Staged public-corpus source loading for the Qwen preprocessing pipeline.

Purpose:
    Resolve the real staged Swedish corpus assets on Hemma into adapter-shaped
    `SourceRecord` rows so the pipeline can run against public corpora without
    relying on repo-fixture smoke inputs.

Relationships:
    - Consumes the staged raw asset layout produced by corpus acquisition.
    - Delegates dataset parsing to the committed `fleurs`, `waxholm`, and
      `rixvox` source adapters.
    - Reuses heartbeats and models from `ml.qwen.preprocessing.sharding`.
"""
...
from scripts.sir_convert_a_lot.ml.qwen.preprocessing.sharding import (
    ...
    SourceSelectionHeartbeatCallback,
    attach_audio_locators_to_source_records,
    build_rixvox_audio_locator_index,
)
from scripts.sir_convert_a_lot.ml.qwen.preprocessing.sources.fleurs import (
    FLEURS_ALLOWED_SPLITS,
    fleurs_sv_source_records,
)
from scripts.sir_convert_a_lot.ml.qwen.preprocessing.sources.rixvox import (
    RIXVOX_ALLOWED_SPLITS,
    rixvox_source_records_from_parquet_with_audio_locators,
)
from scripts.sir_convert_a_lot.ml.qwen.preprocessing.sources.waxholm import (
...
def _source_selection_batch_callback(
    *,
    split: str,
    target_row_cap: int,
    heartbeat_callback: SourceSelectionHeartbeatCallback,
...
def staged_public_corpus_source_records(
    ...
    *,
    ...
    source_selection_heartbeat_callback: SourceSelectionHeartbeatCallback | None = None,
) -> list[SourceRecord]:
