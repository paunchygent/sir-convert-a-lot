"""Run the Chatterbox segmented experiment Chatterbox segmented-text experiment on Hemma.

Purpose:
    Compare the current single-pass Chatterbox path against the new segmented
    normal-text path on Hemma and write deterministic evidence for both lanes.

Relationships:
    - Reuses the committed Chatterbox benchmark runner for both synthesis lanes.
    - Proves the Chatterbox segmented experiment segmentation layer through the same ADR-0007
    sidecar
      contract used by the existing Chatterbox benchmark.
"""
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/chatterbox-segmented-hemma")
...
class ExperimentSettings:
    """Normalized CLI settings for the Chatterbox segmented experiment Hemma experiment."""
...
class LaneSummary:
    """
    One summarized Chatterbox benchmark lane outcome for the Chatterbox segmented experiment report.
    """
...
def _parse_args(argv: list[str]) -> ExperimentSettings:
    """Parse CLI arguments into normalized Chatterbox segmented experiment settings."""
    parser = argparse.ArgumentParser(
        description="Run the Chatterbox segmented experiment on Hemma."
    )
...
def _prepare_output_root(output_root: Path) -> dict[str, Path]:
    """Create a clean deterministic output tree for Chatterbox segmented experiment."""
    output_root.mkdir(parents=True, exist_ok=True)
    inputs_dir = output_root / "inputs"
    single_pass_dir = output_root / "single_pass"
    segmented_dir = output_root / "segmented"
    for directory in (inputs_dir, single_pass_dir, segmented_dir):
        directory.mkdir(parents=True, exist_ok=True)
    paths = {
        "inputs_dir": inputs_dir,
        "single_pass_dir": single_pass_dir,
...
def _load_lane_summary(lane_id: str, output_root: Path) -> LaneSummary:
    """Load one Chatterbox benchmark report into the segmented experiment summary."""
...
def _write_summary(
    *,
    ...
    single_pass_lane: LaneSummary,
    segmented_lane: LaneSummary,
) -> None:
    """Write one deterministic Chatterbox segmented experiment summary bundle."""
    payload = {
        "benchmark_id": "chatterbox-segmented-hemma",
        "generated_at": _utc_now_iso(),
        "reference_audio_path": settings.reference_audio_path.as_posix(),
        "probe_text": settings.probe_text,
        "single_pass_lane": asdict(single_pass_lane),
        "segmented_lane": asdict(segmented_lane),
    }
    ...
    markdown = "\n".join(
        [
            "# Chatterbox segmented experiment Chatterbox Segmented Experiment",
            "",
            ...
            f"- segment_stitch_mode: `{settings.segment_stitch_mode}`",
            "",
            "## Single Pass Lane",
            f"- synthesized_ok: `{single_pass_lane.synthesized_ok}`",
            f"- output_path: `{single_pass_lane.output_path}`",
            f"- duration_seconds: `{single_pass_lane.duration_seconds}`",
            f"- peak_vram_used_bytes: `{single_pass_lane.peak_vram_used_bytes}`",
            "",
            "## Segmented Lane",
            f"- synthesized_ok: `{segmented_lane.synthesized_ok}`",
            f"- output_path: `{segmented_lane.output_path}`",
            f"- duration_seconds: `{segmented_lane.duration_seconds}`",
            f"- peak_vram_used_bytes: `{segmented_lane.peak_vram_used_bytes}`",
            f"- segment_debug_dir: `{segmented_lane.segment_debug_dir}`",
        ]
    )
...
def main(argv: list[str] | None = None) -> int:
    """Run the Chatterbox segmented experiment Hemma experiment and write deterministic evidence."""
    ...
    LOGGER.info("Running Chatterbox segmented experiment single-pass baseline lane")
    baseline_returncode = _run_chatterbox_benchmark_lane(
        lane_output_root=paths["single_pass_dir"],
    ...
    LOGGER.info("Running Chatterbox segmented experiment segmented lane")
    segmented_returncode = _run_chatterbox_benchmark_lane(
        lane_output_root=paths["segmented_dir"],
        reference_audio_path=settings.reference_audio_path,
        probe_text_file=paths["probe_text"],
        exaggeration=settings.exaggeration,
        cfg_weight=settings.cfg_weight,
        ...
        segment_max_chars=settings.segment_max_chars,
        segment_cross_fade_ms=settings.segment_cross_fade_ms,
        segment_stitch_mode=settings.segment_stitch_mode,
        build_image=False,
    )
    if segmented_returncode != 0:
        return segmented_returncode
    _write_summary(
        output_root=settings.output_root,
        settings=settings,
        single_pass_lane=_load_lane_summary("single_pass", paths["single_pass_dir"]),
        segmented_lane=_load_lane_summary("segmented", paths["segmented_dir"]),
    )
    return 0
