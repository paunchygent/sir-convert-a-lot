"""Runtime helpers for Task 112 Hemma storage remediation.
...
Relationships:
    - Used by `run_task112_hemma_storage_remediation.py`.
    - Aligns live Hemma storage with Task 112, Task 113, and the Qwen
...
class Task112StorageSettings:
    """Normalized settings for the Task 112 storage remediation runner."""
...
@dataclass(frozen=True)
class Task112StorageReport:
    """Deterministic report for one Task 112 remediation pass."""
...
def migrate_repo_build_to_scratch(settings: Task112StorageSettings) -> None:
...
def migrate_qwen_data_to_storage(settings: Task112StorageSettings) -> None:
...
def docker_system_df() -> str:
    ...
    return run_checked(["sudo", "-n", "docker", "system", "df"], label="docker system df task112")


def filesystem_df() -> str:
    """Return the key Hemma filesystem usage summary."""
    return run_checked(["df", "-h", "/", "/srv/scratch", "/srv/storage"], label="df task112")
...
def cleanup_non_active_docker_state() -> None:
    ...
    run_checked(
        ...
        label="docker container prune task112",
    )
    run_checked(
        ["sudo", "-n", "docker", "image", "prune", "-af"],
        label="docker image prune task112",
    )
    run_checked(
        ["sudo", "-n", "docker", "volume", "prune", "-f"],
        label="docker volume prune task112",
    )
    run_checked(
        ["sudo", "-n", "docker", "builder", "prune", "-af"],
        label="docker builder prune task112",
    )


def build_storage_report(
    settings: Task112StorageSettings,
    *,
...
def run_task112_storage_remediation(settings: Task112StorageSettings) -> Task112StorageReport:
    """Execute the Task 112 storage remediation and return one deterministic report."""
