"""Reporting types and rendering helpers for the Hemma TTS benchmark.
...
Relationships:
    ...
    - Produces the `report.json` and `report.md` artifacts under
      `build/verification/hemma-tts-sidecar/`.
"""
...
class SpeechRequestEvidence:
    """Resolved request-input evidence for one TTS sidecar benchmark audio probe run."""
...
class PythonRecommendation:
    """Recommendation emitted from the live TTS sidecar benchmark runtime evidence."""
...
class BenchmarkReport:
    """Top-level JSON report payload for TTS sidecar benchmark evidence."""
...
def build_report_markdown(report: BenchmarkReport) -> str:
    """Render a human-readable markdown summary for TTS sidecar benchmark evidence."""
    lines = [
        "# Hemma TTS Sidecar Benchmark",
        "",
