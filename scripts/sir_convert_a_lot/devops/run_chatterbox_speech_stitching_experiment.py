"""Run the Chatterbox speech-aware stitching Chatterbox speech-aware stitching experiment from the
local repo.

Purpose:
    Invoke the committed Hemma-side Chatterbox speech-aware stitching experiment through the
    canonical
    `run-hemma` wrapper and mirror the resulting evidence bundle back into the
    local repo copy.

Relationships:
    - Calls `benchmark:chatterbox-speech-stitching-hemma` remotely in the Hemma repo clone.
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse CLI arguments for the local Chatterbox speech-aware stitching orchestrator."""
...
def main(argv: list[str] | None = None) -> int:
    """Run the remote Chatterbox speech-aware stitching experiment and sync the evidence locally."""
    ...
    remote_command = [
        ...
        "pdm",
        "run",
        "benchmark:chatterbox-speech-stitching-hemma",
        "--output-root",
    ...
    LOGGER.info("Running Chatterbox speech-aware stitching remotely on Hemma")
    ...
    LOGGER.info("Syncing Chatterbox speech-aware stitching evidence bundle back to the local repo")
