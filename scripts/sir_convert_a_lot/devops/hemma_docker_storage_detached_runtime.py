"""Runtime helpers for detached Hemma Docker storage remediation.

Purpose:
    Launch and inspect the Hemma Docker storage remediation Docker storage migration through a
    detached
    remote tmux session so host-wide Hemma maintenance work does not depend on
    the local client session remaining attached.

Relationships:
    - Used by `run_hemma_docker_storage_detached.py`.
    - Wraps the committed `hemma-docker-storage-remediation` runner.
    - Writes local launch/status metadata while the canonical migration report
      is still produced by the remote Hemma Docker storage remediation runner.
"""
...
DEFAULT_REMOTE_OUTPUT_ROOT = (
    DEFAULT_REMOTE_REPO_ROOT / "build" / "verification" / ("hemma-docker-storage-remediation")
)
DEFAULT_SESSION_NAME_PREFIX = "hemma-docker-storage-docker-storage"


@dataclass(frozen=True)
class HemmaDockerStorageDetachedLaunch:
    """
    Deterministic launch metadata for one detached Hemma Docker storage remediation migration run.
    """
...
@dataclass(frozen=True)
class HemmaDockerStorageDetachedStatus:
    """
    Deterministic status snapshot for one detached Hemma Docker storage remediation migration run.
    """
...
def build_remote_shell_command(*, output_root: Path) -> tuple[str, Path, Path]:
    """
    Build the remote shell command for one detached Hemma Docker storage remediation migration run.
    """
    ...
    remote_command = " ".join(
        [
            ...
            "&&",
            (
                "pdm run hemma-docker-storage-remediation "
...
def launch_detached_docker_storage_migration(
    *,
    session_name: str,
    output_root: Path,
) -> HemmaDockerStorageDetachedLaunch:
    """Launch one detached remote tmux session for Hemma Docker storage remediation."""
    ...
    run_local_checked(
        ...
        label="launch detached hemma-docker-storage tmux session",
    )
...
def inspect_detached_docker_storage_migration(
    launch: HemmaDockerStorageDetachedLaunch,
) -> HemmaDockerStorageDetachedStatus:
    """Inspect one detached Hemma Docker storage session plus report and log artifacts."""
    ...
    return HemmaDockerStorageDetachedStatus(
...
def _read_optional_remote_int(path: Path) -> int | None:
    ...
    try:
        ...
        raise SystemExit(
            f"Detached Hemma Docker storage remediation exit code file was malformed: {stripped}"
        ) from exc
...
def _read_optional_remote_json(path: Path) -> dict[str, object] | None:
    ...
    if not isinstance(loaded, dict):
        raise SystemExit(
            "Detached Hemma Docker storage remediation remote JSON payload was malformed."
        )
    return loaded
...
def _read_optional_remote_tail(path: Path, *, line_count: int) -> str:
    ...
    return run_local_checked(
        ...
        label="read optional remote log tail hemma-docker-storage",
    )
...
def _read_optional_remote_file(path: Path) -> str | None:
    ...
    payload = run_local_checked(
        ["pdm", "run", "run-hemma", "--", "/bin/bash", "-lc", command],
        label="read optional remote file hemma-docker-storage",
    )
