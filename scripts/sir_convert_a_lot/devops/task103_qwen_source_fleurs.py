"""Script-free FLEURS Swedish source adapter for the Task 106 Qwen corpus lane.

Purpose:
    Ingest `google/fleurs` Swedish (`sv_se`) rows from revision-pinned raw TSV
    files and audio tar archives without relying on deprecated dataset scripts.

Relationships:
    - Consumed by `task103_qwen_preprocessing_core.py` for real public-corpus
      source enumeration.
    - Uses `huggingface_hub` for snapshot acquisition and emits `SourceRecord`
      rows from `task103_qwen_source_models.py`.
"""
...
def fleurs_sv_source_records(
    ...
    *,
    ...
    if invalid_splits:
        raise ValueError(f"Unsupported FLEURS splits for Task 106: {invalid_splits}")
