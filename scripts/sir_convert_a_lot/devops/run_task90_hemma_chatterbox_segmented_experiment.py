"""Run the Task 90 Chatterbox segmented-text experiment on Hemma.
...
Relationships:
    ...
    - Proves the Task 90 segmentation layer through the same ADR-0007 sidecar
...
class ExperimentSettings:
    """Normalized CLI settings for the Task 90 Hemma experiment."""
...
@dataclass(frozen=True)
class LaneSummary:
    """One summarized Task 86 lane outcome for the Task 90 report."""
...
def _parse_args(argv: list[str]) -> ExperimentSettings:
    """Parse CLI arguments into normalized Task 90 settings."""
    parser = argparse.ArgumentParser(
        description="Run the Task 90 segmented Chatterbox experiment on Hemma."
    )
...
def _prepare_output_root(output_root: Path) -> dict[str, Path]:
    """Create a clean deterministic output tree for Task 90."""
...
def _run_task86_lane(
    *,
    ...
    """Execute one Task 86 lane in-process on Hemma."""
    argv = [
        "--output-root",
...
def _load_lane_summary(lane_id: str, output_root: Path) -> LaneSummary:
    """Load one Task 86 report into the Task 90 summary shape."""
...
def _write_summary(
    *,
    ...
    """Write one deterministic Task 90 summary bundle."""
    payload = {
        "benchmark_id": "task-90-chatterbox-segmented-hemma",
    ...
    markdown = "\n".join(
        [
            "# Task 90 Chatterbox Segmented Experiment",
            "",
...
def main(argv: list[str] | None = None) -> int:
    """Run the Task 90 Hemma experiment and write deterministic evidence."""
    ...
    LOGGER.info("Running Task 90 single-pass baseline lane")
    baseline_returncode = _run_task86_lane(
    ...
    LOGGER.info("Running Task 90 segmented lane")
    segmented_returncode = _run_task86_lane(
