...
Purpose:
    Provide the governed Hemma-only AMD PaddleOCR container probe probe that inventories the
...
Relationships:
    - Consumes an existing formula candidate evaluation rendered formula crop/page image.
    - Produces AMD PaddleOCR container probe runtime evidence under `build/verification/`.
    - Complements specialist OCR command adapter by probing native AMD container support instead of
...
DEFAULT_IMAGE = "ccr-2vdh3abv-pub.cnc.bj.baidubce.com/paddlepaddle/paddleocr-vl:latest-amd-gpu"
CONTAINER_INPUT_PATH = Path("/paddleocr-amd/input.png")
CONTAINER_OUTPUT_DIR = Path("/paddleocr-amd/output")
CONTAINER_PROBE_PATH = Path("/paddleocr-amd/output/paddleocr-amd_paddleocr_amd_inner_probe.py")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    probe_path = output_dir / "paddleocr-amd_paddleocr_amd_inner_probe.py"
    ...
    host_metadata = {
        "schema_version": "paddleocr-amd_paddleocr_amd_container_host_v1",
    ...
    (output_dir / "paddleocr-amd-paddleocr-amd-host-metadata.json").write_text(
...
def build_parser() -> argparse.ArgumentParser:
    """Build the AMD PaddleOCR container probe probe parser."""
...
def main() -> int:
    ...
    payload: dict[str, object] = {
        "schema_version": "paddleocr-amd_paddleocr_amd_container_inner_v1",
        "created_at_utc": datetime.now(UTC).isoformat(),
        "hostname": platform.node(),
...
def write_payload(output_dir: Path, payload: dict[str, object]) -> None:
    (output_dir / "paddleocr-amd-paddleocr-amd-probe.json").write_text(
