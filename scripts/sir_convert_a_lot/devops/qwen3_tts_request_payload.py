"""Request payload and input-evidence helpers for the TTS sidecar benchmark Qwen benchmark.
...
Relationships:
    - Used by `run_tts_sidecar_hemma_benchmark`.
    - Used by `tts_sidecar_benchmark_runtime`.
    - Used by `tts_sidecar_benchmark_reporting` through the prepared input
    ...
    *,
    ...
    if task_type == "CustomVoice":
        if voice is None or voice.strip() == "":
            raise SystemExit(
                "TTS sidecar benchmark CustomVoice requests require a configured voice."
            )
        payload["voice"] = voice
        return payload
    if task_type == "Base":
        if reference_audio is None:
            raise SystemExit("TTS sidecar benchmark Base requests require `--reference-audio`.")
        if reference_transcript is None or reference_transcript.strip() == "":
            raise SystemExit(
                "TTS sidecar benchmark Base requests require `--reference-transcript` or "
                "`--reference-transcript-file`."
            )
        payload["ref_audio"] = encode_audio_to_data_url(reference_audio)
        payload["ref_text"] = reference_transcript
        return payload
    raise SystemExit(f"Unsupported TTS sidecar benchmark task type: {task_type}")
