"""Reusable runtime helpers for Task 344 page-window replay diagnostics.
...
Relationships:
    - Imported by `devops.task344_page_window_replay`.
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/task-344-page-window-replay")
MODULE_NAME = "scripts.sir_convert_a_lot.devops.task344_page_window_replay"
...
def utc_run_id() -> str:
    ...
    return datetime.now(UTC).strftime("task344-page-window-replay-%Y%m%dT%H%M%SZ")
...
def run_replay(settings: RunSettings) -> int:
    ...
    report_payload: dict[str, object] = {
        "schema_version": "task344_page_window_replay_v1",
