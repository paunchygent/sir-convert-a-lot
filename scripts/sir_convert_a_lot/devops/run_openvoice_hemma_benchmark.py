"""Run the Hemma OpenVoice sidecar benchmark on the live R9700 host.
...
Relationships:
    - Intended to run on Hemma via `pdm run run-hemma -- pdm run benchmark:openvoice ...`.
    - Uses the normalized ADR-0007 sidecar endpoints:
      `/health`, `/capabilities`, `/voices`, and `/synthesize`.
    - Writes deterministic evidence under `build/verification/openvoice-v2-hemma/`.
"""
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/openvoice-v2-hemma")
DEFAULT_DOCKERFILE = Path("containers/tts-sidecar-openvoice/Dockerfile")
DEFAULT_IMAGE = "sir-convert-a-lot/openvoice-sidecar-openvoice:local"
DEFAULT_NETWORK = "hule-network"
DEFAULT_NETWORK_ALIAS = "sir-convert-a-lot-openvoice-openvoice"
DEFAULT_CONTAINER_NAME = "sir_convert_a_lot_openvoice_openvoice"
...
def _parse_args(argv: list[str]) -> BenchmarkSettings:
    """Parse CLI arguments into normalized OpenVoice benchmark settings."""
    parser = argparse.ArgumentParser(description="Run the Hemma OpenVoice benchmark.")
...
def _official_support_summary() -> list[str]:
    """Return the official upstream support statements that frame OpenVoice benchmark evidence."""
    return [
        ...
        "This OpenVoice benchmark adapter uses facebook/mms-tts-swe as the Swedish base speaker to "
...
def main(argv: list[str] | None = None) -> int:
    """Run the OpenVoice benchmark and write deterministic evidence artifacts."""
    ...
    try:
        ...
        if synthesis_result.ok is not True:
            raise SystemExit(
                "OpenVoice benchmark acceptance failed: `/synthesize` did not return wav audio."
            )
