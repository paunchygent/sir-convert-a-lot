"""Script-free Waxholm source adapter for the Task 106 Qwen corpus lane.

Purpose:
    Ingest labeled `KTH/waxholm` rows from raw repository files and `.smp.mix`
    transcript labels without relying on deprecated dataset scripts.

Relationships:
    - Consumed by `task103_qwen_preprocessing_core.py` for high-trust control
      corpus enumeration.
