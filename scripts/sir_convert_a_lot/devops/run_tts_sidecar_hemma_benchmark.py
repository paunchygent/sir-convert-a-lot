"""Run the Hemma TTS sidecar benchmark on the live R9700 host.
...
Relationships:
    - Intended to run on Hemma via `pdm run run-hemma -- pdm run benchmark:tts-sidecar`.
    - Uses the committed TTS sidecar benchmark stage-config asset shipped in this repo.
    - Writes deterministic evidence under `build/verification/hemma-tts-sidecar/`.
"""
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/tts-sidecar-hemma")
...
DEFAULT_NETWORK_ALIAS = "sir-convert-a-lot-tts-tts-sidecar"
DEFAULT_CONTAINER_NAME = "sir_convert_a_lot_tts_tts-sidecar"
...
STAGE_CONFIG_PATH = Path("scripts/sir_convert_a_lot/devops/qwen3_tts_stage_config.yaml")
...
def _default_hf_cache_dir() -> Path:
    """Resolve the canonical Hemma Hugging Face cache path for TTS sidecar benchmark."""
...
def _parse_args(argv: list[str]) -> BenchmarkSettings:
    ...
    parser = argparse.ArgumentParser(description="Run the Hemma TTS benchmark.")
    ...
    model = (
        ...
        if args.model is not None
            ...
            DEFAULT_BASE_MODEL if str(args.profile_type) == "Base" else DEFAULT_CUSTOM_VOICE_MODEL
        )
    )
    ...
    return BenchmarkSettings(
        ...
        model=model,
        task_type=str(args.profile_type),
...
def main(argv: list[str] | None = None) -> int:
    """Run the TTS sidecar benchmark sidecar benchmark and write report artifacts."""
    ...
    try:
        ...
        if settings.task_type != "Base" and settings.voice not in voice_names:
            ...
            raise SystemExit(
                "Configured TTS sidecar benchmark voice "
        ...
        if not any(result.ok and result.response_format == "wav" for result in audio_results):
            raise SystemExit(
                "TTS sidecar acceptance failed: `/v1/audio/speech` did not return wav."
            )
        ...
        with suppress(SystemExit):
            logs_output = docker_checked(
                ...
                label="docker logs tts-sidecar container",
            )
        ...
        if not settings.retain_container:
            with suppress(SystemExit):
                docker_checked(
                    ["rm", "-f", settings.container_name], label="docker rm -f tts-sidecar"
                )
