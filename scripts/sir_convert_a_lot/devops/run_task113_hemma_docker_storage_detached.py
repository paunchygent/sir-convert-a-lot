"""Launch and inspect detached Task 113 Hemma Docker storage remediation.

Purpose:
    Provide the committed launch/status surface for the host-wide Task 113
    Docker storage migration so the live Hemma change can run detached from the
    local client session.

Relationships:
    - Wraps `task113_hemma_docker_storage_detached_runtime.py`.
    - Launches the committed `task-113-docker-storage-remediation` runner on
      Hemma through a remote tmux session.
    - Writes deterministic local launch/status artifacts under
      `build/verification/task-113-hemma-docker-storage-remediation-detached/`.
"""
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/task-113-hemma-docker-storage-remediation-detached")


def _build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser for detached Task 113 launch/status workflows."""
    parser = argparse.ArgumentParser(
        description="Launch and inspect detached Task 113 Hemma Docker storage remediation."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    launch = subparsers.add_parser("launch", help="Launch one detached Task 113 migration run.")
    ...
    status = subparsers.add_parser("status", help="Inspect one detached Task 113 migration run.")
...
def _prepare_output_root(output_root: Path) -> None:
    """Create the deterministic local output root for detached Task 113 artifacts."""
...
def _load_launch(output_root: Path) -> Task113DetachedLaunch:
    """Load the previously written detached Task 113 launch metadata."""
    payload = json.loads(_launch_path(output_root).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise SystemExit("Detached Task 113 launch metadata was malformed.")
...
def _required_str(payload: dict[str, object], key: str) -> str:
    ...
    if not isinstance(value, str):
        raise SystemExit(f"Detached Task 113 metadata returned malformed `{key}`.")
    return value


def _render_status_markdown(status: Task113DetachedStatus) -> str:
    """Render one concise Markdown status view for detached Task 113."""
    lines = [
        "# Task 113 Detached Docker Storage Migration Status",
        "",
...
def main(argv: list[str] | None = None) -> int:
    """Launch or inspect detached Task 113 Hemma Docker storage remediation."""
    ...
    if args.command == "launch":
        ...
        launch = launch_detached_task113_migration(
    ...
    if args.command == "status":
        ...
        status = inspect_detached_task113_migration(launch)
    ...
    raise SystemExit(f"Unsupported detached Task 113 command: {args.command}")
