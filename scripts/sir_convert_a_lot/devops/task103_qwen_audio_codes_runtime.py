...
Relationships:
    - Imported by `task103_qwen_preprocessing_finalization.py` to configure the
      warm tokenizer loader.
    - Embedded in `Task103PreprocessingSettings` so Task 103/109 finalization
      can request the governed tokenizer posture explicitly.
    - Reused by Task 101 bundle runtime helpers so bundle provenance can record
...
class Task103AudioCodesRuntimeSettings:
...
def governed_qwen_audio_codes_runtime_settings() -> Task103AudioCodesRuntimeSettings:
    """Return the canonical governed ROCm-backed audio-code runtime posture."""
    return Task103AudioCodesRuntimeSettings(
...
def render_audio_codes_runtime_settings(
    settings: Task103AudioCodesRuntimeSettings,
) -> dict[str, object]:
