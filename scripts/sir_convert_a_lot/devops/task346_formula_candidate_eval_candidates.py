"""Run Task 346 formula/OCR candidate adapters.
...
Relationships:
    - Consumes source inputs from `task346_formula_candidate_eval_inputs`.
    - Feeds candidate summaries to Task 346 report writers.
...
def run_granite_baseline(
    *,
    ...
    return {
        ...
        "label": "Current Task 344 Granite/Docling baseline",
        "status": "succeeded",
        ...
        "notes": "Persisted Task 344 affected-window Markdown, not rerun.",
    }
        ...
        if deepseek_batch_template is None or not deepseek_batch_template.strip():
            return {
                ...
                "detail": (
                    ...
                    "SIR_CONVERT_A_LOT_TASK346_DEEPSEEK_OCR2_BATCH_COMMAND."
                ),
            }
        ...
        if deepseek_template is None or not deepseek_template.strip():
            return {
                ...
                "detail": (
                    ...
                    "SIR_CONVERT_A_LOT_TASK346_DEEPSEEK_OCR2_COMMAND."
                ),
            }
...
def collect_marker_counts(text: str) -> dict[str, int]:
    """Count Task 344 known bad output markers."""
...
def default_external_candidates() -> tuple[CandidateSpec, ...]:
    """Return the Task 346 candidate matrix."""
    return (
        CandidateSpec(
...
def baseline_elapsed_ms(report: Mapping[str, object]) -> int | None:
    """Extract the Task 344 baseline elapsed time when present."""
