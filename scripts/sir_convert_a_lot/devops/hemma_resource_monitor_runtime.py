"""Runtime helpers for the detached Hemma resource monitor.
...
Relationships:
    - Used by `run_hemma_resource_monitor.py`.
...
from scripts.sir_convert_a_lot.devops.hemma_resource_monitor_models import (
    ...
    HemmaResourceMonitorRunState,
    HemmaResourceMonitorStatus,
    HemmaResourceMonitorSummary,
    HemmaResourceSample,
    RuntimeKind,
)
...
def default_launch_id() -> str:
    ...
    return datetime.now(UTC).strftime("hemma-resource-monitor-resource-%Y%m%dt%H%M%Sz").lower()
...
def load_samples(launch_root: Path) -> list[HemmaResourceSample]:
    ...
    if not path.exists():
        return []
    samples: list[HemmaResourceSample] = []
...
def summarize_samples(
    launch_id: str, samples: list[HemmaResourceSample]
) -> HemmaResourceMonitorSummary:
    ...
    return HemmaResourceMonitorSummary(
        launch_id=launch_id,
...
def write_launch_metadata(path: Path, payload: HemmaResourceMonitorLaunch) -> None:
    """Write one launch metadata artifact."""
    write_json(path, asdict(payload))


def write_worker_state(path: Path, payload: HemmaResourceMonitorRunState) -> None:
    """Write one worker-state artifact."""
    write_json(path, asdict(payload))


def write_summary(path: Path, payload: HemmaResourceMonitorSummary) -> None:
    """Write one summary artifact."""
    write_json(path, asdict(payload))


def write_status(path: Path, payload: HemmaResourceMonitorStatus) -> None:
...
def build_status(launch_root: Path) -> HemmaResourceMonitorStatus:
    ...
    return HemmaResourceMonitorStatus(
    ...
    *,
    ...
    try:
        while True:
            ...
            latest_sample = HemmaResourceSample(
