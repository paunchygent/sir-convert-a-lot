...
Relationships:
    - Used by `run_task204_hemma_scratch_policy.py` for timer commands.
    - Used by `task205_hemma_scratch_maintenance_runtime.py` for shared host
...
def _user_linger_enabled(username: str) -> bool:
    ...
    output = run_local_checked(
        ...
        label="loginctl show-user linger task205",
    )
    ...
    *,
    ...
    if enable_linger and not lingering_before:
        run_local_checked(
            ...
            label="enable linger task205",
        )
    run_local_checked(
        ["systemctl", "--user", "daemon-reload"],
        label="systemctl daemon-reload task205",
    )
    run_local_checked(
        ["systemctl", "--user", "enable", "--now", settings.timer_name],
        label="systemctl enable timer task205",
    )
...
def status_scratch_timer(settings: ScratchTimerSettings) -> ScratchTimerStatusReport:
    ...
    timer_list_output = run_local_checked(
        ...
        label="systemctl list-timers task205",
    )
...
def render_timer_status_markdown(report: ScratchTimerStatusReport) -> str:
    ...
    return "\n".join(
        [
            "# Task 205 Scratch Maintenance Timer Status",
            "",
