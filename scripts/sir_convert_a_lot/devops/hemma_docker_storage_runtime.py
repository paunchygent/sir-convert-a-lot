"""Runtime helpers for Hemma Docker storage-root remediation.
...
Relationships:
    - Used by `run_hemma_docker_storage_remediation.py`.
    - Complements Hemma storage remediation by fixing Docker's host-wide storage contract.
...
DEFAULT_DOCKER_ROOT_BACKUP = Path(
    "/var/snap/docker/common/var-lib-docker.hemma-docker-storage-backup"
)
...
class HemmaDockerStorageSettings:
    """Normalized settings for the Hemma Docker storage migration runner."""
...
@dataclass(frozen=True)
class HemmaDockerStorageReport:
    """Deterministic report for one Hemma Docker storage migration pass."""
...
def docker_root_dir() -> str:
    ...
    return run_checked(
        ...
        label="docker info root dir hemma-docker-storage",
    )
...
def snap_data_root() -> str:
    ...
    return run_checked(
        ...
        label="snap get data-root hemma-docker-storage",
    )
...
def docker_ps() -> str:
    ...
    return run_checked(
        ...
        label="docker ps hemma-docker-storage",
    )
...
def filesystem_df() -> str:
    ...
    return run_checked(
        ["df", "-h", "/", "/srv/scratch", "/srv/storage"], label="df hemma-docker-storage"
    )
...
def ensure_directory(path: Path) -> None:
    ...
    try:
        ...
        run_checked(
            ...
            label="sudo mkdir -p hemma-docker-storage",
        )
    ...
    if path == DEFAULT_FSTAB_PATH:
        ...
        try:
            run_checked(
                ...
                label="sudo cp fstab hemma-docker-storage",
            )
        finally:
...
def update_fstab_for_bind(
    *,
    ...
    """Persist the bind mount and remove superseded home-path entries."""
...
def stop_docker_snap() -> None:
    ...
    run_checked(
        ["sudo", "-n", "snap", "stop", "docker"], label="snap stop docker hemma-docker-storage"
    )
...
def start_docker_snap() -> None:
    ...
    run_checked(
        ["sudo", "-n", "snap", "start", "docker"], label="snap start docker hemma-docker-storage"
    )
...
def set_snap_data_root(docker_root: Path) -> None:
    ...
    run_checked(
        ...
        label="snap set docker data-root hemma-docker-storage",
    )
...
def rsync_tree(*, source: Path, destination: Path) -> None:
    ...
    run_checked(
        ...
        label="rsync docker root hemma-docker-storage",
    )
...
def move_tree(source: Path, destination: Path) -> None:
    ...
    if source.exists():
        run_checked(
            ...
            label="mv docker root hemma-docker-storage",
        )


def remove_tree(path: Path) -> None:
    """Delete one tree with sudo."""
    run_checked(["sudo", "-n", "rm", "-rf", path.as_posix()], label="rm -rf hemma-docker-storage")
...
def unmount_path(path: Path) -> None:
    ...
    if find_mount_source(path) is None:
        return
    run_checked(["sudo", "-n", "umount", path.as_posix()], label="umount hemma-docker-storage")
...
def mount_bind(source: Path, target: Path) -> None:
    ...
    run_checked(
        ...
        label="mount --bind hemma-docker-storage",
    )
...
) -> HemmaDockerStorageReport:
    """Build the deterministic Hemma Docker storage remediation post-migration report."""
    return HemmaDockerStorageReport(
...
def run_docker_storage_migration(
    settings: HemmaDockerStorageSettings,
) -> HemmaDockerStorageReport:
    """Execute the Hemma Docker storage migration and return one report."""
