"""Pilot-scoped resource-monitor helpers for detached Task 101 launches.

Purpose:
    Launch and inspect a high-resolution sibling resource monitor for each
    long-running Task 101 Qwen pilot so monitor evidence is discoverable from
    Task 101 artifacts without operator-side ad hoc commands.

Relationships:
    - Imported by `run_task101_hemma_qwen_pilot.py` during launch/resume.
    - Reuses Task 116 monitor runtime helpers for detached worker orchestration
      and sample/summary loading.
    - Feeds monitor evidence into Task 101 status inspection surfaces.
"""
...
DEFAULT_TASK101_RESOURCE_MONITOR_INTERVAL_SECONDS = 1.0
DEFAULT_TASK101_RESOURCE_MONITOR_RUNTIME_KIND: RuntimeKind = "rocm"


def default_task101_resource_monitor_launch_id(task101_launch_id: str) -> str:
    """Return the deterministic monitor launch id for one Task 101 launch."""
    return f"{task101_launch_id}-resource-monitor"


def task101_resource_monitor_output_root(task101_launch_root: Path) -> Path:
    """Return the pilot-scoped monitor output root for one Task 101 launch."""
    return task101_launch_root / "resource-monitor"


def launch_task101_resource_monitor(
    *,
    task101_launch_id: str,
    task101_launch_root: Path,
    runtime_kind: RuntimeKind,
    interval_seconds: float,
    duration_seconds: float | None,
) -> dict[str, object]:
    """Launch one pilot-scoped detached resource monitor and return metadata."""
    if interval_seconds <= 0:
        raise ValueError("Task 101 resource monitor interval must be positive.")
    if duration_seconds is not None and duration_seconds <= 0:
        raise ValueError("Task 101 resource monitor duration must be positive when provided.")
    monitor_output_root = task101_resource_monitor_output_root(task101_launch_root)
    monitor_output_root.mkdir(parents=True, exist_ok=True)
    monitor_launch_id = default_task101_resource_monitor_launch_id(task101_launch_id)
...
def inspect_task101_resource_monitor(
    resource_monitor: Mapping[str, object] | None,
    *,
    phase_history: Sequence[Mapping[str, object]] | None,
) -> dict[str, object] | None:
    """Inspect one linked Task 101 resource monitor and compute phase summaries."""
    ...
    if not isinstance(launch_root_value, str) or launch_root_value.strip() == "":
        return {"available": False, "error": "Task 101 monitor metadata lacked `launch_root`."}
    launch_root = Path(launch_root_value)
    if not launch_root.exists():
        return {
            "available": False,
            "error": (f"Task 101 monitor launch root does not exist: {launch_root.as_posix()}"),
        }
...
def _group_samples_by_phase(
    *,
    ...
    """Group monitor samples by Task 101 phase-history timestamps."""
...
def _sorted_phase_events(
    ...
    """Return sorted `(timestamp, phase)` events from Task 101 phase history."""
