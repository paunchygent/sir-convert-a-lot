"""Stdout summary helpers for the Hemma PDF throughput benchmark command.
...
Relationships:
    - Used by `run_pdf_throughput_hemma_benchmark`.
...
def build_stdout_summary(
    *,
    ...
    if not isinstance(runtime_parity_obj, dict):
        raise SystemExit("PDF throughput benchmark payload is missing `runtime_parity`.")
