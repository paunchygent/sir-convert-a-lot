"""Launch and inspect the detached Task 108 Qwen preprocessing proof on Hemma.

Purpose:
    Provide the committed detached execution surface for the bounded Task 108
    proof so long-running Hemma preprocessing does not depend on the local
    client session remaining attached.

Relationships:
    - Wraps `task108_qwen_detached_proof_runtime.py`.
    - Reuses the canonical Task 109 container/runtime and Task 100 image
      helpers.
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/task-108-qwen-detached-proof")
DEFAULT_CONTAINER_NAME_PREFIX = "task108-qwen-proof"
DEFAULT_RIXVOX_SPLITS = ("train", "dev", "test")
DEFAULT_RIXVOX_MAX_ROWS_PER_SPLIT = 64


def _parse_shared_settings(args: argparse.Namespace) -> Task109ContainerizedPreprocessingSettings:
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the committed CLI parser for detached Task 108 proof workflows."""
    parser = argparse.ArgumentParser(
        description="Launch and inspect the detached Task 108 Qwen preprocessing proof on Hemma."
    )
    ...
    launch.add_argument(
        ...
        default=DEFAULT_CONTAINER_NAME_PREFIX,
    )
    launch.add_argument(
        "--skip-build",
        action="store_true",
    ...
    status = subparsers.add_parser("status", help="Inspect one detached Task 108 proof container.")
...
def _status_markdown(status: Task108DetachedProofStatus) -> str:
    """Render one concise Markdown summary for the detached Task 108 proof."""
    lines = [
        "# Task 108 Detached Qwen Proof Status",
        "",
...
def _load_launch(output_root: Path) -> Task108DetachedProofLaunch:
    ...
    if not isinstance(payload, dict):
        raise SystemExit("Detached Task 108 launch metadata was malformed.")
...
def _required_str(payload: dict[str, object], key: str) -> str:
    ...
    if not isinstance(value, str):
        raise SystemExit(f"Detached Task 108 metadata returned malformed `{key}`.")
    return value
...
def main(argv: list[str] | None = None) -> int:
    """Launch or inspect the detached bounded Task 108 proof on Hemma."""
    ...
    if args.command == "launch":
        ...
        run_checked(
            ...
            label="rocm-smi task108 detached preflight",
        )
        ...
        launch = launch_detached_task108_proof(
            settings,
            repo_root=repo_root,
            hf_mount=hf_mount,
    ...
    raise SystemExit(f"Unsupported detached Task 108 command: {args.command}")
