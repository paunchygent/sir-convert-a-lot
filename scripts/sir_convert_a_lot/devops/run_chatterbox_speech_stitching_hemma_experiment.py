"""Run the Chatterbox speech-aware stitching Chatterbox speech-aware stitching experiment on Hemma.
...
Relationships:
    ...
    - Proves the Chatterbox speech-aware stitching speech-aware stitching layer through the same
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/chatterbox-speech-aware-stitching-hemma")
...
class ExperimentSettings:
    """Normalized CLI settings for the Chatterbox speech-aware stitching Hemma experiment."""
...
@dataclass(frozen=True)
class LaneSummary:
    """One summarized Chatterbox benchmark segmented-lane outcome for the Chatterbox speech-aware
    stitching report.
    """
...
def _parse_args(argv: list[str]) -> ExperimentSettings:
    """Parse CLI arguments into normalized Chatterbox speech-aware stitching settings."""
    parser = argparse.ArgumentParser(
        description="Run the Chatterbox speech-aware stitching experiment on Hemma."
    )
...
def _prepare_output_root(output_root: Path) -> dict[str, Path]:
    """Create a clean deterministic output tree for Chatterbox speech-aware stitching."""
...
def _run_chatterbox_benchmark_lane(
    *,
    ...
    """Execute one Chatterbox benchmark segmented lane in-process on Hemma."""
    argv = [
        "--output-root",
...
def _load_lane_summary(lane_id: str, stitch_mode: str, output_root: Path) -> LaneSummary:
    """
    Load one Chatterbox benchmark report into the Chatterbox speech-aware stitching summary shape.
    """
...
def _write_summary(
    *,
    ...
    """Write one deterministic Chatterbox speech-aware stitching summary bundle."""
    payload = {
        "benchmark_id": "chatterbox-speech-aware-stitching-hemma",
    ...
    markdown = "\n".join(
        [
            "# Chatterbox speech-aware stitching Chatterbox Speech-Aware Stitching Experiment",
            "",
...
def main(argv: list[str] | None = None) -> int:
    """Run the Chatterbox speech-aware stitching Hemma experiment."""
    ...
    LOGGER.info("Running Chatterbox speech-aware stitching segmented simple-stitch baseline lane")
    simple_returncode = _run_chatterbox_benchmark_lane(
    ...
    LOGGER.info("Running Chatterbox speech-aware stitching segmented speech-aware stitch lane")
    speech_aware_returncode = _run_chatterbox_benchmark_lane(
