"""Hemma production profile parsing for PDF throughput benchmark evidence.

Purpose:
    Convert canonical Hemma environment values into the deployed service profile
    recorded by the PDF throughput benchmark report.

Relationships:
    - Used by `run_pdf_throughput_hemma_benchmark` after syncing the production env
      mirror.
