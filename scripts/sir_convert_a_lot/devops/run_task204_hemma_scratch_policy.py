...
Purpose:
    Provide one deterministic CLI for Task 204 scratch auditing/remediation and
    Task 205 recurring maintenance/timer operations.

Relationships:
    - Wraps `task204_hemma_scratch_policy_runtime.py` for audit/remediation.
    - Wraps `task205_hemma_scratch_maintenance_runtime.py` for idle-safe
      recurring maintenance.
    - Wraps `task205_hemma_scratch_timer_runtime.py` for timer installation and
      status inspection.
...
from scripts.sir_convert_a_lot.devops.task204_hemma_scratch_policy_reporting import (
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/task-204-hemma-scratch-policy")
