"""Run the committed Task 112 Hemma storage remediation surface.
...
Relationships:
    - Wraps `task112_hemma_storage_runtime.py`.
    - Writes deterministic evidence under
      `build/verification/task-112-hemma-storage-remediation/`.
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/task-112-hemma-storage-remediation")


def _parse_args(argv: list[str] | None) -> tuple[Path, Task112StorageSettings]:
    """Parse CLI arguments into one deterministic Task 112 settings object."""
    parser = argparse.ArgumentParser(
        description="Run Task 112 Hemma storage remediation with deterministic evidence."
    )
...
def _prepare_output_root(output_root: Path) -> tuple[Path, Path]:
    """Create the deterministic Task 112 report paths."""
...
def _render_markdown(payload: dict[str, object]) -> str:
    """Render one concise Task 112 Markdown report."""
    return (
        "# Task 112 Hemma Storage Remediation Report\n\n"
...
def main(argv: list[str] | None = None) -> int:
    """Run Task 112 storage remediation and write deterministic evidence."""
    output_root, settings = _parse_args(argv)
    report = run_task112_storage_remediation(settings)
