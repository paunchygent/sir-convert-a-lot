"""Run the Chatterbox benchmark Hemma Chatterbox Multilingual sidecar benchmark.
...
Relationships:
    - Intended to run on Hemma via `pdm run run-hemma -- pdm run benchmark:chatterbox`.
    ...
    - Writes deterministic evidence under
      `build/verification/chatterbox-hemma/`.
"""
...
from scripts.sir_convert_a_lot.devops.chatterbox_benchmark_reporting import (
    BenchmarkReport,
    build_report_markdown,
)
from scripts.sir_convert_a_lot.devops.chatterbox_benchmark_runtime import (
    BenchmarkSettings,
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/chatterbox-hemma")
DEFAULT_DOCKERFILE = Path("containers/tts-sidecar-chatterbox/Dockerfile")
DEFAULT_IMAGE = "sir-convert-a-lot/chatterbox-sidecar-chatterbox:local"
DEFAULT_NETWORK = "hule-network"
DEFAULT_NETWORK_ALIAS = "sir-convert-a-lot-chatterbox-chatterbox"
DEFAULT_CONTAINER_NAME = "sir_convert_a_lot_chatterbox_chatterbox"
...
def _resolve_text_argument(*, text: str | None, text_file: Path | None, label: str) -> str:
    ...
    if text_file is None:
        if text is None:
            raise SystemExit(f"Chatterbox benchmark {label} text is missing.")
        return text
    if not text_file.exists():
        raise SystemExit(f"Chatterbox benchmark {label} text file is missing: {text_file}")
    file_text = text_file.read_text(encoding="utf-8").strip()
    if file_text == "":
        raise SystemExit(f"Chatterbox benchmark {label} text file is empty: {text_file}")
    return file_text


def _parse_args(argv: list[str]) -> BenchmarkSettings:
    """Parse CLI arguments into normalized Chatterbox benchmark settings."""
    parser = argparse.ArgumentParser(
        description="Run the Chatterbox benchmark Hemma Chatterbox benchmark."
    )
...
def _ensure_preconditions(settings: BenchmarkSettings) -> None:
    ...
    if not settings.dockerfile_path.resolve().exists():
        raise SystemExit(f"Chatterbox benchmark Dockerfile is missing: {settings.dockerfile_path}")
    if not settings.reference_audio_path.exists():
        raise SystemExit(
            f"Chatterbox benchmark reference audio is missing: {settings.reference_audio_path}"
        )
    if (
        ...
        raise SystemExit(
            "Chatterbox benchmark English reference audio is missing: "
            f"{settings.english_reference_audio_path}"
        )
    running = run_checked(
...
def main(argv: list[str] | None = None) -> int:
    """Run the Chatterbox benchmark and write deterministic evidence artifacts."""
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    settings = _parse_args(sys.argv[1:] if argv is None else argv)
    LOGGER.info(
        (
            "Chatterbox benchmark starting: output_root=%s skip_build=%s exaggeration=%s "
    ...
    try:
        ...
        LOGGER.info(
            "Chatterbox benchmark completed successfully: smoke=%s clone=%s",
