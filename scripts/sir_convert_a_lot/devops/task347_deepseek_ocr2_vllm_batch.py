"""Run DeepSeek-OCR-2 vLLM batch inference for Task 346 replay.

Purpose:
    Provide the governed Hemma-only DeepSeek-OCR-2/vLLM command adapter used by
    Task 346 to evaluate affected page images in one model-load batch.

Relationships:
    - Invoked by `task346_formula_candidate_eval` through its batch command
      template.
    - Reuses the Task 301/309 ROCm vLLM container, device, and cache pattern.
...
DEFAULT_WORKSPACE_ROOT = Path(
    "/home/paunchygent/.data/sir-convert-a-lot/task-347/deepseek-ocr2-vllm"
)
...
CONTAINER_INPUT_DIR = Path("/task347/input")
CONTAINER_OUTPUT_DIR = Path("/task347/output")
CONTAINER_REPO_DIR = Path("/deepseek-ocr-2")
CONTAINER_RUNNER_PATH = Path("/task347/output/task347_deepseek_ocr2_vllm_inner.py")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    runner_path = output_dir / "task347_deepseek_ocr2_vllm_inner.py"
    runner_path.write_text(render_inner_runner(), encoding="utf-8")
    metadata_path = output_dir / "task347-vllm-host-metadata.json"
    ...
    payload = {
        "schema_version": "task347_deepseek_ocr2_vllm_host_v1",
...
def main() -> int:
    ...
    metadata = {
        "schema_version": "task347_deepseek_ocr2_vllm_inner_v1",
        "created_at_utc": datetime.now(UTC).isoformat(),
    ...
    (args.output_dir / "task347-vllm-inner-metadata.json").write_text(
