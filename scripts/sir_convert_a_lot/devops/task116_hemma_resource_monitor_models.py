"""Typed contracts for the detached Task 116 Hemma resource monitor.

Purpose:
    Centralize the Task 116 monitor launch, sample, summary, and status models
    so the CLI and runtime stay under the repo's module-size boundary.

Relationships:
    - Imported by `run_task116_hemma_resource_monitor.py` for operator I/O.
    - Imported by `task116_hemma_resource_monitor_runtime.py` for worker state and
...
class Task116ResourceMonitorLaunch:
...
@dataclass(frozen=True)
class Task116ResourceMonitorRunState:
...
@dataclass(frozen=True)
class Task116ResourceMonitorSummary:
...
@dataclass(frozen=True)
class Task116ResourceMonitorStatus:
