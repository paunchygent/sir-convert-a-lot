"""Run the Task 89 Chatterbox eSpeak experiment on Hemma.
...
Relationships:
    - Reuses the existing Task 86 benchmark runner for the actual Chatterbox
      synthesis lanes.
    - Keeps eSpeak preprocessing outside the Chatterbox sidecar contract.
    - Intended to be invoked on Hemma through the local Task 89 orchestrator.
"""
...
from scripts.sir_convert_a_lot.devops.task81_openvoice_runtime import docker_checked
...
DEFAULT_HELPER_IMAGE = "sir-convert-a-lot/textprep-espeak-task89:local"
...
class ExperimentSettings:
    """Normalized CLI settings for the Task 89 Hemma experiment."""
...
@dataclass(frozen=True)
class LaneSummary:
    """One summarized lane outcome for the Task 89 report."""
...
def _parse_args(argv: list[str]) -> ExperimentSettings:
    """Parse CLI arguments into normalized Task 89 settings."""
    parser = argparse.ArgumentParser(description="Run the Task 89 Hemma eSpeak experiment.")
...
def _prepare_output_root(output_root: Path) -> dict[str, Path]:
    """Create a clean deterministic output tree for Task 89."""
...
def _ensure_helper_image(settings: ExperimentSettings) -> tuple[bool, str]:
    ...
    try:
        image_id = docker_checked(
            ...
            label="docker image inspect task89 helper",
        )
    except SystemExit:
        image_present = False
        image_id = ""
    ...
    if build_performed:
        docker_checked(["buildx", "version"], label="docker buildx version task89 helper")
        docker_checked(
            [
                "buildx",
                "build",
            ...
            label="docker buildx build task89 helper image",
        )
        image_id = docker_checked(
            ["image", "inspect", settings.helper_image, "--format", "{{.Id}}"],
            label="docker image inspect task89 helper after build",
        )
...
def _run_phonemizer(
    ...
    *,
    ...
    docker_checked(command, label="docker run task89 helper")
...
def _load_lane_summary(lane_id: str, output_root: Path) -> LaneSummary:
    """Load one Task 86 report into the Task 89 summary shape."""
...
def _write_summary(
    *,
    ...
    """Write one deterministic Task 89 summary bundle."""
    payload = {
        "benchmark_id": "task-89-chatterbox-espeak-hemma",
    ...
    markdown = "\n".join(
        [
            "# Task 89 Chatterbox eSpeak Experiment",
            "",
...
def main(argv: list[str] | None = None) -> int:
    """Run the Task 89 Hemma experiment and write deterministic evidence."""
    ...
    espeak_returncode = _run_task86_lane(
    ...
    if baseline_returncode != 0 or espeak_returncode != 0:
        LOGGER.error(
            "Task 89 completed with failing lanes: baseline=%s espeak=%s",
            baseline_returncode,
            espeak_returncode,
        )
        return 1
    LOGGER.info("Task 89 completed successfully")
    return 0
