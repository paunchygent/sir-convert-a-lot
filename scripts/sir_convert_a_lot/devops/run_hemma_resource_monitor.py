"""Launch and inspect the detached Hemma resource monitor.
...
Relationships:
    - Uses `hemma_resource_monitor_runtime.py` for worker execution,
      detached-process spawn, and artifact helpers.
    - Writes deterministic evidence under
      `build/verification/hemma-resource-monitor/`.
"""
...
DEFAULT_OUTPUT_ROOT = Path("build/verification/hemma-resource-monitor-hemma-resource-monitor")


def _build_parser() -> argparse.ArgumentParser:
    """Build the committed Hemma resource monitor CLI."""
    parser = argparse.ArgumentParser(
        description="Launch and inspect a detached Hemma resource monitor."
    )
...
def _resolve_launch_root(output_root: Path, launch_root: Path | None) -> Path:
    ...
    if not isinstance(launch_root_obj, str) or launch_root_obj.strip() == "":
        raise SystemExit("Hemma resource monitor latest-launch metadata was malformed.")
...
def _summary_markdown(summary: HemmaResourceMonitorSummary) -> str:
    """Render one concise markdown summary."""
    lines = [
        "# Hemma Resource Monitor Summary",
        "",
...
def _status_markdown(status: HemmaResourceMonitorStatus) -> str:
...
def _summary_for_launch_root(launch_root: Path) -> HemmaResourceMonitorSummary:
...
def main(argv: list[str] | None = None) -> int:
    """Run the committed Hemma resource monitor CLI."""
