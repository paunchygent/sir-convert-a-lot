"""Run the Chatterbox segmented experiment Chatterbox segmented experiment from the local repo.

Purpose:
    Invoke the committed Hemma-side Chatterbox segmented experiment experiment through the canonical
    `run-hemma` wrapper and mirror the resulting evidence bundle back into the
    local repo copy.

Relationships:
    - Calls `benchmark:chatterbox-segmented-hemma` remotely in the Hemma repo clone.
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse CLI arguments for the local Chatterbox segmented experiment orchestrator."""
    parser = argparse.ArgumentParser(
        description="Run the Chatterbox segmented experiment experiment on Hemma."
    )
...
def main(argv: list[str] | None = None) -> int:
    """Run the remote Chatterbox segmented experiment experiment and sync the evidence locally."""
    ...
    remote_command = [
        ...
        "pdm",
        "run",
        "benchmark:chatterbox-segmented-hemma",
        "--output-root",
    ...
    LOGGER.info("Running Chatterbox segmented experiment remotely on Hemma")
    ...
    LOGGER.info("Syncing Chatterbox segmented experiment evidence bundle back to the local repo")
