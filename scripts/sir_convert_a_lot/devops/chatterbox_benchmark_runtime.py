"""Runtime helpers for the Chatterbox benchmark Hemma Chatterbox benchmark.

Purpose:
    Keep Docker, cache, readiness, GPU sampling, and synthesis-probe logic
    separate from the Chatterbox benchmark orchestration script.

Relationships:
    - Consumed by `run_chatterbox_hemma_benchmark`.
...
from scripts.sir_convert_a_lot.devops.chatterbox_benchmark_reporting import ProbeResult
...
class BenchmarkSettings:
    """Normalized CLI settings for the Chatterbox benchmark run."""
...
def ensure_image_present(settings: BenchmarkSettings) -> tuple[bool, str]:
    """Build the Chatterbox benchmark sidecar image with BuildKit and return the image id."""
    image_present = True
    try:
    ...
    if build_performed:
        ...
        docker_checked(
            ...
            label="docker buildx build chatterbox image",
        )
        image_id = docker_checked(
            ["image", "inspect", settings.image, "--format", "{{.Id}}"],
...
def start_sidecar(settings: BenchmarkSettings, *, hf_mount: MountResolution) -> None:
    """Launch the Chatterbox benchmark sidecar container on the internal Hemma Docker network."""
    command = [
        "run",
        "-d",
        "--name",
    ...
    docker_checked(command, label="docker run chatterbox sidecar")
    ...
    with httpx.Client(timeout=30.0) as client:
        while time.monotonic() < deadline:
            try:
                ...
                time.sleep(3.0)
    raise SystemExit(
        f"Timed out waiting for Chatterbox benchmark sidecar readiness after "
...
def probe_from_service_container(settings: BenchmarkSettings) -> tuple[bool, str, bool]:
    ...
    probe_output = docker_checked(
        ...
        label="docker exec service-container chatterbox health probe",
    )
...
def write_runtime_versions(settings: BenchmarkSettings, *, output_path: Path) -> None:
    ...
    output = docker_checked(
        ...
        label="docker exec chatterbox package versions",
    )
...
def synthesize_probe(
    *,
    ...
    if not response.is_success:
        raise SystemExit(
            "Chatterbox benchmark synthesis failed.\n"
...
def restart_sidecar_and_measure(settings: BenchmarkSettings) -> float:
    ...
    docker_checked(["stop", settings.container_name], label="docker stop chatterbox")
    docker_checked(["start", settings.container_name], label="docker start chatterbox")
