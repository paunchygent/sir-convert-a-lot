"""Runtime helpers for the Hemma OpenVoice sidecar benchmark.

Purpose:
    Keep the OpenVoice benchmark runner small by isolating Docker, cache,
    readiness, runtime-inspection, and synthesis-probe behavior needed to
    evaluate the OpenVoice V2 adapter on Hemma.

Relationships:
    - Used by `scripts.sir_convert_a_lot.devops.run_openvoice_hemma_benchmark`.
...
CONTAINER_DEBUG_ARTIFACT_DIR = "/tmp/openvoice-debug-artifacts"


@dataclass(frozen=True)
class BenchmarkSettings:
    """Normalized CLI settings for the OpenVoice benchmark run."""
    ...
    try:
        docker_checked(
            [
                ...
                (
                    "from pathlib import Path; "
                    "probe = Path('/cache-probe/.openvoice_probe'); "
                    "probe.write_text('ok', encoding='utf-8'); "
                    "print(probe.read_text(encoding='utf-8')); "
                    "probe.unlink()"
                ),
            ],
            label="docker run openvoice cache probe",
        )
    except SystemExit:
        return False
    return True
...
def resolve_effective_cache_dir(
    *, cache_dir: Path, home_mount: Path, image: str
    ...
    raise SystemExit(
        f"OpenVoice benchmark could not establish a Docker-mountable cache path for {cache_dir}."
    )
...
def ensure_sidecar_preconditions(settings: BenchmarkSettings) -> None:
    ...
    if not settings.dockerfile_path.resolve().exists():
        raise SystemExit(f"OpenVoice benchmark Dockerfile is missing: {settings.dockerfile_path}")
    if not settings.reference_audio_path.exists():
        raise SystemExit(
            f"OpenVoice benchmark reference audio is missing: {settings.reference_audio_path}"
        )
...
def remove_existing_benchmark_container(container_name: str) -> None:
    """Remove a stale benchmark container before a fresh OpenVoice benchmark run."""
...
def ensure_image_present(settings: BenchmarkSettings) -> tuple[bool, str]:
    """Build the OpenVoice benchmark sidecar image when requested and return the image id."""
    image_present = True
    try:
    ...
    if build_performed:
        docker_checked(
            ...
            label="docker buildx build openvoice image",
        )
        image_id = docker_checked(
            ["image", "inspect", settings.image, "--format", "{{.Id}}"],
...
def prefetch_openvoice_assets(settings: BenchmarkSettings, mount: MountResolution) -> None:
    ...
    if not archive_path.exists():
        request = urllib.request.Request(
            settings.openvoice_checkpoint_url, headers={"User-Agent": "sir-convert-a-lot-openvoice"}
        )
        with (
    ...
    if not converter_checkpoint.exists() or not converter_config.exists():
        raise SystemExit("OpenVoice checkpoint extraction did not materialize converter assets.")
...
def prefetch_vad_assets(settings: BenchmarkSettings, mount: MountResolution) -> None:
    ...
    docker_checked(
        ...
        label="docker run openvoice vad prefetch",
    )
...
def start_sidecar(
    ...
    *,
    ...
    """Launch the OpenVoice benchmark sidecar container on the internal Hemma Docker network."""
    ...
    docker_checked(run_args, label="docker run openvoice sidecar")
    ...
    with httpx.Client(timeout=30.0) as client:
        while time.monotonic() < deadline:
            try:
                ...
                time.sleep(3.0)
    raise SystemExit(
        f"Timed out waiting for OpenVoice benchmark sidecar readiness after "
...
def inspect_runtime(settings: BenchmarkSettings, *, image_id: str) -> SidecarRuntime:
    ...
    if not isinstance(payload_obj, dict):
        raise SystemExit(
            "OpenVoice benchmark sidecar metadata probe returned an unexpected payload."
        )
    ...
    if not isinstance(python_version_obj, str) or not isinstance(package_versions_obj, dict):
        raise SystemExit("OpenVoice benchmark sidecar metadata probe payload is malformed.")
...
def probe_from_service_container(settings: BenchmarkSettings) -> InternalProbeEvidence:
    ...
    probe_output = docker_checked(
        ...
        label="docker exec service-container openvoice health probe",
    )
    payload_obj = json.loads(probe_output)
    if not isinstance(payload_obj, dict):
        raise SystemExit("OpenVoice benchmark service-container probe payload is malformed.")
    ...
    if not isinstance(backend_id_obj, str) or not isinstance(ready_obj, bool):
        raise SystemExit(
            "OpenVoice benchmark service-container probe did not return normalized health fields."
        )
...
def copy_debug_artifacts_from_container(*, container_name: str, artifacts_dir: Path) -> None:
    ...
    for child in sorted(artifacts_dir.iterdir()):
        ...
        if child.is_dir():
            ...
            continue
        child.unlink()
    with TemporaryDirectory(prefix="openvoice-debug-copy-", dir="/tmp") as temp_dir_raw:
        ...
        if result.returncode != 0:
            ...
            if "no such file or directory" in stdout or "cannot open" in stdout:
                return
            raise SystemExit(
                "OpenVoice benchmark could not stream debug artifacts from the sidecar container.\n"
...
def _extract_debug_artifact_archive(*, archive: tarfile.TarFile, destination_dir: Path) -> None:
    ...
    for member in archive.getmembers():
        ...
        if destination_root not in target_path.parents and target_path != destination_root:
            raise SystemExit(
                f"OpenVoice benchmark received an unsafe debug-artifact path: {member.name}"
            )
