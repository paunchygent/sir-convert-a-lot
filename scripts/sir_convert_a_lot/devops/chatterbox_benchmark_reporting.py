"""Reporting helpers for the Chatterbox benchmark Hemma Chatterbox benchmark.

Purpose:
    Keep Chatterbox benchmark evidence models and markdown rendering separate from the
    orchestration script so the benchmark runner stays small and discoverable.

Relationships:
    - Consumed by `run_chatterbox_hemma_benchmark`.
    - Paired with the runtime helpers in `chatterbox_benchmark_runtime`.
"""
...
class BenchmarkReport:
    """Top-level JSON payload for Chatterbox benchmark Hemma evidence."""
...
def build_report_markdown(report: BenchmarkReport) -> str:
    """Render one operator-friendly markdown summary for Chatterbox benchmark evidence."""
    lines = [
        "# Chatterbox benchmark Hemma Chatterbox Benchmark",
        "",
