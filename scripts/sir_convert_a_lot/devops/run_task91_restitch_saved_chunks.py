"""Re-stitch saved Task 91 Chatterbox chunks without rerunning model inference.

Purpose:
    Reuse the already generated raw Task 91 chunk WAV files and produce a new
...
Relationships:
    - Consumes the `segment-debug/` artifacts emitted by Task 91.
...
DEFAULT_SOURCE_DEBUG_DIR = Path(
    "build/verification/task-91-chatterbox-speech-aware-stitching-hemma/speech_aware/segment-debug"
)
DEFAULT_OUTPUT_ROOT = Path(
    "build/verification/task-91-chatterbox-speech-aware-stitching-hemma/speech_aware_relaxed_fade"
)
...
class SavedSegmentPlan:
    """Typed representation of one saved Task 91 segment plan."""
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse CLI arguments for the saved-chunk Task 91 restitcher."""
    parser = argparse.ArgumentParser(description="Re-stitch saved Task 91 chunks locally.")
...
def _load_segment_plan(source_debug_dir: Path) -> SavedSegmentPlan:
    """Load the original segment plan emitted by Task 91."""
...
def main(argv: list[str] | None = None) -> int:
    """Re-stitch saved Task 91 chunks and write a new local evidence bundle."""
