"""Run the Task 87 Chatterbox tuning sweep against Hemma.
...
Relationships:
    - Invokes `benchmark:task-86` remotely through the canonical
      `pdm run run-hemma -- ...` wrapper.
    - Uses the Task 86 evidence shape as the per-lane source of truth.
    - Writes the Task 87 sweep summary under `build/verification/`.
"""
...
DEFAULT_SWEEP_OUTPUT_ROOT = Path("build/verification/task-87-chatterbox-tuning-sweep")
...
@dataclass(frozen=True)
class SweepSettings:
    """Normalized CLI settings for the Task 87 tuning sweep."""
...
@dataclass(frozen=True)
class LaneResult:
    """One summarized lane outcome for the Task 87 report."""
    ...
    for exaggeration, cfg_weight in combos:
        ...
        lanes.append(
            SweepLane(
                ...
                output_root=Path(f"build/verification/task-87-chatterbox-{slug}"),
            )
        )
    return lanes
...
def _parse_args(argv: list[str]) -> SweepSettings:
    """Parse CLI arguments into normalized Task 87 settings."""
    parser = argparse.ArgumentParser(description="Run the Task 87 Chatterbox tuning sweep.")
...
def _run_lane(settings: SweepSettings, lane: SweepLane) -> int:
    """Run one Task 86 lane remotely on Hemma."""
    command = [
        "pdm",
        "run",
        "run-hemma",
...
def _load_lane_result(lane: SweepLane) -> LaneResult:
    """Load one local Task 86 report into the Task 87 summary shape."""
...
def _write_summary(*, output_root: Path, settings: SweepSettings, lanes: list[LaneResult]) -> None:
    """Write one deterministic Task 87 summary bundle."""
    output_root.mkdir(parents=True, exist_ok=True)
    summary_json = output_root / "report.json"
    summary_md = output_root / "report.md"
    payload = {
        "benchmark_id": "task-87-chatterbox-tuning-sweep",
    ...
    markdown_lines = [
        "# Task 87 Chatterbox Tuning Sweep",
        "",
...
def main(argv: list[str] | None = None) -> int:
    """Run the complete Task 87 sweep and mirror the evidence locally."""
    ...
    if failures:
        LOGGER.error(
            "Task 87 completed with failing lanes: %s", ", ".join(lane.slug for lane in failures)
        )
        return 1
    LOGGER.info("Task 87 completed successfully for %s lanes", len(lane_results))
    return 0
