"""Run PaddleOCR formula recognition for formula candidate evaluation replay.
...
Relationships:
    - Invoked by `formula_candidate_eval` through its Paddle command
      template.
    - Uses the official PaddleOCR formula-recognition Python API.
    - Writes PaddleOCR JSON/image artifacts into the formula candidate evaluation evidence bundle.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    metadata = {
        "schema_version": "paddle_formula_command_v1",
    ...
    (output_dir / "deepseek-ocr2-paddle-metadata.json").write_text(
