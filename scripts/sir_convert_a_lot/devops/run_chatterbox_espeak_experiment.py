"""Run the Chatterbox eSpeak experiment Chatterbox eSpeak experiment from the local repo.

Purpose:
    Invoke the committed Hemma-side Chatterbox eSpeak experiment experiment runner through the
    canonical `run-hemma` wrapper and mirror the resulting evidence bundle back
    into the local repo copy.

Relationships:
    - Calls `benchmark:chatterbox-espeak-hemma` remotely in the Hemma repo clone.
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse CLI arguments for the local Chatterbox eSpeak experiment orchestrator."""
    parser = argparse.ArgumentParser(
        description="Run the Chatterbox eSpeak experiment eSpeak experiment on Hemma."
    )
...
def main(argv: list[str] | None = None) -> int:
    """Run the remote Chatterbox eSpeak experiment experiment and sync the evidence locally."""
    ...
    LOGGER.info("Running Chatterbox eSpeak experiment remotely on Hemma")
    ...
    LOGGER.info("Syncing Chatterbox eSpeak experiment evidence bundle back to the local repo")
