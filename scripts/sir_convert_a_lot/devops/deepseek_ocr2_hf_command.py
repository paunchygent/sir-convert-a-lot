...
Purpose:
    Provide a minimal control lane for specialist OCR command adapter that runs the official
    DeepSeek-OCR-2 `AutoModel.infer` path against one rendered page image.

Relationships:
    - Complements the specialist OCR command adapter vLLM adapter by separating model/input quality
...
from scripts.sir_convert_a_lot.devops.deepseek_ocr2_vllm_batch import (
    CONTAINER_CACHE,
...
CONTAINER_INPUT_PATH = Path("/deepseek-ocr2/input.png")
CONTAINER_OUTPUT_DIR = Path("/deepseek-ocr2/output")
CONTAINER_REPO_DIR = Path("/deepseek-ocr-2")
CONTAINER_RUNNER_PATH = Path("/deepseek-ocr2/output/deepseek-ocr2_deepseek_ocr2_hf_inner.py")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    (output_dir / "deepseek-ocr2_deepseek_ocr2_hf_inner.py").write_text(
        render_inner_runner(),
        encoding="utf-8",
    )
    ...
    metadata = {
        "schema_version": "deepseek-ocr2_deepseek_ocr2_hf_host_v1",
    ...
    (output_dir / "deepseek-ocr2-hf-host-metadata.json").write_text(
...
def main() -> int:
    ...
    metadata = {
        "schema_version": "deepseek-ocr2_deepseek_ocr2_hf_inner_v1",
        "created_at_utc": datetime.now(UTC).isoformat(),
    ...
    (args.output_dir / "deepseek-ocr2-hf-inner-metadata.json").write_text(
        json.dumps(metadata, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    return 0
