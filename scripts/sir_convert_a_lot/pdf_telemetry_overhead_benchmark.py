"""Benchmark v2 telemetry overhead and saturation evidence for PDF throughput lane.
...
Purpose:
    ...
    telemetry-call bypass modes to produce telemetry overhead benchmark overhead evidence.
...
DEFAULT_OUTPUT_JSON = Path(
    "build/benchmarks/pdf-throughput/pdf-telemetry-overhead-telemetry-overhead-local.json"
)
DEFAULT_DATA_ROOT = Path("build/benchmarks/pdf-throughput/pdf-telemetry-overhead-telemetry-runtime")
    ...
    *,
    ...
    def _stub_execute(**_kwargs: object) -> V2ExecutionResult:
        ...
        return V2ExecutionResult(
            ...
            options_fingerprint="pdf-telemetry-overhead-benchmark",
            ocr_enabled=None,
    ...
    try:
        ...
        for index in range(total_jobs):
            created = runtime.create_job(
                spec=_job_spec(filename=f"pdf-telemetry-overhead-{index:03d}.md"),
...
def run_benchmark(
    *,
    ...
    payload: BenchmarkPayload = {
        "benchmark_id": "pdf-telemetry-overhead-telemetry-overhead",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run telemetry overhead benchmark telemetry overhead benchmark."
    )
