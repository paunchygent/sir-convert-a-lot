"""Run the bounded RixVox train staging surface on Hemma.
...
def _parse_args(argv: list[str] | None) -> RixvoxTrainStagingSettings:
    ...
    return RixvoxTrainStagingSettings(
...
def _build_report_markdown(report: RixvoxTrainStagingReport) -> str:
    """Render one concise markdown summary for RixVox train staging."""
    staged_lines = "\n".join(
        f"- `{record.filename}` -> `{record.staged_path}`" for record in report.downloaded_files
    )
    return (
        "# Qwen RixVox Train Staging Report\n\n"
...
def _render_stdout_summary(report: RixvoxTrainStagingReport) -> str:
    """Render one stable stdout summary for the completed staging run."""
    return json.dumps(asdict(report), indent=2, ensure_ascii=False, sort_keys=True)


def main(argv: list[str] | None = None) -> int:
    """Run the bounded staging pass and emit deterministic evidence."""
    settings = _parse_args(argv)
    report_json_path = settings.output_root / "report.json"
    report_md_path = settings.output_root / "report.md"
    enforce_generated_output_path(report_json_path, label="rixvox_train_staging_report_json")
    enforce_generated_output_path(report_md_path, label="rixvox_train_staging_report_md")
