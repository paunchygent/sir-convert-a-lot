"""Launch and inspect detached Story 29 fallback standalone eval on Hemma.

Purpose:
    Provide the committed detached execution surface for the Story 29 fallback
    standalone eval so Hemma proof evidence survives the local client session.

Relationships:
    - Wraps `ml.qwen.training.story29_fallback_eval_detached`.
    - Used by the shared Story 29 proof wrapper during fallback close-out.
"""
...
from scripts.sir_convert_a_lot.ml.qwen.training.story29_fallback_eval_detached import (
    ...
    inspect_detached_story29_eval,
    launch_detached_story29_eval,
)


def _build_parser() -> argparse.ArgumentParser:
    """Build the committed CLI parser for detached Story 29 eval workflows."""
    parser = argparse.ArgumentParser(
        description="Launch and inspect detached Story 29 fallback standalone eval on Hemma."
    )
...
def _status_markdown(status_payload: dict[str, object]) -> str:
    """Render one concise Markdown summary for detached Story 29 eval status."""
    lines = [
        "# Detached Story 29 Fallback Eval Status",
        "",
...
def _load_launch(output_root: Path) -> DetachedStory29EvalLaunch:
    """Load one persisted detached Story 29 eval launch payload."""
    payload = json.loads(_launch_metadata_path(output_root).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise SystemExit("Detached Story 29 eval launch metadata was malformed.")
...
def _required_str(payload: dict[str, object], key: str) -> str:
    ...
    if not isinstance(value, str):
        raise SystemExit(f"Detached Story 29 eval metadata returned malformed `{key}`.")
    return value
...
def main(argv: list[str] | None = None) -> int:
    """Launch or inspect the detached Story 29 fallback eval worker on Hemma."""
    ...
    if args.command == "launch":
        ...
        launch = launch_detached_story29_eval(
    ...
    if args.command == "status":
        ...
        status = inspect_detached_story29_eval(launch)
    ...
    raise SystemExit(f"Unsupported detached Story 29 eval command: {args.command}")
