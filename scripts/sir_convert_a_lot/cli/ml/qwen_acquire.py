"""Run the canonical Hemma-hosted Qwen corpus acquisition surface.
...
Relationships:
    - Intended to run on Hemma via the canonical preprocessing CLI surface.
    - Uses `ml.qwen.preprocessing.acquisition`.
...
def _build_report_markdown(report: AcquisitionReport) -> str:
    """Render one concise markdown summary for acquisition."""
    dataset_lines = "\n".join(
        f"- `{dataset}`: `{count}` files @ `{report.dataset_revisions[dataset]}`"
        for dataset, count in sorted(report.counts_by_dataset.items())
    )
    return (
        "# Qwen Corpus Acquisition Report\n\n"
...
def _render_stdout_summary(report: AcquisitionReport) -> str:
...
def main(argv: list[str] | None = None) -> int:
    """Run the acquisition pass and emit deterministic evidence."""
    settings = _parse_args(argv)
    report_json_path = settings.output_root / "report.json"
    report_md_path = settings.output_root / "report.md"
    enforce_generated_output_path(report_json_path, label="acquisition_report_json")
    enforce_generated_output_path(report_md_path, label="acquisition_report_md")
