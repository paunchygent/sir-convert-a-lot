"""HTTP profile execution for Task 74 throughput benchmarks.

Purpose:
    Execute one Story 20/Task 74 profile through the in-process v2 HTTP app,
    collect job metadata, metrics peaks, latency summaries, and OCR/backend
    result metadata.

Relationships:
    - Used by `story20_profile_runner` for per-profile execution.
    - Uses `story20_profiles.ProfileSpec` for the governed benchmark profile
      settings.
