"""PDF throughput benchmark throughput benchmark payload assembly.

Purpose:
    Orchestrate PDF throughput lane/PDF throughput benchmark profile execution, merge Hemma deploy
    verification runtime
    parity evidence, add optional dirty PDF OCR corpus dirty-corpus summaries, and write
...
Relationships:
    - Uses `pdf_throughput_http_profile` for in-process HTTP profile execution.
    - Uses `pdf_throughput_corpus` for deterministic synthetic smoke corpus creation
...
from .pdf_ocr_runtime_preflight import assert_in_process_runtime_supports_requested_ocr
from .pdf_runtime_parity import RuntimeParityInputs, build_runtime_parity_summary
from .pdf_throughput_corpus import build_verified_dirty_corpus, generate_corpus
from .pdf_throughput_http_profile import run_profile as run_in_process_profile
from .pdf_throughput_profiles import (
...
from .pdf_throughput_report import write_report
from .pdf_throughput_service_profile import run_profile as run_service_profile
...
def _profile_payloads_for_runtime(
    *,
    ...
    raise ValueError(f"Unsupported PDF throughput benchmark runtime mode `{runtime_mode}`.")
...
def _build_payload(
    *,
    ...
    return {
        "benchmark_id": "pdf-throughput-throughput-benchmark",
...
def run_benchmark(
    *,
    ...
    """Run the PDF throughput benchmark throughput benchmark and return the payload."""
