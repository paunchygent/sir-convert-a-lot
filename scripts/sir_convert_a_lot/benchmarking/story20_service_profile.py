"""Production-service profile execution for Task 74 Hemma benchmarks.

Purpose:
    Execute one Task 74 benchmark profile through the deployed Sir Convert-a-Lot
    v2 HTTP service instead of an in-process FastAPI `TestClient`.

Relationships:
    - Used by `story20_profile_runner` when `runtime_mode=production_service`.
    - Shares payload helpers with `story20_http_profile` so service-backed and
      in-process smoke evidence keep the same report schema.
    - Keeps Task 271 final proof tied to the production service lane on Hemma.
"""
...
from .story20_http_profile import (
...
from .story20_profiles import ProfileSpec
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if status in {JobStatus.SUCCEEDED, JobStatus.FAILED, JobStatus.CANCELED}:
            return payload
        time.sleep(0.2)
    raise RuntimeError(f"Task 74 service benchmark polling timed out for {job_id}.")
...
def _read_result_metadata(
    *,
    ...
    if not isinstance(result_obj, dict):
        raise RuntimeError(f"Task 74 result payload missing result object for {job_id}.")
    metadata_obj = result_obj.get("conversion_metadata")
    if not isinstance(metadata_obj, dict):
        raise RuntimeError(f"Task 74 result payload missing conversion metadata for {job_id}.")
    ...
    with httpx.Client(base_url=service_url, timeout=timeout) as client:
        for index, corpus_record in enumerate(corpus_records, start=1):
            ...
            create_response = client.post(
                ...
                headers={
                    "X-API-Key": api_key,
                    "Idempotency-Key": f"task74-service-{profile.profile_name}-{index:03d}",
                    "X-Correlation-ID": f"corr_task74_service_{profile.profile_name}_{index:03d}",
                },
                files={
