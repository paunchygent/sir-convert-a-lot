"""OCR runtime preflight for PDF throughput benchmark local smoke execution.

Purpose:
    Fail fast when the local in-process PDF throughput lane/PDF throughput benchmark command-surface
    smoke
    cannot satisfy the requested OCR engine/model configuration. Acceptance
    benchmark evidence for PDF throughput benchmark/dirty PDF OCR benchmark remains Hemma-only.

Relationships:
    - Used by `pdf_throughput_profile_runner` before launching local in-process smoke
      jobs.
...
def assert_in_process_runtime_supports_requested_ocr(
    *,
    ...
    if importlib.util.find_spec("easyocr") is None:
        raise RuntimeError(
            "PDF throughput benchmark local in-process smoke runtime is missing EasyOCR. "
            "Run `pdm sync` in the benchmark environment or use the canonical "
            "`benchmark:pdf-throughput-hemma` workflow before benchmarking."
        )
    ...
    if not model_dir.exists():
        raise RuntimeError(
            "PDF throughput benchmark local smoke is missing the EasyOCR model directory "
