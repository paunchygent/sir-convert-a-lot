"""Command-line interface for the PDF throughput benchmark throughput benchmark.

Purpose:
    Parse PDF throughput benchmark arguments, support metadata-only dirty-corpus
...
Relationships:
    - Called by the public `pdf_throughput_benchmark_report` module.
    - Uses `pdf_throughput_profile_runner.run_benchmark` for benchmark execution.
...
from .pdf_runtime_parity import RuntimeParityInputs, coerce_optional_str
from .pdf_throughput_profile_runner import DEFAULT_PAGE_COUNTS, run_benchmark
...
DEFAULT_OUTPUT_ROOT = Path("build/benchmarks/pdf-throughput/pdf-throughput-throughput")
DEFAULT_OUTPUT_JSON = DEFAULT_OUTPUT_ROOT / "pdf-throughput-throughput-benchmark-local.json"
DEFAULT_OUTPUT_REPORT = DEFAULT_OUTPUT_ROOT / "pdf-throughput-throughput-report.md"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run PDF throughput lane PDF throughput benchmark throughput benchmark."
    )
    ...
    parser.add_argument("--deploy-parity-report-json", type=Path)
...
def main() -> None:
    """Parse CLI args and run the PDF throughput benchmark harness."""
    ...
    payload = run_benchmark(
        ...
        runtime_parity_inputs=RuntimeParityInputs(
            report_json_path=args.deploy_parity_report_json,
