"""Task 74 benchmark profile policy and safety checks.

Purpose:
    Define the Story 20/Task 74 benchmark profile matrix, bounded two-worker
    sweep variants, and dirty-corpus fail-closed safety checks.

Relationships:
    - Used by the Task 74 benchmark runner and CLI.
    - Encodes the Story 39/Task 270 requirement that dirty-corpus evidence stays
      within the Task 74 safe 2-worker profile boundary.
"""
...
def default_profiles() -> list[ProfileSpec]:
    """Return the committed Task 74 baseline profile matrix."""
    return [
        ProfileSpec(
...
def build_two_worker_sweep_profiles(
    *,
    ...
    """Build bounded two-worker sweep profiles inside the Task 74 safe matrix."""
