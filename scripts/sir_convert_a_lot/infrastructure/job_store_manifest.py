"""Manifest composition/parsing helpers for the filesystem job store.

Purpose:
    Keep manifest serialization and diagnostics parsing isolated from job-store
    state-transition mechanics, so persistence and transition logic stay SRP-aligned.

Relationships:
    - Used by `infrastructure.job_store` for manifest creation and loading.
    - Produces `StoredJobRecord` defined in `infrastructure.job_store_models`.
"""
...
from scripts.sir_convert_a_lot.infrastructure.job_store_models import StoredJobRecord
...
def build_initial_manifest(
    *,
    ...
    """Build initial on-disk manifest structure for a newly created job."""
    return {
        "job_id": job_id,
...
def parse_stored_job_record(
    *,
    ...
    """Parse one manifest payload into a typed durable job record."""
