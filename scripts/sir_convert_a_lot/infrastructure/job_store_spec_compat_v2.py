"""Stored v2 job-spec compatibility normalization.

Purpose:
    Normalize legacy on-disk v2 job manifests before strict domain validation,
...
_LEGACY_CONVERSION_KEYS: frozenset[str] = frozenset({"input_trust_mode"})
...
def normalize_stored_job_spec_payload_v2(spec_payload: Mapping[str, object]) -> dict[str, object]:
    ...
    if isinstance(conversion, Mapping):
        ...
        for key in _LEGACY_CONVERSION_KEYS:
