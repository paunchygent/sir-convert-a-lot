"""Tests for the Task 100 Qwen fine-tuning smoke surface."""
...
from scripts.sir_convert_a_lot.devops.run_task100_hemma_qwen_finetune_smoke import (
...
def test_task100_parse_args_defaults() -> None:
    """The Task 100 smoke runner should expose deterministic defaults."""
...
def test_task100_build_smoke_probe_command_uses_rocm_and_cache_mounts() -> None:
...
def test_task100_run_smoke_probe_parses_runtime_fields(monkeypatch: pytest.MonkeyPatch) -> None:
    """The Task 100 smoke probe parser should keep runtime metadata typed."""
    settings = Task100SmokeSettings(
        output_root=Path("build/verification/task-100-qwen-finetune-smoke"),
        dockerfile_path=Path("containers/qwen-finetune-hemma/Dockerfile"),
        image="sir-convert-a-lot-qwen-finetune-hemma:task100",
