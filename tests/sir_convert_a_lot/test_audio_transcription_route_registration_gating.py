...
AUDIO_ROUTE_STORY_PATH = backlog_document_path(
    category="stories",
    title_slug="audio-transcript-bundle-route-execution-and-json-artifact-persistence",
)
PROFILE_REJECTION_REVIEW_PATH = backlog_document_path(
    category="reviews",
...
def test_audio_transcript_route_block_is_recorded_without_runtime_completion() -> None:
    ...
    assert "production profile rejection" in story_text
    ...
    assert "does not authorize" in review_text
    assert "route registration" in review_text
