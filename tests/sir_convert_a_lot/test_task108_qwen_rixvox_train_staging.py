"""Tests for the Task 108 bounded RixVox train staging surface."""
...
from scripts.sir_convert_a_lot.devops.run_task108_hemma_qwen_rixvox_train_staging import (
...
def test_task108_parse_args_defaults() -> None:
    """The Task 108 runner should expose deterministic default shards."""
...
def test_run_task108_rixvox_train_staging_downloads_metadata_and_shards(
    tmp_path: Path,
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task108_qwen_rixvox_train_staging_runtime.ensure_data_disk_path",
        lambda path, label: None,
    )
    monkeypatch.setattr(Path, "mkdir", lambda self, parents=True, exist_ok=True: None)
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task108_qwen_rixvox_train_staging_runtime.resolve_dataset_revision",
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task108_qwen_rixvox_train_staging_runtime.download_file_with_retry",
        _fake_download_file_with_retry,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task108_qwen_rixvox_train_staging_runtime.stage_downloaded_file",
...
def test_task108_runner_main_writes_report(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.run_task108_hemma_qwen_rixvox_train_staging.run_task108_rixvox_train_staging",
