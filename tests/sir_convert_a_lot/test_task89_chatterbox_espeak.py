"""Tests for the Task 89 Chatterbox eSpeak experiment surfaces.
...
Relationships:
    - Exercises the Task 89 helper container entry module.
    - Exercises the Task 89 local and Hemma runners.
"""
...
from scripts.sir_convert_a_lot.devops import (
    run_task89_chatterbox_espeak_experiment,
    run_task89_hemma_chatterbox_espeak_experiment,
)
...
def test_task89_hemma_parse_args_defaults() -> None:
    settings = run_task89_hemma_chatterbox_espeak_experiment._parse_args([])

    assert settings.helper_image == "sir-convert-a-lot/textprep-espeak-task89:local"
...
def test_task89_local_orchestrator_runs_remote_then_rsync(
    ...
    def _fake_run(command: list[str], cwd: Path, check: bool) -> _Completed:
        assert cwd == run_task89_chatterbox_espeak_experiment.REPO_ROOT
        assert check is False
        commands.append(command)
        return _Completed()

    monkeypatch.setattr(run_task89_chatterbox_espeak_experiment.subprocess, "run", _fake_run)

    result = run_task89_chatterbox_espeak_experiment.main([])
    ...
    assert "task-89-chatterbox-espeak-hemma" in commands[1][2]
    assert "--probe-text" in commands[0]
    assert "--espeak-language" in commands[0]


def test_task89_hemma_run_task86_lane_passes_probe_text_file(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    calls: list[list[str]] = []
    ...
    monkeypatch.setattr(
        run_task89_hemma_chatterbox_espeak_experiment.run_task86_hemma_chatterbox_benchmark,
        "main",
        _fake_main,
    )
    ...
    result = run_task89_hemma_chatterbox_espeak_experiment._run_task86_lane(
...
def test_task89_hemma_load_lane_summary_handles_missing_report(tmp_path: Path) -> None:
    summary = run_task89_hemma_chatterbox_espeak_experiment._load_lane_summary(
        "baseline",
