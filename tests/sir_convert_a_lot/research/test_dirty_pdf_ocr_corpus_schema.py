"""Tests for the dirty PDF OCR corpus dirty PDF OCR corpus schema.

Purpose:
    Validate metadata-only dirty PDF OCR corpus manifests, sanitized PDF throughput benchmark
...
Relationships:
    ...
    - Exercises the PDF throughput benchmark harness extension in
...
def _stub_execute_v2_job_conversion(*, config, **_kwargs):
    ...
    return V2ExecutionResult(
        ...
        options_fingerprint="sha256:dirty-pdf-ocr-corpus-stub",
        ocr_enabled=True,
...
def _dirty_manifest_json(
    *,
    ...
    return (
        ...
        '"corpus_id":"dirty-pdf-ocr-corpus-dirty-corpus-test",'
        '"entries":['
        "{"
...
def test_dirty_corpus_manifest_command_prints_sanitized_summary(
    ...
    assert "dirty-pdf-ocr-corpus-dirty-corpus-test" in output
...
def test_run_benchmark_embeds_verified_dirty_corpus_report_extension(
    ...
    assert dirty_corpus["deploy_parity_required"] is True
    assert dirty_corpus["deploy_parity_proven"] is True
    assert dirty_corpus["all_profiles_safe"] is True
    assert dirty_corpus["ocr_metadata_summary"]["ocr_engine_used_values"] == ["easyocr"]
    assert dirty_corpus["dirty_pdf_ocr_proof"]["target_executed_pages"] == 150
    assert dirty_corpus["dirty_pdf_ocr_proof"]["tuned_total_pages"] == 1
    assert dirty_corpus["dirty_pdf_ocr_proof"]["meets_150_page_target"] is False
    output_json_text = (tmp_path / "pdf-throughput.json").read_text(encoding="utf-8")
    report_text = (tmp_path / "pdf-throughput.md").read_text(encoding="utf-8")
    ...
    assert "## dirty PDF OCR final proof Final Proof Target" in report_text
...
def test_proof_rejects_under_150_pages_even_on_production_service(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    page_count: int,
) -> None:
    ...
    payload = run_benchmark(
        output_json=tmp_path / f"pdf-throughput-{page_count}.json",
        output_report=tmp_path / f"pdf-throughput-{page_count}.md",
    ...
    assert dirty_pdf_ocr_proof["production_service_runtime"] is True
    assert dirty_pdf_ocr_proof["source_hashes_verified"] is True
    assert dirty_pdf_ocr_proof["tuned_total_pages"] == page_count
    assert dirty_pdf_ocr_proof["target_executed_pages"] == 150
    assert dirty_pdf_ocr_proof["target_wall_clock_seconds"] == 3600
    assert dirty_pdf_ocr_proof["meets_150_page_target"] is False
