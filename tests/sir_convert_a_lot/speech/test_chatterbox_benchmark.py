"""Tests for the Chatterbox benchmark Hemma Chatterbox benchmark helpers.
...
Relationships:
    - Exercises `run_chatterbox_hemma_benchmark`.
...
from scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime import MountResolution
...
def test_parse_args_prefers_canonical_hemma_cache_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = run_chatterbox_hemma_benchmark._parse_args([])
...
def test_start_sidecar_uses_buildkit_ready_mounts_and_envs(
    ...
    def _fake_docker_checked(args: list[str], *, label: str) -> str:
        assert label == "docker run chatterbox sidecar"
        recorded_commands.append(args)
        return "container-id"

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.chatterbox_benchmark_runtime.docker_checked",
    ...
    settings = BenchmarkSettings(
        ...
        network_alias="chatterbox-sidecar",
