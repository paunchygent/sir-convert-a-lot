"""Tests for the F5-TTS benchmark Hemma F5 benchmark runner.

Purpose:
    Lock the F5-TTS benchmark CLI parsing defaults and report formatting around the
    current Swedish F5-TTS benchmark surface without requiring Hemma or model
    downloads during local test runs.

Relationships:
    - Exercises `scripts.sir_convert_a_lot.devops.run_f5_tts_hemma_smoke`.
...
from scripts.sir_convert_a_lot.devops.run_f5_tts_hemma_smoke import (
    BenchmarkReport,
...
def test_build_report_markdown_lists_new_f5_controls() -> None:
    report = BenchmarkReport(
        ...
        internal_base_url="http://sir-convert-a-lot-f5-f5-tts:8093",
        image="sir-convert-a-lot/f5-sidecar-f5-tts:local",
...
def test_run_synthesis_segments_and_stitches(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.run_f5_tts_hemma_smoke._synthesize_probe",
