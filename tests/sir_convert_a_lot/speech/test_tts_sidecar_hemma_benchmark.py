"""Tests for the Hemma TTS sidecar benchmark helpers.
...
Relationships:
    - Exercises `tts_sidecar_benchmark_runtime`.
    - Exercises `tts_sidecar_benchmark_reporting`.
"""
...
from scripts.sir_convert_a_lot.devops import run_tts_sidecar_hemma_benchmark
from scripts.sir_convert_a_lot.devops.tts_sidecar_benchmark_reporting import (
    AudioProbeResult,
    BenchmarkReport,
    GpuIdentity,
...
from scripts.sir_convert_a_lot.devops.tts_sidecar_benchmark_runtime import (
    CONTAINER_HF_HOME,
...
def _build_settings(
    ...
    *,
    ...
    network_alias: str = "sir-convert-a-lot-tts-tts-sidecar",
    container_name: str = "sir_convert_a_lot_tts_tts-sidecar",
    ...
    """Return one TTS sidecar benchmark settings fixture with sensible defaults."""
    return BenchmarkSettings(
        ...
        stage_config_path=(
            tmp_path / "tts-sidecar-stage-config.yaml"
...
def test_parse_args_prefers_canonical_hemma_cache_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = run_tts_sidecar_hemma_benchmark._parse_args([])
...
def test_resolve_effective_hf_cache_dir_uses_home_bind_mount_when_srv_probe_fails(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.tts_sidecar_benchmark_runtime._probe_docker_bind_mount",
        _fake_probe,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.tts_sidecar_benchmark_runtime._ensure_home_bind_mount",
        _fake_bind_mount,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.tts_sidecar_benchmark_runtime._is_srv_cache_path",
...
def test_prefetch_qwen3_tts_assets_downloads_tokenizer_and_enables_triton(
    ...
    stage_config_path = tmp_path / "tts-sidecar-stage-config.yaml"
    ...
    def _fake_docker_checked(args: list[str], *, label: str) -> str:
        assert label == "docker run tts-sidecar tokenizer prefetch"
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.tts_sidecar_benchmark_runtime.docker_checked",
...
def test_audio_probe_treats_json_success_payload_as_error(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.tts_sidecar_benchmark_runtime._GpuSampler",
        _FakeSampler,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.tts_sidecar_benchmark_runtime.httpx.Client",
        _FakeClient,
    )
...
def test_prepare_output_root_removes_stale_failure_and_artifacts(tmp_path: Path) -> None:
    output_root = tmp_path / "tts-sidecar-output"
    ...
    prepared = run_tts_sidecar_hemma_benchmark._prepare_output_root(output_root)
...
def test_start_sidecar_uses_persistent_hf_cache_contract(
    ...
    def _fake_docker_checked(args: list[str], *, label: str) -> str:
        assert label == "docker run tts-sidecar sidecar"
        recorded_commands.append(args)
        return "container-id"
...
def test_report_helpers_write_expected_json_and_markdown(tmp_path: Path) -> None:
    report = BenchmarkReport(
        benchmark_id="tts-sidecar-hemma",
        generated_at="2026-03-06T12:00:00Z",
        repo_head="abc123",
        host_base_url="http://127.0.0.1:38091",
        internal_base_url="http://sir-convert-a-lot-tts-tts-sidecar:8091",
        ...
        speech_request=SpeechRequestEvidence(
            ...
            language="English",
            voice=None,
            probe_text_path="build/verification/tts-sidecar-hemma/inputs/probe_text.txt",
            instructions_path="build/verification/tts-sidecar-hemma/inputs/instructions.txt",
            reference_audio_path=(
                "build/verification/tts-sidecar-hemma/inputs/reference_audio.wav"
            ),
            reference_audio_sha256="cafebabe",
            reference_audio_duration_seconds=7.9,
            reference_transcript_path=(
                "build/verification/tts-sidecar-hemma/inputs/reference_transcript.txt"
            ),
        ),
        ...
        sidecar_runtime=SidecarRuntime(
            ...
            container_name="sir_convert_a_lot_tts_tts-sidecar",
            python_version="3.12.11",
            package_versions={"vllm": "0.16.0", "vllm-omni": None, "vllm_omni": None},
            stage_config_path="/workspace/tts-sidecar-stage-config.yaml",
        ...
        audio_results=[
            AudioProbeResult(
                ...
                byte_count=4096,
                sha256="deadbeef",
                output_path="build/verification/tts-sidecar-hemma/artifacts/sample.wav",
                elapsed_seconds=2.5,
        ...
        docker_logs_path="build/verification/tts-sidecar-hemma/docker_logs.txt",
    )
    ...
    assert '"benchmark_id": "tts-sidecar-hemma"' in json_path.read_text(encoding="utf-8")
