"""Unit tests for Task 100 Qwen runtime helpers.

Purpose:
    Cover the shared image-build warning and cache-sync helpers that are used by
    the Task 100 smoke lane and the detached Task 101 pilot launcher.

Relationships:
    - Exercises `task100_qwen_finetune_runtime.py`.
    - Complements the Task 100 smoke and Task 101 runner tests.
"""
...
def _settings(*, build_image: bool) -> Task100SmokeSettings:
    """Build one deterministic Task 100 settings object for helper tests."""
    return Task100SmokeSettings(
        output_root=Path("build/verification/task-100-qwen-finetune-smoke"),
...
def test_sync_home_cache_into_data_disk_uses_rsync(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task100_qwen_finetune_runtime.run_checked",
        _fake_run_checked,
    )

    _sync_home_cache_into_data_disk(canonical_dir, home_mount)

    assert captured["label"] == "rsync task100 home cache"
...
def test_inspect_image_build_plan_requires_build_when_image_missing(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task100_qwen_finetune_runtime.docker_checked",
...
def test_prepare_qwen_image_emits_warning_before_build(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task100_qwen_finetune_runtime.inspect_image_build_plan",
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task100_qwen_finetune_runtime.ensure_image_present",
