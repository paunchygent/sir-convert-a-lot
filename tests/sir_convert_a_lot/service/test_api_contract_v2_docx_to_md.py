"""Contract tests for v2 `docx -> md` lifecycle behavior.

Purpose:
    Verify `docx -> md` is accepted on v2 routes and returns deterministic
...
def _wait_for_terminal(
    ...
    while time.monotonic() < deadline:
        response = client.get(
            ...
            headers={"X-API-Key": api_key, "X-Correlation-ID": "corr_docx_md_poll"},
        )
...
def test_docx_to_md_lifecycle_result_and_artifact(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def _stub_executor(**kwargs) -> V2ExecutionResult:
        ...
        return V2ExecutionResult(
            artifact_bytes=b"# Converted from DOCX\n\nBody\n",
            pipeline_used="docx_to_md_v2",
            backend_used="pandoc",
            acceleration_used=None,
            warnings=["normalized_warning"],
            phase_timings_ms={},
            options_fingerprint="docx_md_contract_test",
            ocr_enabled=None,
    ...
    create_response = client.post(
        ...
        headers={
            ...
            "Idempotency-Key": "idem-docx-to-md-lifecycle",
            "X-Correlation-ID": "corr_docx_md_create",
        },
        files={
            "file": (
                "input.docx",
    ...
    result_response = client.get(
        ...
        headers={"X-API-Key": "secret-key", "X-Correlation-ID": "corr_docx_md_result"},
    )
    ...
    assert result_payload["result"]["conversion_metadata"]["pipeline_used"] == "docx_to_md_v2"

    artifact_response = client.get(
        f"/v2/convert/jobs/{job_id}/artifact",
        headers={"X-API-Key": "secret-key", "X-Correlation-ID": "corr_docx_md_artifact"},
    )
    assert artifact_response.status_code == 200
    assert artifact_response.headers["content-type"].startswith("text/markdown")
    assert artifact_response.content == b"# Converted from DOCX\n\nBody\n"
