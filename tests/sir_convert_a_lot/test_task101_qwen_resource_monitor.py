"""Focused tests for Task 101 pilot-scoped resource-monitor helpers.

Purpose:
    Validate launch and phase-window summary behavior for the Task 101 monitor
    integration introduced in Story 26 without requiring a real Hemma host.

Relationships:
    - Exercises `task101_qwen_pilot_resource_monitor.py`.
    - Reuses Task 116 monitor artifact contracts for launch/status compatibility.
"""
...
from scripts.sir_convert_a_lot.devops.task101_qwen_pilot_resource_monitor import (
    ...
    task101_resource_monitor_output_root,
)


def test_launch_task101_resource_monitor_writes_launch_metadata(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Task 101 monitor launch should persist deterministic Task 116 artifacts."""
    task101_launch_root = tmp_path / "task101-20260313t200000z"
    task101_launch_root.mkdir(parents=True, exist_ok=True)

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task101_qwen_pilot_resource_monitor.spawn_detached_worker",
        lambda command, *, stdout_path, stderr_path: 4242,
    )

    payload = launch_task101_resource_monitor(
        task101_launch_id="task101-20260313t200000z",
        task101_launch_root=task101_launch_root,
        runtime_kind="rocm",
        interval_seconds=1.0,
        duration_seconds=None,
    )

    monitor_root = task101_resource_monitor_output_root(task101_launch_root)
    launch_root = monitor_root / "task101-20260313t200000z-resource-monitor"
...
def test_inspect_task101_resource_monitor_splits_train_and_checkpoint_windows(
    tmp_path: Path,
) -> None:
        ...
        json.dumps(
            {
                ...
                "launch_id": "task101-run-resource-monitor",
                "repo_root": "/repo",
                "pid": 999999,
                "runtime_kind": "rocm",
                "interval_seconds": 1.0,
    ...
    payload = inspect_task101_resource_monitor(
        {
            "launch_id": "task101-run-resource-monitor",
