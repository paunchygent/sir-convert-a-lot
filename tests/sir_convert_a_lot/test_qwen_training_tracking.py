"""Focused tests for Qwen training tracking helpers.

Purpose:
    Validate the tracker-config normalization and detached launch tracking
    metadata merge path introduced for `T156` without requiring a real GPU
    training run or live tracker backend.

Relationships:
    - Exercises `sft_12hz_tracking.py` for MLflow/TensorBoard path semantics.
    - Exercises `task101_qwen_pilot_probe_reporting.py` for launch metadata
      tracking updates written by the detached in-container probe.
"""
...
PROBE_REPORTING = importlib.import_module(
    "scripts.sir_convert_a_lot.devops.task101_qwen_pilot_probe_reporting"
)
...
_merge_launch_tracking_metadata = PROBE_REPORTING._merge_launch_tracking_metadata
...
class _FakeAccelerator:
    ...
    def __init__(self, tracker: _FakeMlflowRun) -> None:
...
def test_build_training_tracker_config_defaults_to_run_scoped_paths(tmp_path: Path) -> None:
    ...
    output_model_path = tmp_path / "task101-20260313t120000z" / "checkpoints"
    ...
    assert config.run_name == "task101-20260313t120000z"
...
def test_refresh_training_tracker_summary_uses_project_named_tensorboard_run_dir(
    ...
    assert summary.run_name == "task101-20260313t120000z"
...
def test_merge_launch_tracking_metadata_updates_existing_launch_artifact(tmp_path: Path) -> None:
    """The probe should merge live tracker metadata back into launch.json."""
    launch_metadata_path = tmp_path / "launch.json"
    launch_metadata_path.write_text(
        json.dumps(
            {
                "launch_id": "task101-20260313t120000z",
                "tracking": {
                    "project_name": "task101-qwen-pilot",
