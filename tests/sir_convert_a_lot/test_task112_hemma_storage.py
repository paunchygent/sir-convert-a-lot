"""Tests for Task 112 Hemma storage remediation.
...
Relationships:
    - Covers `task112_hemma_storage_runtime.py`.
    - Covers `run_task112_hemma_storage_remediation.py`.
    - Protects the storage-tier contract introduced by Task 112.
"""
...
from scripts.sir_convert_a_lot.devops import run_task112_hemma_storage_remediation
from scripts.sir_convert_a_lot.devops.task112_hemma_storage_runtime import (
    Task112StorageReport,
    Task112StorageSettings,
...
def _settings(tmp_path: Path) -> Task112StorageSettings:
    """Build one deterministic Task 112 settings object for tests."""
    repo_root = tmp_path / "repo"
    return Task112StorageSettings(
        repo_root=repo_root,
...
def test_migrate_repo_build_to_scratch_moves_and_symlinks(tmp_path: Path) -> None:
    """Task 112 should move the repo build tree onto scratch and symlink it back."""
...
def test_migrate_qwen_data_to_storage_moves_and_symlinks(tmp_path: Path) -> None:
    """Task 112 should move raw corpus data onto storage and symlink it back."""
    settings = _settings(tmp_path)
...
def test_cleanup_non_active_docker_state_prunes_expected_resources(
    ...
    """Task 112 cleanup should prune containers, images, volumes, and builder cache."""
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task112_hemma_storage_runtime.run_checked",
        _fake_run_checked,
    )
    ...
    assert seen == [
        (["sudo", "-n", "docker", "container", "prune", "-f"], "docker container prune task112"),
        (["sudo", "-n", "docker", "image", "prune", "-af"], "docker image prune task112"),
        (["sudo", "-n", "docker", "volume", "prune", "-f"], "docker volume prune task112"),
        (["sudo", "-n", "docker", "builder", "prune", "-af"], "docker builder prune task112"),
    ]


def test_build_storage_report_resolves_symlink_targets(tmp_path: Path) -> None:
    """Task 112 report building should capture symlink targets deterministically."""
    settings = _settings(tmp_path)
...
def test_task112_runner_writes_report_after_remediation(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Task 112 runner should write deterministic report files after remediation."""
    output_root = tmp_path / "build" / "verification" / "task-112"
    expected_report = Task112StorageReport(
    ...
    calls: list[Task112StorageSettings] = []

    def _fake_run(settings: Task112StorageSettings) -> Task112StorageReport:
        calls.append(settings)
        return expected_report

    monkeypatch.setattr(
        run_task112_hemma_storage_remediation,
        "run_task112_storage_remediation",
        _fake_run,
    )

    exit_code = run_task112_hemma_storage_remediation.main(
