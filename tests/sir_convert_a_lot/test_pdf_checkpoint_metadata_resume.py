"""PDF checkpoint metadata regression tests for PDF checkpoint terminal metadata truth.
...
Relationships:
    - Exercises `infrastructure.v2_pdf_checkpointed_executor` through the v2
      conversion executor.
    - Complements the broader parallel PDF throughput parallel/cancel/resume contract suite.
"""
...
def _pdf_checkpoint_metadata_config(tmp_path: Path) -> ServiceConfig:
    return replace(
...
def _build_ocr_job(tmp_path: Path) -> StoredJobV2:
    job = _build_job(
        tmp_path,
        source_filename="pdf-checkpoint-metadata.pdf",
...
def test_zero_new_chunk_resume_hydrates_ocr_metadata_from_checkpoint(
        ...
        *,
        ...
        return (
            ...
            ConversionMetadata(
                ...
                options_fingerprint="sha256:pdf-checkpoint-metadata",
            ),
            warnings,
    ...
    config = _pdf_checkpoint_metadata_config(tmp_path)
