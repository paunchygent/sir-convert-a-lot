...
Purpose:
    Verify the committed Task 242 bind-root install/status/probe helpers and
...
def test_task242_runner_writes_install_status_and_probe_reports(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """The Task 242 runner should write deterministic artifacts for all public commands."""
    output_root = tmp_path / "build" / "verification" / "task-242"
    bind_roots = (
        QwenDockerBindRootState(
            label="build",
    ...
    probe_report = QwenDockerBindRootsProbeReport(
        ...
        service_name="sir-convert-a-lot-qwen-docker-bind-roots.service",
        probe_image="sir-convert-a-lot-qwen-finetune-hemma:task100",
        probe_results=(
            QwenDockerBindProbeResult(
                label="build",
                canonical_root="/srv/scratch/sir-convert-a-lot/build",
