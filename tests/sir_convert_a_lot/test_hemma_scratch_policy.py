...
Purpose:
    Verify the committed Hemma scratch audit scratch audit/remediation helpers and runner
    without touching real Hemma disks or real Docker state.

Relationships:
    - Covers `hemma_scratch_policy_runtime.py`.
    - Covers `run_hemma_scratch_policy.py`.
    - Protects the recurring hot-versus-cold storage policy used by Qwen fallback proof lane.
"""
...
def test_archive_scratch_paths_moves_tree_and_symlinks_back(tmp_path: Path) -> None:
    """Hemma scratch audit should archive explicit scratch trees and leave a symlink behind."""
...
def test_build_scratch_audit_report_ranks_large_consumers(
    ...
    """Hemma scratch audit audit should rank the largest non-symlink consumers deterministically."""
    scratch_root = tmp_path / "scratch"
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.hemma_scratch_policy_runtime._docker_system_df_or_error",
        lambda: "docker ok",
    )
...
def test_directory_size_bytes_falls_back_to_sudo_for_permission_protected_roots(
    ...
    """Hemma scratch audit audit should retry `du` with sudo when plain access is denied."""
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.hemma_scratch_policy_runtime.run_checked",
        fake_run_checked,
    )
...
def test_runner_writes_audit_and_remediation_reports(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Hemma scratch audit runner should write deterministic audit and remediation artifacts."""
    output_root = tmp_path / "build" / "verification" / "hemma-scratch-policy"
    ...
    audit_exit_code = run_hemma_scratch_policy.main(
        ["audit", "--output-root", output_root.as_posix()]
    )
    remediation_exit_code = run_hemma_scratch_policy.main(
    ...
    assert "Hemma Scratch Audit" in (output_root / "audit.md").read_text(encoding="utf-8")
    assert (
    ...
    assert "Hemma Scratch Remediation" in (output_root / "remediate.md").read_text(encoding="utf-8")
