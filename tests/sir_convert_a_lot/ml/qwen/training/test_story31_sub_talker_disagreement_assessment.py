"""Focused tests for the T235 Story 31 disagreement assessment.

Purpose:
    Lock the post-T234 disagreement contract so the strongest output-scale
    member either converges on one corridor surface or stays explicitly
    ambiguous for the next task.

Relationships:
    - Exercises `story31_sub_talker_disagreement_assessment.py`.
...
def test_validate_post_t234_disagreement_contract_rejects_multi_variant_family() -> None:
    """The T235 disagreement probe should accept only the strongest T234 member."""
    settings = Story31StabilityLabSettings(
        ...
        hook_profile=TALKER_CORE_POST_T234_DISAGREEMENT_HOOK_PROFILE,
...
def test_build_post_t234_disagreement_assessment_matches_downstream_family() -> None:
    """All-three downstream agreement should authorize only downstream seam splitting."""
    assessment = build_post_t234_disagreement_assessment(
        settings=_settings(),
    ...
    assert assessment == Story31PostT234DisagreementAssessment(
        ...
        next_micro_family_rule="T236 may split the downstream layer_15.output seam only.",
    )


def test_build_post_t234_disagreement_assessment_matches_non_repeatable_reversion() -> None:
    """All-three reversion to input-layernorm should close the T234 relocation signal."""
    assessment = build_post_t234_disagreement_assessment(
        settings=_settings(),
        matrix_rows=_matrix_rows(
...
def test_build_post_t234_disagreement_assessment_marks_second_row_outlier() -> None:
