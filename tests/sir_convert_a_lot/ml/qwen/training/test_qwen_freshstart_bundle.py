"""
Tests for Qwen backward-lineage and fresh-start proof lane fresh-start mini-bundle materialization.
...
Relationships:
    - Exercises `qwen_freshstart_bundle.py`.
...
def test_materialize_mini_bundle_copies_selected_rows_and_assets(tmp_path: Path) -> None:
    ...
    payload = materialize_mini_bundle(
        ...
        eval_source_manifest_family="swedish_checkpoint_dev",
        train_line_start=2,
        train_line_end=3,
        eval_line_start=1,
        eval_line_end=1,
...
def test_materialize_mini_bundle_resolves_dated_train_source_bundle_root(tmp_path: Path) -> None:
    """
    The mini-bundle copier should fall back to the canonical dated Qwen pilot training train bundle.
    """
    placeholder_train_bundle = tmp_path / "qwen3-tts-swedish-pilot-bundle"
    dated_train_bundle = tmp_path / "qwen3-tts-swedish-pilot-bundle-20260314T111615Z-refmel"
    eval_source_bundle = tmp_path / "eval-source-bundle"
    _write_manifest(
