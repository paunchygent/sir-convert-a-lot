"""Focused tests for the T243 Story 31 residual/output assessment.

Purpose:
    Lock the post-T241 residual-path split contract so the converged fp32
    output-cap winner can only open one truthful root-cause localization
    branch.

Relationships:
    - Exercises `story31_post_t241_layer15_residual_output_assessment.py`.
...
from scripts.sir_convert_a_lot.ml.qwen.training import (
    story31_post_t241_layer15_residual_output_assessment as t243_residual_output_assessment,
)
...
_BUILD_ASSESSMENT = (
    t243_residual_output_assessment.build_post_t241_layer15_residual_output_assessment
)


def test_validate_post_t241_layer15_residual_output_contract_rejects_wrong_family() -> None:
    """The T243 split probe should accept only the fixed converged winner."""
    settings = Story31StabilityLabSettings(
        ...
        stabilization_variants=(_T243_VARIANT, "off"),
        build_image=False,
    )

    with pytest.raises(SystemExit, match="supports only the fixed T237/T240/T241 winner"):
        t243_residual_output_assessment.validate_post_t241_layer15_residual_output_contract(
            settings
        )


def test_build_post_t241_layer15_residual_output_assessment_marks_residual_input() -> None:
    ...
    assert assessment == Story31PostT241Layer15ResidualOutputAssessment(
        stabilization_variant=_T243_VARIANT,
...
def test_build_post_t241_layer15_residual_output_assessment_marks_residual_sum() -> None:
...
def test_build_post_t241_layer15_residual_output_assessment_marks_output_return() -> None:
...
def test_build_post_t241_layer15_residual_output_assessment_marks_disagreement() -> None:
...
def test_build_post_t241_layer15_residual_output_assessment_marks_regression() -> None:
    """Rows outside the committed corridor should close T243 as regression evidence."""
    assessment = _BUILD_ASSESSMENT(
        settings=_settings(),
        matrix_rows=_matrix_rows(
            pair="talker_core.layer_15.output",
...
def _settings() -> Story31StabilityLabSettings:
    return Story31StabilityLabSettings(
        ...
        text_embedding_mask_policy="text_span_only",
        hook_profile=TALKER_CORE_POST_T241_LAYER15_RESIDUAL_OUTPUT_HOOK_PROFILE,
        stabilization_variants=(_T243_VARIANT,),
        build_image=False,
    )
