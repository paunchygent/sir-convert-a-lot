"""Focused tests for the T244 Story 31 output-return assessment.

Purpose:
    Lock the post-T243 return-path split contract so the converged fp32
    output-cap winner can only open one truthful follow-on branch.

Relationships:
    - Exercises `story31_post_t243_layer15_output_return_assessment.py`.
...
from scripts.sir_convert_a_lot.ml.qwen.training import (
    story31_post_t243_layer15_output_return_assessment as t244_output_return_assessment,
)
...
_BUILD_ASSESSMENT = t244_output_return_assessment.build_post_t243_layer15_output_return_assessment


def test_validate_post_t243_layer15_output_return_contract_rejects_wrong_family() -> None:
    """The T244 split probe should accept only the fixed converged winner."""
    settings = Story31StabilityLabSettings(
        ...
        stabilization_variants=(_T244_VARIANT, "off"),
        build_image=False,
    )

    with pytest.raises(SystemExit, match="supports only the fixed T237/T243 winner"):
        t244_output_return_assessment.validate_post_t243_layer15_output_return_contract(settings)


def test_build_post_t243_layer15_output_return_assessment_marks_pre_scale_output() -> None:
    ...
    assert assessment == Story31PostT243Layer15OutputReturnAssessment(
        stabilization_variant=_T244_VARIANT,
...
def test_build_post_t243_layer15_output_return_assessment_marks_output_return() -> None:
...
def test_build_post_t243_layer15_output_return_assessment_marks_layer16_input() -> None:
...
def test_build_post_t243_layer15_output_return_assessment_marks_disagreement() -> None:
...
def test_build_post_t243_layer15_output_return_assessment_marks_regression() -> None:
    """Rows outside the committed corridor should close T244 as regression evidence."""
    assessment = _BUILD_ASSESSMENT(
        settings=_settings(),
        matrix_rows=_matrix_rows(
            pair="talker_core.layer_15.output",
...
def _settings() -> Story31StabilityLabSettings:
    return Story31StabilityLabSettings(
        ...
        text_embedding_mask_policy="text_span_only",
        hook_profile=TALKER_CORE_POST_T243_LAYER15_OUTPUT_RETURN_HOOK_PROFILE,
        stabilization_variants=(_T244_VARIANT,),
        build_image=False,
    )
