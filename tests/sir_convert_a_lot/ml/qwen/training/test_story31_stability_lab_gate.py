"""Tests for the Story 31 stability-lab promotion gate.

Purpose:
    Prove the first local promotion rule evaluates the exact fresh-start
    failure family from Story 31 results and does not require another bespoke
    harness.

Relationships:
    - Exercises `story31_stability_lab_gate.py`.
    - Complements the Story 31 lab runner tests by checking the promotion
...
def test_story31_stability_lab_gate_cli_reads_existing_results_and_writes_gate_artifacts(
    tmp_path: Path,
    ...
    assert "# Story 31 Promotion Gate" in (tmp_path / "gate.md").read_text(encoding="utf-8")
