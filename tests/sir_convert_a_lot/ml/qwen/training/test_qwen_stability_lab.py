"""Tests for the Qwen stability lab talker-core stability lab.
...
Relationships:
    - Exercises `qwen_stability_lab_runner.py` and
      `qwen_stability_lab.py`.
...
from scripts.sir_convert_a_lot.ml.qwen.training import qwen_stability_lab_runner
...
def test_default_source_bundle_root_reuses_canonical_training_bundle_default() -> None:
    """Qwen stability lab should inherit the canonical Qwen pilot bundle default."""
...
def test_build_matrix_rows_captures_case_signal_and_anomaly_operator() -> None:
    """Compact Qwen stability lab rows should keep the exact failure signal needed for promotion."""
...
def test_qwen_stability_lab_cli_runs_and_persists_compact_artifacts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """The public Qwen stability lab command should persist one compact matrix report."""

    def fake_run_stability_lab(*_args: object, **_kwargs: object) -> QwenStabilityLabReport:
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.qwen_stability_lab.run_stability_lab",
    ...
    assert "# Qwen stability lab Stability Lab" in (tmp_path / "results.md").read_text(
        encoding="utf-8"
    )
...
def test_run_stability_lab_reuses_one_bundle_and_writes_variant_reports(
    ...
    """The Qwen stability lab runner should emit one compact matrix report across variants."""
...
def test_build_sub_boundary_assessment_constrains_normalization_entry_to_one_micro_family() -> None:
    """
    sub-boundary should shape input-layernorm family from the narrowed pair-vs-single sub-boundary
    evidence.
    """
    ...
    assert assessment == QwenSubBoundaryAssessment(
        stabilization_variant="layer16_gated_fp32_rescale_1e3_layer16_out_0p5_layer15_out_0p5",
...
def test_validate_hook_profile_variant_contract_allows_normalization_entry_micro_family() -> None:
    """The narrowed handoff profile admits the exact diagnosed input-layernorm family."""
    settings = QwenStabilityLabSettings(
        output_root=Path("/tmp/qwen_stability"),
        dockerfile_path=Path("Dockerfile"),
        image="test-image",
