"""Tests for the T221 historical Task 101 control surface.

Purpose:
    Verify the committed T221 CLI writes the historical contract diff and
    routes launch metadata through the dedicated runtime surface instead of the
    invalid T220 approximation.

Relationships:
    - Exercises `t221_historical_control.py`.
...
def test_launch_writes_contract_diff_and_launch_metadata(
    ...
    """Launching T221 should persist the contract diff and latest pointer."""

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.t221_historical_control._validate_launch_environment",
        lambda settings, historical_bundle_root: None,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.t221_historical_control._validate_historical_bundle",
        lambda historical_bundle_root, train_manifest_family, eval_manifest_family: (8445, 8),
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.t221_historical_control.run_checked",
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.t221_historical_control.prepare_runtime_dependencies",
        lambda **kwargs: (
            False,
            "sha256:task100",
    ...
    def fake_launch(settings, **kwargs) -> DetachedLaunch:
        return DetachedLaunch(
            ...
            tracking={"project_name": "task101-qwen-pilot"},
            diagnostic={"kind": "t221_historical_task101_control"},
        )

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.t221_historical_control.launch_detached_historical_control",
        fake_launch,
    )
    ...
    assert result == 0
    launch_root = tmp_path / "task221-proof"
    ...
    assert contract_payload["invalid_t220"]["batch_size"] == 8
    assert (
        contract_payload["t221_recreation"]["text_embedding_assembly_mode"] == "full_channel_masked"
    )
    assert contract_payload["t221_recreation"]["text_embedding_mask_policy"] == "text_span_only"
