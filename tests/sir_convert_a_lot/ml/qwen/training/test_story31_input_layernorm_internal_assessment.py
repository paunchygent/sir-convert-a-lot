"""Focused tests for the Story 31 T233 input-layernorm internal assessment.

Purpose:
    Lock the T233 validation and interpretation contract so the next mechanism
    family is derived from one consistent internal normalization surface rather
    than from mixed exploratory evidence.

Relationships:
    - Exercises `story31_input_layernorm_internal_assessment.py`.
...
def test_validate_input_layernorm_internal_contract_rejects_multi_variant_family() -> None:
    """T233 should stay baseline-only and reject any widened variant family."""
    with pytest.raises(SystemExit, match="supports only the ranked T229/T230 baseline variant"):
        validate_input_layernorm_internal_contract(
            _settings(stabilization_variants=(T233_REQUIRED_VARIANT, "off"))
        )
...
def test_build_input_layernorm_internal_assessment_marks_disagreement_ambiguous() -> None:
    ...
    assert (
        assessment.next_micro_family_rule
        == "The next task remains blocked until the T233 ambiguity is resolved."
    )


def _settings(
    *,
    stabilization_variants: tuple[str, ...] = (T233_REQUIRED_VARIANT,),
) -> Story31StabilityLabSettings:
