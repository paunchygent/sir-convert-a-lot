"""Tests for Qwen backward-lineage and fresh-start proof lane backward-lineage mini-bundle
materialization.

Purpose:
    Prove the backward-lineage mini-bundle copier preserves the exact selected prepared
    rows and bundle-local assets for the bounded backward-lineage probe.

Relationships:
    - Exercises `qwen_backward_lineage_bundle.py`.
...
def test_materialize_backward_lineage_bundle_copies_selected_rows_and_assets(
    ...
    assert first_row["qwen_lineage_source_manifest_line_number"] == 4
    assert second_row["qwen_lineage_source_manifest_line_number"] == 2
