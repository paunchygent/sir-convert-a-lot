"""
Focused tests for the downstream convergence Qwen stability lab downstream convergence assessment.

Purpose:
    Lock the post-row-local micro-family downstream split contract so the winning fp32-output-cap
    member can only open one truthful layer-15 split branch.

Relationships:
    - Exercises `qwen_downstream_convergence_assessment.py`.
...
_DOWNSTREAM_CONVERGENCE_VARIANT = (
...
_BUILD_ASSESSMENT = downstream_convergence_assessment.build_downstream_convergence_assessment


def test_validate_downstream_convergence_contract_rejects_wrong_family() -> None:
    """
    The downstream convergence downstream probe should accept only the fixed row-local micro-family
    winner.
    """
    settings = QwenStabilityLabSettings(
        ...
        hook_profile=TALKER_CORE_POST_ROW_LOCAL_MICRO_FAMILY_DOWNSTREAM_CONVERGENCE_HOOK_PROFILE,
        stabilization_variants=(_DOWNSTREAM_CONVERGENCE_VARIANT, "off"),
        build_image=False,
    )

    with pytest.raises(SystemExit, match="supports only the fixed ROW_LOCAL_MICRO_FAMILY winner"):
        downstream_convergence_assessment.validate_downstream_convergence_contract(settings)


def test_build_downstream_convergence_assessment_marks_mlp_down_proj() -> None:
    """All-three agreement at `down_proj` should open the narrowest layer-15 split split."""
    ...
    assert assessment == QwenDownstreamConvergenceAssessment(
        stabilization_variant=_DOWNSTREAM_CONVERGENCE_VARIANT,
        ...
        comparison_rows=(
            PostROW_LOCAL_MICRO_FAMILYDownstreamConvergenceComparisonRow(
                ...
                first_non_finite_talker_core_hook_tensor="talker_core.layer_15.mlp.down_proj",
                matched_corridor_surface="talker_core.layer_15.mlp.down_proj",
            ),
            PostROW_LOCAL_MICRO_FAMILYDownstreamConvergenceComparisonRow(
        ...
        convergence_classification="converged_mlp_down_proj",
        dominant_surface="talker_core.layer_15.mlp.down_proj",
        ...
        next_task_rule=(
            ...
            "`talker_core.layer_15.mlp.down_proj` under "
...
def test_build_downstream_convergence_assessment_marks_layer15_output() -> None:
    """All-three agreement at `layer_15.output` should stay residual/output truth."""
    assessment = _BUILD_ASSESSMENT(
        settings=_settings(),
        matrix_rows=_matrix_rows(
    ...
    assert assessment.convergence_classification == "converged_layer15_output"
    assert assessment.dominant_surface == "talker_core.layer_15.output"


def test_build_downstream_convergence_assessment_marks_layer16_input_handoff() -> None:
    """All-three agreement at `layer_16.input` should stay handoff truth."""
    assessment = _BUILD_ASSESSMENT(
        settings=_settings(),
        matrix_rows=_matrix_rows(
...
def test_build_downstream_convergence_assessment_marks_downstream_disagreement() -> None:
    """Mixed downstream rows should stay disagreement rather than a generic seam."""
    assessment = _BUILD_ASSESSMENT(
        settings=_settings(),
        matrix_rows=_matrix_rows(
            pair="talker_core.layer_15.mlp.down_proj",
...
def test_build_downstream_convergence_assessment_marks_regression() -> None:
    """Upstream guard reuse should close downstream convergence as regression evidence."""
    assessment = _BUILD_ASSESSMENT(
        settings=_settings(),
        matrix_rows=_matrix_rows(
            pair="talker_core.layer_15.output",
    ...
    assert assessment is not None
    assert assessment.convergence_classification == "upstream_or_nonlocal_regression"
...
def _settings() -> QwenStabilityLabSettings:
    return QwenStabilityLabSettings(
        ...
        text_embedding_mask_policy="text_span_only",
        hook_profile=TALKER_CORE_POST_ROW_LOCAL_MICRO_FAMILY_DOWNSTREAM_CONVERGENCE_HOOK_PROFILE,
        stabilization_variants=(_DOWNSTREAM_CONVERGENCE_VARIANT,),
        build_image=False,
    )
