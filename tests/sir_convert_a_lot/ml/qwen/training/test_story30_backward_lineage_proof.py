"""Tests for the Story 30 backward-lineage proof surface.

Purpose:
    Verify the T212 proof CLI prepares deterministic proof packages and drives
    the committed remote Hemma surface without ad hoc shell glue.

Relationships:
    - Exercises `story30_backward_lineage_proof.py`.
...
def test_prepare_writes_backward_lineage_package(
    ...
    """Preparing T212 should write a deterministic proof package."""
    result = main(["prepare", "--output-root", tmp_path.as_posix(), "--proof-id", "t212-proof"])
    capsys.readouterr()

    assert result == 0
    proof_root = tmp_path / "t212-proof"
...
def test_launch_uses_remote_story30_backward_lineage_surface(
    ...
    prepare_result = main(
        ["prepare", "--output-root", tmp_path.as_posix(), "--proof-id", "t212-proof"]
    )
    ...
    def fake_run(
        ...
        return subprocess.CompletedProcess(
            command,
            0,
            json.dumps({"launch_id": "t212-proof-backward-lineage", "pid": 1234}),
            "",
        )

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.story30_backward_lineage_runtime.subprocess.run",
        fake_run,
    )

    result = main(["launch", "--output-root", tmp_path.as_posix(), "--proof-id", "t212-proof"])
    capsys.readouterr()
...
def test_remote_launch_starts_detached_worker(
    ...
    def fake_launch(**kwargs: object) -> object:
        ...
        return type(
            ...
            (),
            {
                ...
                "launch_id": "t212-proof-backward-lineage",
                "pid": 4321,
                "repo_root": Path.cwd().as_posix(),
                "output_root": (tmp_path / "remote-proof" / "t212-proof").as_posix(),
                "log_path": (tmp_path / "remote-proof" / "t212-proof" / "proof.log").as_posix(),
                "worker_status_path": (
                    tmp_path / "remote-proof" / "t212-proof" / "worker-status.json"
                ).as_posix(),
                "report_path": (
                    tmp_path / "remote-proof" / "t212-proof" / "report.json"
                ).as_posix(),
                "failure_path": (
                    tmp_path / "remote-proof" / "t212-proof" / "failure.txt"
                ).as_posix(),
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.training.story30_backward_lineage_proof.launch_detached_backward_lineage_proof",
        fake_launch,
    )
    ...
    assert result == 0
    assert captured["launch_id"] == "t212-proof-backward-lineage"
