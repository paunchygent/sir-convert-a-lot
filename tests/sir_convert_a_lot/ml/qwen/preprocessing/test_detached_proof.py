"""Tests for the detached Qwen proof runner and runtime."""
...
from scripts.sir_convert_a_lot.cli.ml.qwen_detached_proof import (
...
from scripts.sir_convert_a_lot.ml.qwen.preprocessing.detached_proof import (
    DetachedProofLaunch,
    build_detached_proof_command,
    inspect_detached_proof,
)


def test_detached_proof_parser_launch_defaults() -> None:
    """The detached proof runner should expose deterministic launch defaults."""
...
def test_build_detached_proof_command_drops_rm_and_adds_name() -> None:
    """The detached proof command should launch a named background container."""
    settings = ContainerizedPreprocessingSettings(
        output_root=Path("build/verification/qwen-containerized-preprocessing"),
        runs_root=Path("/srv/scratch/sir-convert-a-lot/build/runs/qwen3-tts-swedish-preprocessing"),
        run_id="proof-run",
        run_root=None,
        promote_on_success=False,
        stage="row-processing",
        resume_row_processing=False,
        finalization_families=("swedish_smoke_train", "swedish_pilot_train"),
    ...
    command = build_detached_proof_command(
        ...
        container_name="qwen-proof-20260309t120000z",
    )

    assert command[0:4] == ["run", "-d", "--name", "qwen-proof-20260309t120000z"]
...
def test_load_launch_reads_recorded_metadata(tmp_path: Path) -> None:
    """The detached proof status surface should load prior launch metadata."""
        ...
        json.dumps(
            {
                ...
                "run_root": (
                    ...
                    "qwen3-tts-swedish-preprocessing/qwen-proof-20260309t120000z"
                ),
                "promoted_root": (
                    "/home/paunchygent/apps/sir-convert-a-lot/build/reference/"
                    "qwen3-tts-swedish-corpus"
                ),
    ...
    assert launch.container_name == "qwen-proof-20260309t120000z"
    assert launch.command == ["sudo", "-n", "docker", "run", "-d"]


def test_inspect_detached_proof_reads_container_and_report(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The detached proof status surface should combine Docker and report state."""
    repo_root = tmp_path / "repo"
    run_root = tmp_path / "runs/qwen-proof-20260309t120000z"
    ...
    launch = DetachedProofLaunch(
        ...
        run_root=report_path.parent.as_posix(),
        promoted_root=(repo_root / "build/reference/qwen3-tts-swedish-corpus").as_posix(),
        command=["sudo", "-n", "docker", "run", "-d"],
    )
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.preprocessing.detached_proof.docker_checked",
    ...
    assert status.report["prepared_rows"] == 23
