"""Tests for the containerized Qwen preprocessing surface."""
...
def test_containerized_preprocessing_parse_args_defaults() -> None:
    """The runner should expose deterministic defaults."""
    settings = _parse_args([])

    assert settings.output_root == DEFAULT_OUTPUT_ROOT
    assert settings.runs_root == DEFAULT_RUNS_ROOT
    assert settings.run_id is None
    assert settings.run_root is None
    assert settings.promote_on_success is False
    assert settings.stage == DEFAULT_STAGE
    assert settings.finalization_families == DEFAULT_FINALIZATION_FAMILIES
...
def test_containerized_preprocessing_parse_args_rejects_stage_all_without_explicit_override() -> (
    None
):
    """The runner should reject canonical use of `stage=all`."""
    with pytest.raises(SystemExit, match="no longer treats `stage=all` as canonical"):
        _parse_args(["--stage", "all"])


def test_containerized_preprocessing_build_command_uses_repo_and_absolute_mounts() -> None:
    ...
    settings = ContainerizedPreprocessingSettings(
        ...
        stage="finalization",
        finalization_families=("swedish_scaleup_train",),
...
def test_containerized_preprocessing_parses_inner_report(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The runtime should parse the inner preprocessing report from mixed stdout."""
    settings = ContainerizedPreprocessingSettings(
        output_root=Path("build/verification/qwen-containerized-preprocessing"),
        runs_root=Path("/srv/scratch/sir-convert-a-lot/build/runs/qwen3-tts-swedish-preprocessing"),
        run_id=None,
    ...
    def _fake_docker_checked(args: list[str], *, label: str) -> str:
        assert args[0] == "run"
        assert label == "docker run qwen containerized preprocessing"
        return (
            "\n********\n"
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.ml.qwen.preprocessing.containerized.docker_checked",
