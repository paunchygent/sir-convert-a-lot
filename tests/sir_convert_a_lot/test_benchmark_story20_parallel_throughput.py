"""Tests for the Task 72 parallel throughput benchmark runner.

Purpose:
    Validate payload shape and output-path policy for deterministic Task 72
    benchmark evidence generation.

Relationships:
    - Tests `scripts.sir_convert_a_lot.benchmark_story20_parallel_throughput`.
    - Protects the evidence workflow used to terminalize Task 72 under Story 20.
"""
...
from scripts.sir_convert_a_lot.benchmark_story20_parallel_throughput import (
    DEFAULT_OUTPUT_JSON,
    run_benchmark,
)
...
def test_run_benchmark_writes_expected_payload(tmp_path: Path) -> None:
    output_json = tmp_path / "task72-throughput.json"
    ...
    assert payload["benchmark_id"] == "task-72-parallel-throughput"
...
def test_run_benchmark_rejects_docs_reference_output_path(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="must not target docs/reference"):
        run_benchmark(
            output_json=Path("docs/reference/forbidden-task72-throughput.json"),
            data_root=tmp_path / "runtime_data",
        )
