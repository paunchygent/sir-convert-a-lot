...
Purpose:
    Verify the Task 205 selector, runtime orchestration, timer rendering, and
    CLI reporting without touching real Hemma storage or systemd state.

Relationships:
    - Covers `task205_hemma_scratch_maintenance_selection.py`.
    - Covers `task205_hemma_scratch_maintenance_runtime.py`.
    - Covers `task205_hemma_scratch_timer_runtime.py`.
    - Covers the Task 205 branches in `run_task204_hemma_scratch_policy.py`.
"""
...
from scripts.sir_convert_a_lot.devops.task205_hemma_scratch_maintenance_runtime import (
...
def test_select_maintenance_candidates_skips_kept_recent_and_protected(
    ...
    """Task 205 selection should keep the newest roots and protected RCA artifacts."""
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task205_hemma_scratch_maintenance_selection.directory_size_bytes",
...
def test_run_scratch_maintenance_blocks_when_qwen_containers_are_active(
    ...
    """Task 205 should not archive anything while a Qwen workload is active."""
    scratch_root = tmp_path / "scratch"
    scratch_root.mkdir(parents=True)
    block_file = scratch_root / ".block"

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task205_hemma_scratch_maintenance_runtime.scratch_free_bytes",
        lambda _: 10,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task205_hemma_scratch_maintenance_runtime.active_qwen_container_names",
        lambda: ["qwen-train-task198"],
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task205_hemma_scratch_maintenance_runtime.select_maintenance_candidates",
...
def test_run_scratch_maintenance_archives_only_until_target_headroom(
    ...
    """Task 205 should archive only the candidates needed to reach target headroom."""
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task205_hemma_scratch_maintenance_runtime.archive_scratch_paths",
        fake_archive,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.task205_hemma_scratch_maintenance_runtime.cleanup_non_active_docker_state",
...
def test_render_service_unit_uses_qwen_scratch_policy_maintain_command(tmp_path: Path) -> None:
    """Task 205 timer service should call the committed maintenance surface."""
...
def test_task205_runner_writes_maintenance_and_timer_reports(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Task 205 runner should write deterministic maintenance and timer artifacts."""
    output_root = tmp_path / "build" / "verification" / "task-205"
    ...
    maintain_exit_code = run_task204_hemma_scratch_policy.main(
        ["maintain", "--output-root", output_root.as_posix()]
    )
    install_exit_code = run_task204_hemma_scratch_policy.main(
        ["install-timer", "--output-root", output_root.as_posix()]
    )
    status_exit_code = run_task204_hemma_scratch_policy.main(
    ...
    assert "Task 205 Scratch Maintenance" in (output_root / "maintain.md").read_text(
        encoding="utf-8"
    )
    assert (
