"""Tests for the Task 87 Chatterbox tuning sweep runner.
...
Relationships:
    - Exercises `run_task87_chatterbox_tuning_sweep`.
    - Complements the lower-level Task 86 benchmark tests.
"""
...
from scripts.sir_convert_a_lot.devops import run_task87_chatterbox_tuning_sweep


def test_default_lanes_use_conservative_first_order() -> None:
    lanes = run_task87_chatterbox_tuning_sweep._default_lanes()
    ...
    assert lanes[0].output_root == Path("build/verification/task-87-chatterbox-exag-0p5-cfg-0p5")


def test_parse_args_defaults_to_documented_probe_and_remote_root() -> None:
    settings = run_task87_chatterbox_tuning_sweep._parse_args([])
...
def test_run_lane_builds_canonical_run_hemma_command(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _fake_run(command: list[str], cwd: Path, check: bool) -> _Completed:
        assert cwd == run_task87_chatterbox_tuning_sweep.REPO_ROOT
        assert check is False
        commands.append(command)
        return _Completed()

    monkeypatch.setattr(
        run_task87_chatterbox_tuning_sweep.subprocess,
        "run",
        _fake_run,
    )
    settings = run_task87_chatterbox_tuning_sweep.SweepSettings(
        output_root=Path("build/verification/task-87-chatterbox-tuning-sweep"),
    ...
    lane = run_task87_chatterbox_tuning_sweep.SweepLane(
        slug="exag-0p5-cfg-0p5",
        exaggeration=0.5,
        cfg_weight=0.5,
        output_root=Path("build/verification/task-87-chatterbox-exag-0p5-cfg-0p5"),
    )

    result = run_task87_chatterbox_tuning_sweep._run_lane(settings, lane)

    assert result == 0
    command = commands[0]
    assert command[:7] == ["pdm", "run", "run-hemma", "--", "pdm", "run", "benchmark:task-86"]
...
def test_write_summary_records_lane_results(tmp_path: Path) -> None:
    ...
    lanes = [
        run_task87_chatterbox_tuning_sweep.LaneResult(
            slug="exag-0p5-cfg-0p5",
            exaggeration=0.5,
            cfg_weight=0.5,
            output_root="build/verification/task-87-chatterbox-exag-0p5-cfg-0p5",
            returncode=0,
            synthesized_ok=True,
    ...
    run_task87_chatterbox_tuning_sweep._write_summary(
        output_root=output_root,
        settings=settings,
        lanes=lanes,
    )
    ...
    assert payload["benchmark_id"] == "task-87-chatterbox-tuning-sweep"
    assert payload["lanes"][0]["slug"] == "exag-0p5-cfg-0p5"
    markdown = (output_root / "report.md").read_text(encoding="utf-8")
    assert "Task 87 Chatterbox Tuning Sweep" in markdown
