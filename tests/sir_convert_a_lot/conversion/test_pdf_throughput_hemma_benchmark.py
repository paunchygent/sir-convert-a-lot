"""Tests for the canonical Hemma PDF throughput benchmark runner.

Purpose:
    Validate the env-contract parsing and revision checks that gate the Hemma
    PDF throughput benchmark workflow before any long-running benchmark is started.

Relationships:
    - Tests `scripts.sir_convert_a_lot.devops.run_pdf_throughput_hemma_benchmark`.
    - Protects the canonical Hemma workflow used to prepare env/runtime parity
      before invoking the PDF throughput benchmark harness.
"""
...
from scripts.sir_convert_a_lot.devops import run_pdf_throughput_hemma_benchmark
...
def _empty_command(
    ...
    *,
    label: str,
    cwd: Path = run_pdf_throughput_hemma_benchmark.CANONICAL_REPO_ROOT,
    redactions: tuple[str, ...] = (),
) -> str:
    _ = (argv, label, cwd, redactions)
    return ""


def _remote_sha_command(
    argv: list[str],
    *,
    label: str,
    cwd: Path = run_pdf_throughput_hemma_benchmark.CANONICAL_REPO_ROOT,
...
def test_verify_env_contract_accepts_canonical_symlink(
    ...
    monkeypatch.setattr(run_pdf_throughput_hemma_benchmark, "CANONICAL_ENV_PATH", canonical_env)
    monkeypatch.setattr(run_pdf_throughput_hemma_benchmark, "CANONICAL_ENV_LINK", repo_env_link)
    monkeypatch.setattr(
        run_pdf_throughput_hemma_benchmark,
        "_run_command",
        _empty_command,
    )

    contract = run_pdf_throughput_hemma_benchmark._verify_env_contract()
...
def test_verify_env_contract_rejects_missing_required_keys(
    ...
    with pytest.raises(SystemExit, match="missing required PDF throughput benchmark keys"):
        run_pdf_throughput_hemma_benchmark._verify_env_contract()
...
def test_verify_expected_revision_rejects_mismatch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(SystemExit, match="does not match expected revision"):
        run_pdf_throughput_hemma_benchmark._verify_expected_revision("expected-sha")


def test_parse_args_switches_to_two_worker_sweep_defaults() -> None:
    settings = run_pdf_throughput_hemma_benchmark._parse_args(
        [
            "--expected-revision",
            "abc1234",
            "--two-worker-sweep",
    ...
    assert settings.output_json == run_pdf_throughput_hemma_benchmark.DEFAULT_SWEEP_OUTPUT_JSON
    assert settings.output_report == run_pdf_throughput_hemma_benchmark.DEFAULT_SWEEP_OUTPUT_REPORT
    assert settings.corpus_root == run_pdf_throughput_hemma_benchmark.DEFAULT_SWEEP_CORPUS_ROOT
    assert settings.data_root == run_pdf_throughput_hemma_benchmark.DEFAULT_SWEEP_DATA_ROOT
...
def test_main_stdout_excludes_performance_metrics(
    ...
    def execute_workflow_stub(
        settings: run_pdf_throughput_hemma_benchmark.HemmaPdfThroughputSettings,
    ) -> dict[str, object]:
    ...
    exit_code = run_pdf_throughput_hemma_benchmark.main(
        [
            "--expected-revision",
            "abc1234",
            "--output-json",
...
def test_run_pdf_throughput_benchmark_uses_scratch_backed_miopen_cache(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    monkeypatch.setattr(run_pdf_throughput_hemma_benchmark, "_run_command", run_command_stub)
    settings = run_pdf_throughput_hemma_benchmark._parse_args(
        [
            "--expected-revision",
            "abc1234",
    ...
    run_pdf_throughput_hemma_benchmark._run_pdf_throughput_benchmark(
        settings,
        api_key="secret-key",
        ...
        deployed_profile=run_pdf_throughput_hemma_benchmark.ProfileSpec(
    ...
    assert commands[-1][:3] == ["pdm", "run", "benchmark:pdf-throughput"]
