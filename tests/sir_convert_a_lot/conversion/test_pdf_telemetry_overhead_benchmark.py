"""Tests for telemetry overhead benchmark telemetry overhead benchmark runner.
...
Relationships:
    - Tests `scripts.sir_convert_a_lot.pdf_telemetry_overhead_benchmark`.
    - Protects telemetry overhead benchmark evidence workflow under PDF throughput lane.
"""
...
from scripts.sir_convert_a_lot.pdf_telemetry_overhead_benchmark import (
    DEFAULT_OUTPUT_JSON,
    run_benchmark,
)
...
def test_run_benchmark_writes_expected_payload(tmp_path: Path) -> None:
    output_json = tmp_path / "pdf-telemetry-overhead.json"
    ...
    assert payload["benchmark_id"] == "pdf-telemetry-overhead-telemetry-overhead"
...
def test_run_benchmark_rejects_docs_reference_output_path(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="must not target docs/reference"):
        run_benchmark(
            output_json=Path("docs/reference/forbidden-pdf-telemetry-overhead.json"),
            data_root=tmp_path / "runtime_data",
            total_jobs=3,
            max_workers=2,
