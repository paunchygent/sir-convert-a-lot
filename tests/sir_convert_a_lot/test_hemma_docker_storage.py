"""Tests for Hemma Docker storage-root remediation.
...
Relationships:
    - Covers `hemma_docker_storage_runtime.py`.
    - Covers `run_hemma_docker_storage_remediation.py`.
    - Protects the host-wide Docker storage contract introduced by Hemma Docker storage remediation.
"""
...
from scripts.sir_convert_a_lot.devops import run_hemma_docker_storage_remediation
...
def test_ensure_fstab_bind_entry_text_appends_once(tmp_path: Path) -> None:
    """Hemma Docker storage remediation should add the bind-mount fstab entry exactly once."""
...
def test_find_mount_source_returns_none_for_plain_directory(
    ...
    """Hemma Docker storage remediation should ignore plain directories that are not mountpoints."""
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.hemma_docker_storage_runtime.subprocess.run",
        _fake_run,
    )
...
def test_runner_writes_report_artifacts(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Hemma Docker storage remediation runner should write deterministic report files."""
    output_root = tmp_path / "build" / "verification" / "hemma-docker-storage"
    expected_report = HemmaDockerStorageReport(
        docker_root="/var/snap/docker/common/var-lib-docker",
        scratch_docker_root="/srv/scratch/docker/data-root",
        docker_root_backup="/var/snap/docker/common/var-lib-docker.hemma-docker-storage-backup",
    ...
    def _fake_run(settings: HemmaDockerStorageSettings) -> HemmaDockerStorageReport:
        return expected_report

    monkeypatch.setattr(
        run_hemma_docker_storage_remediation,
        "run_docker_storage_migration",
        _fake_run,
    )

    exit_code = run_hemma_docker_storage_remediation.main(["--output-root", output_root.as_posix()])
    ...
    assert "Hemma Docker Storage Remediation Report" in report_md_path.read_text(encoding="utf-8")
    assert (
