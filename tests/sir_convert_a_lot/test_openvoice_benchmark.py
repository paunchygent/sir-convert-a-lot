"""Tests for the Hemma OpenVoice benchmark helpers.
...
Relationships:
    - Exercises `run_openvoice_hemma_benchmark` argument parsing.
    - Exercises `openvoice_benchmark_runtime` cache and sidecar helper functions.
"""
...
from scripts.sir_convert_a_lot.devops import run_openvoice_hemma_benchmark
from scripts.sir_convert_a_lot.devops.openvoice_benchmark_reporting import (
    BenchmarkReport,
    BenchmarkStatus,
    CacheEvidence,
    EvidenceStatus,
...
def test_parse_args_accepts_reference_audio_only(tmp_path: Path) -> None:
    ...
    settings = run_openvoice_hemma_benchmark._parse_args(
        [
            "--reference-audio",
    ...
    assert settings.image == "sir-convert-a-lot/openvoice-sidecar-openvoice:local"
...
def test_resolve_effective_cache_dir_uses_home_bind_mount_when_srv_probe_fails(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime._probe_docker_bind_mount",
        _fake_probe,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime._ensure_home_bind_mount",
        _fake_bind,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime._is_srv_cache_path",
...
def test_ensure_image_present_builds_with_buildkit_buildx_load(
    ...
    """OpenVoice benchmark image builds must use BuildKit buildx, not plain docker build."""
    ...
    def _fake_docker_checked(args: list[str], *, label: str) -> str:
        ...
        if label == "docker image inspect after build":
            return "sha256:openvoice"
        return ""

    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime.docker_checked",
    ...
    settings = BenchmarkSettings(
        ...
        image="sir-convert-a-lot/openvoice-sidecar-openvoice:test",
        openvoice_checkpoint_url="https://example.invalid/checkpoints.zip",
        base_model_id="facebook/mms-tts-swe",
        network="sir-openvoice",
        network_alias="openvoice-sidecar",
    ...
    assert image_id == "sha256:openvoice"
    assert (
        [
            "buildx",
            "build",
        ...
        "docker buildx build openvoice image",
    ) in calls
...
def test_start_sidecar_uses_persistent_cache_mounts(
    ...
    def _fake_docker_checked(args: list[str], *, label: str) -> str:
        assert label == "docker run openvoice sidecar"
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime._gpu_device_group_ids",
        lambda: ["993", "44"],
    )
    settings = BenchmarkSettings(
        output_root=tmp_path / "output",
...
def test_prefetch_vad_assets_declares_torch_home_under_hf_cache(
    ...
    def _fake_docker_checked(args: list[str], *, label: str) -> str:
        assert label == "docker run openvoice vad prefetch"
        recorded_commands.append(args)
        return ""
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime._silero_vad_cached",
...
def test_synthesize_probe_posts_normalized_contract_and_writes_artifact(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime._GpuSampler",
        _FakeSampler,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime.httpx.Client",
        _FakeClient,
    )
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime._wav_metadata",
        lambda _: (22050, 1.25),
    )
    reference_audio = tmp_path / "voice.m4a"
...
def test_copy_debug_artifacts_from_container_streams_tar_archive(
    ...
    monkeypatch.setattr(
        "scripts.sir_convert_a_lot.devops.openvoice_benchmark_runtime.subprocess.run",
        _fake_run,
    )
    artifacts_dir = tmp_path / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    ...
    copy_debug_artifacts_from_container(container_name="openvoice", artifacts_dir=artifacts_dir)
...
def test_report_helpers_render_markdown(tmp_path: Path) -> None:
    report = BenchmarkReport(
        benchmark_id="openvoice-v2-hemma",
        ...
        internal_base_url="http://sir-convert-a-lot-openvoice-openvoice:8092",
        ...
        setup_artifacts=SetupArtifactEvidence(
            processed_reference_dir="build/verification/openvoice-v2-hemma/artifacts/processed_reference",
            processed_reference_segment_count=3,
            base_output_path="build/verification/openvoice-v2-hemma/artifacts/base_sv.wav",
            base_output_sample_rate_hz=16000,
            converter_input_path=(
                "build/verification/openvoice-v2-hemma/artifacts/base_sv_converter_input.wav"
            ),
        ...
        synthesis_result=SynthesisProbeResult(
            ...
            byte_count=4096,
            sha256="deadbeef",
            output_path="build/verification/openvoice-v2-hemma/artifacts/sample_sv.wav",
            elapsed_seconds=2.0,
        ...
        docker_logs_path="build/verification/openvoice-v2-hemma/docker_logs.txt",
    )
    ...
    assert '"benchmark_id": "openvoice-v2-hemma"' in json_path.read_text(encoding="utf-8")
...
def test_main_writes_partial_report_when_setup_artifact_export_fails(
    ...
    monkeypatch.setattr(run_openvoice_hemma_benchmark, "_parse_args", lambda argv: settings)
    monkeypatch.setattr(
        run_openvoice_hemma_benchmark,
        "enforce_generated_output_path",
        lambda output_root, *, label: None,
    )
    monkeypatch.setattr(
        run_openvoice_hemma_benchmark, "ensure_sidecar_preconditions", lambda settings: None
    )
    monkeypatch.setattr(
        run_openvoice_hemma_benchmark,
        "run_checked",
    ...
    monkeypatch.setattr(
        run_openvoice_hemma_benchmark, "prefetch_hf_assets", lambda settings, mount: None
    )
    monkeypatch.setattr(
        run_openvoice_hemma_benchmark, "prefetch_vad_assets", lambda settings, mount: None
    )
    monkeypatch.setattr(
        run_openvoice_hemma_benchmark,
        "start_sidecar",
    ...
    monkeypatch.setattr(run_openvoice_hemma_benchmark, "synthesize_probe", _fake_synthesize_probe)
    monkeypatch.setattr(
        run_openvoice_hemma_benchmark,
        "copy_debug_artifacts_from_container",
        lambda container_name, artifacts_dir: (_ for _ in ()).throw(
            SystemExit(
                "OpenVoice benchmark could not copy debug artifacts from the sidecar container.\n"
                "stdout:\n\nstderr:\npermission denied"
            )
        ),
    )
    ...
    exit_code = run_openvoice_hemma_benchmark.main([])
